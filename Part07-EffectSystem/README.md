...
5부는 효과를 모나드로 인코딩했고, 6부는 그 효과들을 한 스택으로 쌓았습니다. 그런데 가장 흔한 효과, 곧 실제 부수 작용 (콘솔 출력, 파일 읽기, 네트워크 호출, 시간 조회) 은 아직 다루지 않았습니다.

7부의 출발점은 함수형이 부작용 (side-effect) 을 순수하게 다루는 핵심 발상, 곧 **효과를 즉시 실행하지 않고 값으로 인코딩** 하는 것이고, `IO<A>` 와 `Eff` 가 그 발상의 구현체입니다. 콘솔 출력도, 파일 읽기도, 예외도 일단 값이 되면 합성·재시도·테스트의 대상이 됩니다. 이것이 책 목표 4 (실무의 자신감) 의 정면입니다.

- `IO<A>` — 부수 작용을 즉시 실행하지 않고 DSL 노드 (`IOBind` / `IOApply` / `IOCatch` …) 의 트리로 인코딩 합니다. `EnvIO` (취소 토큰 · 자원 · 동기화 컨텍스트) 위에서 `Run` 될 때 비로소 실행됩니다. 자유 모나드 / 연속 (continuation) 인코딩과 스택 안전성을 직접 구현으로 봅니다.
...
- `IO<A>` 가 값으로 인코딩된 효과 프로그램임을 — 즉 부수 작용이 `Run` 전까지 일어나지 않음을 — DSL 노드의 직접 구현으로 이해.
...
`Run` 전까지 아무 부수 작용도 일어나지 않는 효과 프로그램을 직접 짜고 읽어내는 능력이 7부의 도달점입니다.
...
| 시민 | 시그니처 (개념) | 의미 |
|---|---|---|
| **IO\<A\>** | `EnvIO → A` (DSL 인코딩) | `Run` 될 때 부수 작용을 수행하고 `A` 를 돌려주는 효과 프로그램. |
...
기초의 비유가 정점에 닿습니다. Elevated World 로 lift 한다는 한 동사가, 여기서는 부수 작용을 값으로 lift 해 실행을 미룬다는 의미가 됩니다.
...
`IO<A>` 는 부수 작용을 즉시 실행하지 않고 DSL 노드 (`IOBind` / `IOApply` / `IOCatch` / `IOFail` …) 의 트리로 인코딩합니다. 학습용 `MyIO<A>` 를 직접 정의하고 `Run(EnvIO)` 에서 그 트리를 해석하는 인터프리터를 구현합니다. 깊은 `bind` 체인이 스택을 터뜨리지 않도록 트램폴린으로 스택 안전성을 확보하는 자리를 봅니다.
