...
이 대비가 끌어올림 (lift) 과 `Pure` 의 자리 차이입니다. 1장에서 끌어올림은 "아래 세계의 값이나 함수를 위 세계의 어휘로 들어올리는 일" 이었습니다. `lift` 는 그 발상을 변환기에 씁니다. 이미 내부 효과 (`Many` 의 여러 갈래) 를 가진 계산을, 그 효과를 보존한 채 한 층 위 스택으로 들어올립니다. `Pure` 는 효과가 아예 없는 단일 값을 올리는 가장 단순한 진입점입니다. 둘 다 결과는 `Right` 지만, `lift` 는 효과를 통과시키고 `Pure` 는 효과 없는 값을 올린다는 점이 다릅니다.
