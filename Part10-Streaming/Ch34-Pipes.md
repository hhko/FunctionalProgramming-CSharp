...
여기가 세 역할 중 가장 중요한 통찰입니다. `Pipe<I, O>` 속에 든 것은 `Func<Producer<I>, Producer<O>>`, 곧 생산자를 받아 생산자를 내놓는 함수입니다. 파이프는 흐르는 값 한 개를 받아 바꾸는 것이 아니라, `I` 를 내놓는 생산자 전체를 `O` 를 내놓는 생산자 전체로 바꿉니다. 입력도 생산자, 출력도 생산자라서, 파이프를 끼운 결과가 다시 생산자입니다. 그래서 파이프를 자유롭게 합성할 수 있습니다.
