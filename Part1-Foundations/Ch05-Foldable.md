...
5장의 핵심은 한 줄로 압축됩니다. 3장 / 4장이 *Normal → Elevated 로의 lift* (1인자 / N 인자) 였다면, *5장은 반대 방향입니다*. *Elevated World 의 구조를 Normal World 의 한 값으로 돌아오게 (끌어내림 / lower) 하는 함수가 `fold` 입니다*. `E<a> → b` 의 시그니처가 그 끌어내림을 정확히 표현합니다. 3·4장의 *lift* 와 5장의 *lower* 가 두 평행 세계 사이의 *두 방향 사다리* 가 됩니다.

> *5장의 3부 구조* — 3장 / 4장과 같은 narrative arc 로 구성됩니다.
>
> - **§5.1 *목적*** — §1.6.1 의 *lower (반대 방향)* 자리. 컨테이너의 구조를 Normal 한 값으로 압축하는 패턴을 *fold* 한 번 정의로 해소.
> - **§5.2 *기능*** — `Foldable<F>` trait 의 시그니처와 `FoldRight` + `FoldLeft` 두 멤버. 시그니처가 *F 가 결과에서 사라짐* 을 컴파일 타임에 강제.
...
이 *fold* 가 *함수형의 본질 — 합성 가능한 Elevated World 로 lift* (§1.6.1) 의 *반대 방향* 입니다. 3장 *map* 이 *lift* (끌어올림, Normal → Elevated) 의 가장 단순한 형태였다면, 5장 *fold* 는 *lower* (끌어내림, Elevated → Normal) 의 일반화입니다. 두 추상이 두 평행 세계 사이의 *두 방향 사다리* 가 됩니다.
...
**그림 5-2. `map` 끌어올림 vs `fold` 끌어내림** — 두 추상이 격자 위에서 *서로 반대 방향* 의 화살표. `map` 은 *수직 끌어올림* (Normal 의 함수 `a → b` 를 Elevated 의 함수 `E<a> → E<b>` 로). `fold` 는 *대각선 끌어내림* (Elevated 의 컨테이너 `E<a>` 를 Normal 의 한 값 `b` 로). *방향과 시그니처* 만 다른 두 추상이 *같은 격자* 위에 자리잡습니다. `fold` 의 step 함수와 seed 의 세부는 §5.1.4 의 mental model 과 §5.2 의 trait 시그니처에서 다룹니다 (그림 5-1 참고). 7장의 Traversable 은 두 방향을 동시에 다루는 추상이 됩니다.
...
세 결정적 차이가 보입니다. 첫째, *함수 `f` 의 인자 수가 다름* (1-인자 vs 2-인자 step). 둘째, *방향이 정반대* (수직 끌어올림 vs 대각선 끌어내림). 셋째, *출력에 F 가 살아 있는지 사라지는지* (모양 보존 vs 모양 소비). 그러나 *Normal 의 `f` 라는 기저 함수가 출발점* 이라는 본질은 같습니다. 두 추상이 *같은 가족의 다른 작용* 입니다.
...
`pure` 와 `fold` 가 두 세계 사이의 두 방향 사다리 입니다. `pure` 는 Normal 의 값을 Elevated 로 들어올리고, `fold` 는 Elevated 의 구조를 Normal 로 끌어내립니다. 두 추상이 합쳐져야 Elevated World 사이의 자유로운 이동이 완성됩니다.

이 장은 `E<a> → b` 자리의 Lowering 추상만 다룹니다. `a → E<b>` 유형 (World-crossing) 의 처리는 6장 (Monad) 의 자리입니다.

### 5.1.4 fold 의 mental model — 함수형의 for-loop

`fold` 가 처음 만나면 추상적이지만, *명령형 for-loop 의 함수형 대응* 으로 보면 즉시 직관이 잡힙니다. 두 코드가 완전히 같은 일을 합니다.
...
> *David Raab 의 통찰* — "`fold` 가 immutable 자료 구조에 하는 일은 `for-loop` 가 mutable 자료 구조에 하는 일과 같습니다." ([Understanding fold](https://davidraab.github.io/posts/understanding-fold/)). fold 는 for-loop 의 상태 변경을 함수 인자로 옮긴 것 일 뿐. 추상화의 본질은 *순회 + 누적* 이라는 패턴을 *step 함수와 seed* 두 부품으로 분리한 데 있습니다.

이 mental model 위에서 §5.2 의 trait 시그니처를 보면 *왜 step 함수와 seed 두 부품으로 구성되었는지* 가 자명해집니다. for-loop 의 두 핵심 (반복 본체 + 시작 상태) 을 함수로 명시화 한 것입니다.
...
**그림 5-4. FoldRight: 오른쪽 끝에서 시작해 왼쪽으로 접어 나갑니다** — 잎 `[1, 2, 3]` 은 정순 배치 그대로 (그림 5-3 과 동일). 결정적 차이는 **chain 방향이 우→좌**. seed `0` 이 오른쪽 끝에서 시작해 단계 1 (잎 3 처리) → 단계 2 (잎 2) → 단계 3 (잎 1, 최종) 으로 *왼쪽* 으로 흘러갑니다. 모든 잎→단계 화살표는 수직 (잎이 자기 바로 아래 단계로). **chain 의 우→좌 방향 자체가 *Right* 의 시각 메타포** 입니다. 오른쪽 끝에서 출발하기 때문에 FoldRight. 결합 순서 `1 + (2 + (3 + 0))` 이 그림의 공간 흐름과 정확히 일치. 도착점은 같은 `6` 이지만 *접히는 공간 방향이 그림 5-3 과 정반대*.

> *핵심* — 두 그림이 같은 자료 + 같은 step + 같은 seed 에서 같은 결과 6 에 도착하지만 *내부 계산 순서가 다릅니다*. 가환·결합 step (덧셈, 곱셈, AND, OR) 이면 결과가 같고, 비가환 step (문자열 연결, 뺄셈) 이면 다릅니다. §5.6 의 결과 일관성 법칙이 *언제 같은가* 의 조건을 형식화합니다.
...
두 어법 모두 trait 의 정적 `Foldable<F>.FoldRight` / `Foldable<F>.FoldLeft` 호출을 감싼 generic 한 줄 입니다. 자료 타입별 코드는 없습니다. `where F : Foldable<F>` 제약 한 줄이 어떤 F 든 받아 그 정적 메서드로 디스패치합니다.
...
같은 Foldable 메서드가 완전히 다른 자료 구조 (시퀀스 vs 옵셔널) 에서 각자 의미 있는 동작을 합니다. 세 어법은 호출 형태의 차이일 뿐, Foldable 의 동작은 자료 구조의 분기가 결정 합니다.
...
같은 `FoldRight` / `FoldLeft` 시그니처 지만 호출 횟수와 의미는 자료 구조에 달려 있습니다. Foldable 자체는 누적의 추상 이고, 몇 번 / 어디 누적할지는 자료 구조의 분기입니다.

### 5.4.5 문자열 파싱 결과의 합산 예제

3장의 ParseInt + Map(f) (§3.4.5) 와 *같은 자료 (MyMaybe) 위의 평행 예제* 입니다. 3장이 *Elevated 안에서의 1인자 lift* (`E<a> → E<b>`) 였다면, 5장은 *Elevated 를 Normal 로 회수하는 lower* (`E<a> → b`) 자리 — *효과 인코딩 + 일반 누적 합성* 의 정통 lower 예제. *Fold* 의 결정적 가치는 *반복 처리* 라는 문법적 형태가 아니라 ***세 책임의 깔끔한 분리*** 에 있습니다 — *효과는 타입* / *누적은 함수* / *합성은 Fold*.
...
| **합성은 Fold 한 줄** | `FoldLeft(step, 0, …)` 가 *효과 (있음/없음) 두 분기 처리* 와 *일반 누적 적용* 을 한 줄로 합칩니다. *명령형의 `if (parsed != null) total += parsed.Value` 분기 코드* 가 사라짐. |

명령형 코드라면 *효과 검사 (`null` 비교, `TryParse` 분기) 와 일반 누적* 이 *함수 본문 안에 섞여서* 작성됩니다. 함수형은 ***효과는 타입, 누적은 함수, 합성은 Fold*** — 세 책임이 깔끔히 나뉩니다. 독자가 작성하는 건 *늘 효과를 모르는 일반 누적 함수* 뿐, *분기 어법은 trait 의 `FoldLeft` 가 자동으로* 입혀 줍니다. §1.7.6 의 결정적 통찰이 *lower 자리* 로 확장되는 자리입니다.

![FoldLeft — Elevated → Normal 끌어내림](./images/Ch05-Foldable/05-parse-fold-lower.svg)

**그림 5-6. FoldLeft — Elevated → Normal 끌어내림 (lower)** — 3장 §3.4.5 의 *Map* 화살표가 *Normal → Elevated 위 방향 lift* 였다면, 여기 *FoldLeft* 화살표는 *Elevated → Normal 아래 방향 lower* 입니다. 위 행 Elevated World 에 입력 `MyMaybe<int>` 두 분기 (있음 / 없음). 아래 행 Normal World 에 결과 `int` 두 분기 (`Just(42) → step(42, 0) → 42`, `Nothing → seed 그대로 → 0`). 좌·우 *주황 lower 실선 화살표* 가 양 끝 박스의 끌어내림 자리를 표시합니다. 가운데 *점선 lower 화살표* 가 *fold 함수 자체* 로, Elevated 의 두 분기를 한 Normal 값으로 회수합니다. ***fold 의 가치는 반복 처리가 아니라 효과 두 분기를 Normal 의 한 값으로 회수하는 lower 자리에 있습니다***. 분기 (Just / Nothing) 처리는 trait 의 `FoldLeft` 가 자동으로 처리하고, `step` 은 효과를 모르는 평범한 2인자 누적 함수로 남습니다.

같은 `MyMaybe<int>` 위에서 *Functor 의 Map* 과 *Foldable 의 FoldLeft* 가 평행으로 작동하지만, *방향이 반대* — Map 은 Elevated 안에서 변환만 (`MyMaybe<int> → MyMaybe<int>`), FoldLeft 는 Elevated 를 Normal 로 회수 (`MyMaybe<int> → int`). `MyMaybeF.FoldLeft(step, seed, …)` 한 줄이 *lower (§1.6.1)* 의 가장 단순한 실현입니다. Elevated World 의 값을 *일반 누적 함수* 에 통과시켜 같은 Normal World 의 한 값으로 회수해 줍니다. 효과 인코딩의 분기 처리는 시그니처가 책임지고, 누적 로직은 *효과를 모르는 평범한 함수* 로 남습니다.
...
`FoldRight` 와 `FoldLeft` 는 같은 자료의 두 방향 순회 입니다. 이 둘이 서로 무관하게 임의 구현되면 `Count`, `Sum`, `All` 같은 자유 함수가 어느 abstract 로 정의되는지에 따라 결과가 달라집니다 — 자유 함수의 일관성이 무너집니다.
...
step 의 값 의미 (덧셈) 는 순수 인데, 카운터가 부수 효과 입니다. `Sum` 자유 함수를 호출자가 직접 FoldLeft 로 또 계산 하면 counter 가 두 배가 됩니다. 어디서 누가 fold 했는지가 결과에 영향을 미칩니다 — 추론 불가.
...
>
> Foldable 의 약속은 컨테이너를 소비 합니다. 결과 타입에 F 가 없는 것이 그 형식화 (§5.2.1). `Sum`, `Count` 의 결과는 Normal 의 한 값 이고, 그 결과로 원래 컨테이너를 다시 만들 수 없습니다. Foldable 은 단방향 끌어내림입니다. 컨테이너 위에서 다시 변환이 필요하면 Functor (3장), 양방향 이동이 필요하면 Traversable (7장) 으로 갑니다.
...
>
> 다음 자료 타입에 Foldable 인스턴스를 직접 부착합니다. 본문이 다룬 List / Maybe 와 달리 재귀 자료 구조 입니다.
>
> ```csharp
...
> - 재귀 정합성 — `Branch(L, R)` 의 fold 결과가 왼쪽 자식의 fold 와 오른쪽 자식의 fold 의 조합 입니까?
>
...
>
> 다음 다섯 메서드 시그니처를 보고 각자가 Foldable 의 `Fold` 자리 인지 답합니다. 아니라면 어떤 trait 의 자리 입니까?
>
...
>
> 1. 시그니처가 `E<a> → b` 형태 입니까? (입력에 E, 출력에 E 없음)
...
두 필수 챌린지가 노리는 것은 책의 마지막 페이지를 덮었을 때 독자가 손에 쥐어야 할 두 능력 입니다.
...
- `Foldable<F>` trait 는 `E<a> → b` 유형 함수의 일반화 입니다 (1장 4분면의 `E<a> → b`). *함수형의 본질 — 합성 가능한 Elevated World 로 lift* (§1.6.1) 의 *반대 방향 (lower / 끌어내림)* 이 등장하는 자리.
- `fold` 는 elevated-world 글의 Lowering 정통입니다. Elevated World 의 구조를 Normal World 의 한 값으로 끌어내립니다. 3장 `map` (lift / 끌어올림) 의 짝입니다.
- 두 법칙 (결과 일관성·부수 효과 없음) 이 두 abstract 의 정합성 약속을 구현 사이의 공통성으로 못 박습니다. §1.6.1 의 *셋째 축 합성 가능성* 의 lowering 판본입니다.
- 7장의 Traversable 은 Functor + Foldable 의 합으로, `T<E<a>> ↔ E<T<a>>` 양방향 변환을 다룹니다. 두 trait 의 능력이 한 추상으로 묶입니다.
- 4장의 pure (lift) 와 이 장의 fold (lower) 가 두 세계 사이의 두 방향 사다리입니다. 두 추상이 합쳐져야 Elevated World 사이의 자유로운 이동이 완성됩니다.

이 다섯 줄이 5장이 1부 안에서 차지하는 자리의 요약입니다. *lift 와 lower 가 두 평행 세계의 두 방향 사다리* 라는 §1.6.1 의 그림이 코드에 박히는 자리입니다.
...
5장까지 *두 평행 세계 사이의 두 방향 사다리* (lift / lower) 가 완성되었습니다. 6장 Monad 는 그 사다리 위에서 또 다른 자리를 답합니다. *출력 타입만 Elevated 인 함수 (`a → E<b>`)* 끼리는 직접 합성이 안 됩니다 (1장 §1.7.4 의 *어법 어긋남* 자리). `bind` 가 이 *World-crossing 함수* 의 합성을 회복합니다. [6장 — Monad](./Ch06-Monad.md) 로 넘어갑니다.

> *§5 → §6 의 다리* — 5장의 *fold* 가 *Elevated → Normal* 의 lower 였다면, 6장의 *bind* 는 `a → E<b>` 같은 *어법이 어긋난 함수* 의 합성을 Elevated 안에서 *회복* 하는 자리입니다. 1장 §1.7.4 의 합성 어긋남이 6장 *Monad* 의 `bind` 한 메서드로 *해소* 됩니다.