# 4장. Applicative / `pure` + `apply` (다인자 함수의 끌어올림)

> *이 장에서 다룰 주제* — 3장의 `map` 으로는 못 풀던 자리 (두 Elevated 값을 한 함수로 결합하는 자리) 를 푸는 trait. Pure 는 Normal 값을 가장 단순한 모양으로 끌어올리고, Apply 는 컨테이너 안의 함수와 컨테이너 안의 값을 만나게 합니다. 두 멤버 위에서 `Lift2`, `Lift3`, `Lift4` 가 자라며, 4장의 `E<a> → E<b>` 자리가 다인자로 넓어집니다.
...
> - [ ] applicative 누적과 monadic 단락이 왜 시그니처 단계 에서 갈리는지 답할 수 있습니다.

> *참고* — 이 장의 Elevated World 어휘는 Scott Wlaschin 의 "Map and Bind and Apply, Oh My!" 시리즈 (<https://fsharpforfunandprofit.com/posts/elevated-world/>) 의 어휘 분류를 따릅니다. 그 시리즈의 Part 2. Understanding apply 가 이 장의 직접 대응 글입니다.

---

## §4.1 `map` 만으로는 풀 수 없는 다인자 함수 — 목적

4장의 핵심은 한 줄로 압축됩니다. *Normal World 의 N 인자 함수를 Elevated World 에서 그대로 사용할 수 있도록 제공하는 lift 함수가 `pure` 와 `apply` 입니다*. 3장 *map* 이 *1인자 lift* 였다면, 4장 *pure + apply* 는 *N 인자 lift* 입니다. 두 멤버 위에서 `Lift2`, `Lift3`, `Lift4` 같은 다인자 헬퍼가 자랍니다.

> *4장의 3부 구조* — 3장과 같은 narrative arc 로 구성됩니다.
>
> - **§4.1 *목적*** — §1.6.1 의 *N 인자 lift* 자리. `map` 의 1인자 한계 + 다인자 함수를 Elevated 결합하는 패턴을 *pure + apply* 한 번 정의로 해소.
> - **§4.2 *기능*** — `Applicative<F>` trait 의 시그니처와 `Pure` + `Apply` 두 멤버. Functor 상속 + 시그니처가 *다인자 결합* 을 컴파일 타임에 강제.
> - **§4.3 ~ §4.5 *예제*** — `MyMaybe Applicative` / `MyValidation Applicative` + 어떤 Applicative 든 받는 일반 함수 (`Curry + Lift`).
> - **§4.6 ~ §4.7 *기능 (법칙·합성)*** — 다섯 법칙 (Identity / Homomorphism / Interchange / Composition / Functor 정합) + Functor 와의 상속 정합.
> - **§4.8 ~ §4.9 *예제 (경계·챌린지)*** — Applicative 가 *아닌* 변환들 + 직접 풀어 보는 챌린지.
> - **§4.10 ~ §4.12 *마무리*** — §1.6.1 framing 정합 + 5장 Foldable (반대 방향 lower) 다리.

### 4.1.1 4분면 격자의 `E<a> → E<b>` 가 다인자로 넓어지는 자리

1장 §1.7 에서 두 세계 사이의 함수 유형을 4분면 격자로 정리했습니다. 3장의 `map` 은 그중 `E<a> → E<b>` 의 *1인자* 끌어올림이고, `a → b` 한 개를 받습니다. 그런데 실세계의 함수는 *2인자 이상* 인 경우가 더 많습니다.
...
3장에서 본 `Map` 의 첫 자리는 *1인자 Normal 함수* 입니다.
...
그런데 `add : (int, int) → int` 는 *2인자 함수* 라 `Map` 의 첫 자리에 직접 못 들어갑니다. Curry 로 한 인자씩 받는 체인으로 비틀어야 들어갈 수 있습니다 (§4.5.1 의 Curry 어휘를 미리 빌립니다).
...
위 *필요한 시그니처* `K<F, a → b> → K<F, a> → K<F, b>` 가 정확히 ***`Apply` 의 시그니처*** (§4.2.3) 입니다. `Pure + Apply` 두 멤버가 함께 있어야 갇힌 함수를 풀어내고 두 Elevated 값을 한 Normal 함수로 합칠 수 있습니다.
...
| `E<a> → E<b>` | **3장 (Functor / `map`) + 4장 (Applicative / `apply`)** | Lifted function |

3장의 `map` 은 `E<a> → E<b>` 의 *1인자* 자리만 다뤘습니다. `a → b` 한 개를 받아 `E<a> → E<b>` 로 끌어올렸습니다. 4장의 Applicative 는 같은 `E<a> → E<b>` 를 다인자로 넓힙니다. `Lift2` 가 `(a, b) → c` 한 개를 `E<a> → E<b> → E<c>` 로, `Lift3` 가 3인자를 `E<a> → E<b> → E<c> → E<d>` 로 끌어올립니다.
...
![다인자 함수의 끌어올림](./images/Ch04-Applicative/01-multi-arg-lift.svg)

**그림 4-1. `Map` 의 한계와 `Apply` 의 필요성: 단계 ① → ② → ③** — §4.1.1 의 단계별 시그니처 추적과 일대일 대응합니다. **① 시작**: 두 Elevated 값 `K<F, a>`, `K<F, b>` 는 Elevated 에 있고 2인자 함수 `f : (a, b) → c` 는 Normal 에 따로 있어 결합 길이 없습니다. **② Map 후**: `addC = curry(f)` 로 비튼 뒤 첫 인자 `fa` 를 Map 으로 적용. 결과는 **`K<F, a → b>`** (두 번째 인자를 기다리는 갇힌 함수). `K<F, b>` 는 여전히 결합 대기 중. **③ Apply 후**: §4.2.3 에서 도입할 `Apply` 가 갇힌 함수를 풀어내 `K<F, b>` 와 결합 → `K<F, c>` 완성. **`Map` 한 개로는 갇힌 함수만 남길 뿐**, 두 번째 Elevated 값과의 결합은 새 도구 `Apply` 가 필요합니다.

---

## §4.2 `Applicative<F>` trait 시그니처 — 기능 (Pure + Apply)
...
`Pure` 는 Normal 값을 Elevated World 의 가장 단순한 시민 으로 끌어올립니다. 어떤 변환도 없이 컨테이너 한 겹을 둘러 줍니다.
...
값의 모양은 자료 구조마다 다르지만, Normal 값을 받아 가장 단순한 Elevated 모양으로 둘러싼다 는 약속은 모두 같습니다.

![pure 사다리](./images/Ch04-Applicative/02-pure-ladder.svg)
...
`E<a → b>` 가 어디서 오는지 봅니다. 대부분 `Pure` 와 결합해 만들거나, `Map` 이 다인자 함수를 부분 적용한 결과로 자동 생성됩니다. §4.5 의 `Lift2` 가 그 결합 패턴입니다.

![apply 결합](./images/Ch04-Applicative/03-apply-combine.svg)

**그림 4-3. `Apply`: 컨테이너 안 함수를 lifted function 으로 풀어냄** — Wlaschin 의 `apply` 다이어그램을 따라간 unpacking 시각. 단일 Elevated World 띠 안에서 좌측 `K<F, a → b>` (컨테이너 안에 갇힌 함수) 한 박스 → 파란 `apply` 화살표 → 우측 `K<F, a> → K<F, b>` 의 *Elevated lifted function*. **시그니처는 curried 형. `apply : K<F, a → b> → ( K<F, a> → K<F, b> )`**. 함수 한 개를 받아 Elevated 의 함수 한 개 를 돌려주는 구조가 `Apply` 의 본질입니다.

#### `Map` 과 `Apply` — *꺼내는 컨테이너 개수* 의 차이

시그니처 차이가 *동작의 차이* 로 직결됩니다. 두 도구가 *몇 개의 컨테이너를 꺼내는가* 로 가르면 결정적 한 줄이 보입니다.
...
*`Apply` 가 두 컨테이너를 동시에 다룬다* 는 점이 `Map` 과의 결정적 차이입니다. `MyMaybe.Apply` 의 네 분기 (§4.3.3) 가 정확히 *두 컨테이너 × 두 상태 (`Just` / `Nothing`) = 4가지 조합* 의 직접 표현입니다.
...
이 차이가 §4.4 의 `MyValidation.Apply` 에서 결정적입니다. *두 컨테이너를 양쪽 다 꺼내려 시도* 하므로 *두 Invalid 의 에러를 모두 모을 수 있습니다*. `Map` 이 한 컨테이너만 다루는 자리에서는 *효과 누적 자체가 시그니처적으로 불가능* 합니다. 6장의 `Bind` 가 *누적이 아니라 단락* 인 이유도 같은 자리에서 갈립니다. `Bind` 도 한 컨테이너만 꺼냅니다 (§6.7.3).
...
이 등식이 *Applicative 가 Functor 를 자연스럽게 함의* 한다는 사실의 한 줄 증명입니다. `Pure + Apply` 만 정의하면 `Map` 은 자동으로 따라옵니다. **`Applicative<F> : Functor<F>` 의 상속은 이 함의의 직접 반영** 입니다.

![Map ≡ Apply ∘ Pure 의 두 경로](./images/Ch04-Applicative/04-map-equals-apply-pure.svg)

**그림 4-4. `Map ≡ Apply ∘ Pure`: 같은 결과의 두 경로** — Wlaschin 의 *apply vs map* 다이어그램 (<https://fsharpforfunandprofit.com/posts/elevated-world/vgfp_apply_vs_map.png>) 을 따라간 *등식* 시각. **좌측 (Apply 경로)**: Normal 의 `a → b` 함수가 `Pure` 로 Elevated 한 칸 올라간 뒤 (`K<F, a → b>`), `apply` 가 `K<F, a>` 와 결합해 `K<F, b>` 를 냅니다. **우측 (Map 경로)**: 같은 `a → b` 함수가 `Map` 한 번으로 *직접* `K<F, a>` 를 `K<F, b>` 로 변환합니다. 가운데 큰 `=` 등호가 두 경로의 결과가 동일함을 보이는데, **`Map(f, fa) ≡ Apply(Pure(f), fa)`** 입니다. *Pure 가 Normal 함수를 Apply 의 첫 자리에 맞는 모양으로 끌어올려 주는 사다리* 이고, 이게 Applicative 가 Functor 의 능력을 자연스럽게 함의하는 시각적 증거입니다. 이 등식이 §4.6.6 의 Functor 정합 법칙의 형식화이고, §4.5.5 의 `lift1 = map` 결정의 근거입니다.
...
`Apply` 의 두 입력은 F 자리가 일치해야 하며, 함수도 같은 Elevated 세계에 살아야 합니다. 다른 세계의 컨테이너끼리는 `Apply` 로 합칠 수 없습니다. 그 자리는 7장 Traversable 의 층 swap 이 다룹니다.
...
└── Tests/ApplicativeLaws.cs              ← 다섯 법칙 검증 헬퍼 (§4.6)
```
...
## §4.3 첫 인스턴스 — MyMaybe Applicative (예제)

### 4.3.1 3-tuple 패턴으로 부착

추상 trait 만으로는 동작하지 않습니다. 3장의 `MyMaybe` 가 Functor 였는데, 같은 자료 타입에 Applicative 까지 부착합니다. 3-tuple 패턴 (자료 / 태그 / trait 구현) 의 두 번째 trait 적용 예시입니다.

```csharp
// ① 자료 타입 — Just / Nothing 의 두 variant. 3장에서 이미 부착.
...
| 조각 | 책임 |
|---|---|
| ① `MyMaybe<A>` 자료 타입 | Just / Nothing 의 두 variant — 3장과 동일 |
...
마지막 줄이 결정적입니다. `MyMaybe` 의 Apply 는 둘 다 Nothing 이라도 한 Nothing 만 돌려줍니다. *오류 정보가 한 자리만 있어* 모을 곳이 없기 때문입니다. §4.4 의 `MyValidation` 이 이 자리를 누적으로 바꿉니다.

### 4.3.4 호출 모양 — 세 가지 어법

§4.1.1 의 `add : (int, int) → int` 를 curried 형태 로 비틀어 두면, 첫 인자를 `Map` 으로 적용한 결과가 정확히 *두 번째 인자를 기다리는 wrapped 함수*. 그 wrapped 함수에 `Apply` 한 번이면 결합이 끝납니다. **§4.1.1 의 약속이 비로소 풀리는 자리** 입니다.

```csharp
// 시작 — §4.1.1 의 2인자 add 를 curried 형태로 (§4.5.1 의 Curry 어휘 미리 사용)
...
`Map(첫 인자 소비) → Apply(두 번째 인자 소비)` 의 2단계 사슬로 `add(5, 3) = 8` 의 Elevated 결과를 얻습니다. **§4.1.1 의 "두 Elevated 값을 한 Normal 함수로 결합" 약속이 비로소 풀린 자리**. 이 패턴이 §4.5.2 의 `Lift2` 로 한 줄에 압축됩니다.
...
> 두 형태의 관계:
>
> | 학습용 펼친 형태 (§4.3.4) | 실전 압축형 (§4.5.2 의 `Lift2`) |
> |---|---|
> | `Map(addC, five).Apply(three)` | `Lift2((x, y) => x + y, five, three)` |
> | 사용자가 직접 curried 함수 를 만들어 넘김 | 2인자 함수 그대로 넘김 — Curry 가 내부 자동 |
> | 2단계 (Map + Apply) 가 코드에 노출 | 1단계 (Lift2) — Curry → Pure → Apply 사슬을 캡슐화 |
> | 각 단계의 시그니처 변화 가 보임 | 결과 만 보임 (단계는 헬퍼 안) |
>
> 두 코드 모두 시그니처와 결과 (`Just(8)`) 가 동일. **학습 단계에서는 펼친 형태로 *무엇이 일어나는지* 보고, 실무 코드에서는 압축형을 씁니다**. §4.5.2 에서 `Lift2` 의 4단계 분해 (`Curry → Pure → Apply → Apply`) 가 바로 이 압축의 내부.

> *참고 — `a, b` 는 무엇이고 `n` (또는 `y`) 은 무엇인가요?*
>
> 독자가 자주 혼동하는 두 자리를 정리합니다.
>
> **첫째, 타입 변수 vs 매개변수 이름** — 시그니처의 `a, b, c` 는 **타입 변수** (`int`, `string` 같은 타입 을 가리킴). 람다 안의 `x`, `y` 는 **매개변수 이름** (호출 시 값 이 들어갈 자리). 둘은 다른 카테고리입니다.
>
...
> ```
>
> **§4.1.1 의 *2인자* `(a, b) → c` 는 어디로 갔나요?** — `addC` 가 그 `add` 의 curried 형태. `Map(addC, five)` 가 첫 인자 `5` 를 소비해 *2인자가 1인자로 줄어든* wrapped 함수 (`mf`) 를 만듭니다. 이 자리에서 시그니처가 `(a, b) → c` 에서 `a → b` 로 바뀐 셈. `Apply(mf, three)` 가 마지막 1인자를 소비해 결합 완성. **§4.5.2 의 `Lift2` 는 이 2단계 (`Map → Apply`) 를 `Pure → Apply → Apply` 의 3단계로 일반화** 합니다. 어떤 N인자 함수든 같은 패턴.
...
함수 측이 Nothing 이면 값을 보지도 않고 결과 Nothing. 어느 쪽이든 Nothing 이면 결과 Nothing 의 단락 동작입니다. 사용자에게 왜 실패했는지 의 정보가 없다는 점이 7장 Validation 에서 누적으로 바뀝니다.

---

## §4.4 두 번째 인스턴스 — MyValidation Applicative (예제)

### 4.4.1 같은 trait 시그니처, 다른 결합 동작

같은 trait 을 완전히 다른 결합 의미를 가진 자료 구조에 부착합니다. `MyValidation<E, A>` 는 *성공 + 실패* 두 케이스인데, 실패 케이스가 에러 리스트 를 들고 있고 `Apply` 가 *양쪽 에러를 누적* 합니다.
...
네 입력이 모두 실패할 때 *네 에러를 한 번에* 사용자에게 보고할 수 있습니다. 회원가입 폼의 표준 패턴이고, 7장 Validation 실전에서 본격적으로 다룹니다.
...
같은 `Apply` 시그니처지만 결합 규칙이 자료 구조에 달려 있습니다. `Apply` 자체는 함수와 값의 결합 의 추상이고, 실패를 어떻게 모을지 는 자료 구조의 분기가 결정합니다.

### 4.4.5 회원가입 폼 검증 예제 — 세 책임 분리의 N 인자 확장

두 필드 (`CustomerId`, `EmailAddress`) 의 검증 결과로 `CustomerInfo` 를 만드는 자리는 *효과 인코딩 + 일반 연산 합성* 의 정통 *N 인자 확장* 예제입니다. *Lift2 / Lift3 / Lift4* 의 결정적 가치는 *N 회 Apply* 라는 문법적 형태가 아니라 ***세 책임의 깔끔한 분리*** 에 있습니다 — *효과는 타입* / *연산은 함수* / *합성은 Lift*. 3장 *Map* 이 *하나의 효과* 위에서 1인자 일반 함수를 끌어올렸다면, 여기서는 *두 (또는 N 개) 효과* 위에서 *N 인자 일반 함수* 를 끌어올립니다.
...
- **합성 — `Lift2(createCustomer, …, …)`** — *Lift2* 한 줄이 *2인자 일반 함수* 를 *효과 두 분기 위로 승격* 시킵니다. *둘 다 성공 (`Valid`) 이면 결합 결과 생성*, *한 측이라도 실패 (`Invalid`) 면 에러 누적* — 분기 코드 `if (id.IsValid && email.IsValid) ...` 자체가 사라집니다. *합성은 Lift 의 책임*.
...
| **일반 연산은 N 인자 평범한 함수** | `createCustomer = (id, email) => new CustomerInfo(id, email)` 는 *효과를 모르는* 단순한 2인자 생성자. *Lift3 / Lift4 도 같은 발상의 확장* — `(n, e, a, t) => new User(...)` 같은 N 인자 생성자에 `LiftN` 한 줄로 끌어올림 적용 (§4.4.3 의 4인자 회원가입 예제). *생성자 코드는 검증 어법 없이 한 번만 작성*. |
| **합성은 Lift 한 줄** | `Lift2(f, x, y)` 가 *효과 (통과/실패) 두 분기 처리 + 두 측 에러 누적* 과 *2인자 일반 연산 적용* 을 한 줄로 합칩니다. *명령형의 `if (x.IsValid && y.IsValid) …` / 두 에러 합치는 boilerplate* 가 사라짐. |

명령형 코드라면 *두 검증 결과의 조합 + 에러 누적 + 생성자 호출* 이 *함수 본문 안에 섞여서* 작성됩니다. 함수형은 ***효과는 타입, 연산은 N 인자 함수, 합성은 Lift*** — 세 책임이 깔끔히 나뉩니다. 독자가 작성하는 건 *늘 효과를 모르는 N 인자 일반 함수* 뿐, *N 개 효과의 결합 + 에러 누적 어법은 trait 의 `Apply` 가 자동으로* 입혀 줍니다. §1.7.6 의 결정적 통찰이 *N 인자 자리* 로 확장되는 자리입니다.

![Lift2 — 2인자 함수의 curried 사슬 끌어올림](./images/Ch04-Applicative/05-customer-validation.svg)

**그림 4-5. Lift2 — 2인자 함수의 curried 사슬 끌어올림** — 3장의 1인자 lift (`Map : (a → b) → (E<a> → E<b>)`) 가 N 인자로 확장된 형태입니다. 아래 행 Normal World 에 `createCustomer : CustomerId → Email → CustomerInfo` 의 curried 사슬 (세 박스 + 두 가로 화살표). 위 행 Elevated World 에 끌어올린 curried 사슬 `MyValidation<…, CustomerId> → MyValidation<…, Email> → MyValidation<…, CustomerInfo>` (세 박스 + 두 가로 화살표). 좌·우 *파란 lift 실선 화살표* 가 양 끝 박스 (`CustomerId` 와 `CustomerInfo`) 의 lift 자리를 표시합니다. 가운데 *점선 lift 화살표* 가 *Lift2 함수 자체* 로, 2인자 Normal 함수 `createCustomer` 를 받아 두 MyValidation 결합 함수로 승격시킵니다. ***Lift2 의 가치는 인자 수가 늘어도 lift 패턴이 한 줄로 유지된다는 점에 있습니다***. 효과 결합 + 에러 누적은 trait 의 `Apply` 가 자동으로 처리합니다.

3장의 ParseInt + Map(f) (§3.4.5) 가 *하나의 효과 위 1인자 lift* 였다면, 여기 ValidateId × ValidateEmail + Lift2(createCustomer) 는 *두 효과 위 2인자 lift* 입니다. *효과 개수 + 일반 함수 인자 수* 가 같이 늘어나도 *세 책임의 분리* 는 그대로 — `LiftN` 한 줄이 N 개의 분기 + 에러 누적을 자동으로 입혀 줍니다.

---

## §4.5 어떤 Applicative 든 받는 일반 함수 — 예제 (Curry + Lift)

§4.5 가 *함수형의 본질 — 합성 가능한 Elevated World 로 lift* (§1.6.1) 의 *lift 의 다인자 확장* 을 코드로 보여 주는 자리입니다. 3장 *map* 이 *1인자 lift* `(a → b) → (E<a> → E<b>)` 였다면, *Lift2 / Lift3 / Lift4* 는 같은 발상의 *2 / 3 / 4인자 확장* 입니다. 두 도구 `Curry` 와 `Apply` 가 다인자 함수를 *한 인자씩 받는 사슬* 로 비틀어 *N 회 Apply* 로 끌어올립니다.
...
> *참고 (부분 적용 (partial application))* — Currying 의 자연스러운 부산물 입니다. `Curry.Of(add)(10)` 만 호출하면 *10 을 더하는 새 함수* `Func<int, int>` 를 얻습니다. C# 의 LINQ `xs.Where(n => n > threshold)` 의 `threshold` 가 외부 변수로 부분 적용된 모양. 함수형 프로그래밍의 일상 도구입니다.
...
![lift2단계 분해](./images/Ch04-Applicative/06-lift2-pipeline.svg)

**그림 4-6. `Lift2`: Normal 의 curried 함수 사슬을 Elevated 로 통째 끌어올림** — Wlaschin 의 `lift2` 다이어그램을 따라간 *상태 전환* 시각. 좌측 (Lift2 전) 에서 Normal World 의 `a → b → c` 사슬이 활성, Elevated 의 `K<F, a> → K<F, b> → K<F, c>` 자리는 흐릿. 가운데 파란 `Lift2` 화살표 → 우측 (Lift2 후) 에서 Elevated 의 lifted curried function chain 이 활성, Normal 은 흐릿. **`Lift2` 는 curried 함수 사슬 전체의 World 이동** 으로, 인자가 한 개씩 들어올 때마다 사슬이 한 칸씩 줄어드는 부분 적용 가능성 이 Elevated 안에서 그대로 보존됩니다.

> *참고 — 실전 LanguageExt 의 구현 방향은 다릅니다*
>
> 위 4단계 분해는 학습 명료성을 위해 `Curry.Of` 를 컨테이너 밖에서 명시적 으로 호출합니다. 실전 LanguageExt v5 의 `Applicative.lift<F, A, B, C>` 는 같은 결과를 컨테이너 안 currying 으로 압축한 한 줄입니다.
>
> ```csharp
...
>
> 두 가지가 결정적으로 다릅니다.
> - **Currying 의 자리** — 이 책은 `Curry.Of(f)` 를 *Normal World 에서* 먼저 적용한 뒤 `F.Pure` 로 올립니다. 라이브러리는 *Multi-arg `Func<A, B, C>` 그대로* `F.Pure(f)` 로 올린 뒤, `.Apply(fa)` 의 multi-arg 확장 메서드 가 내부에서 `F.Map(curry, mf)` 로 *Elevated World 안에서* currying 을 수행합니다.
> - **사용자 어법** — 이 책은 `Lift2` 가 4단계 분해 를 그대로 보입니다. 라이브러리는 `lift(f, fa, fb)` 한 줄이 현장 사용 어법 으로 압축됩니다.
>
> ```csharp
...
> ```
>
> 결과·의미는 이 책의 4단계 분해와 동일 — Pure ∘ Curry → Apply → Apply 의 등식이 두 방향에서 모두 성립합니다. 이 책의 명시적 4단계는 어떤 일이 일어나는지 의 학습용 펼침, 라이브러리 한 줄은 현장 사용 어법 으로의 압축. 두 접근이 시그니처 단계에서 같은 함수임을 4장 코드의 `Lift.Lift2` 와 LanguageExt 의 `Applicative.lift` 를 나란히 호출해 검증할 수 있습니다.
...
![lift3 사슬](./images/Ch04-Applicative/07-lift3-chain.svg)
...
지금까지의 `Lift2 / Lift3` 는 *즉시 적용* 형태입니다 — 결과가 `K<F, C>` 한 값. 함수 자체 를 결과로 돌려주는 *curried 형태* 도 있습니다. 두 형태가 같은 작용을 가리키지만 *결과의 모양* 이 다릅니다.
...
`Lift2Curried` 의 시그니처가 Normal 함수 `(a, b) → c` 를 Elevated 함수 `K<F, a> → K<F, b> → K<F, c>` 로 끌어올린다 는 의도를 그대로 담습니다. **elevation 의 본질은 함수 자체가 Elevated 세계에 사는 것** — *lift* 한 동사가 가리키는 것이 정확히 이 자리입니다.
...
`Lift1` 의 제약이 `Functor<F>` *만* 입니다 — Applicative 의 멤버 (`Pure`, `Apply`) 가 쓰이지 않습니다. 즉 **`Lift1` 은 사실 `Map` 의 curried 형태** 입니다.
...
`Lift1 = Map` 의 시각적 근거는 §4.2.4 의 *그림 4-4* — `Map ≡ Apply ∘ Pure` 의 두 경로 등식이 그대로 *`Lift1` 이 진짜 Applicative 의 능력을 쓰지 않는다* 는 사실의 시각화입니다. 그림의 우측 (Map 경로) 이 정확히 `Lift1(f)(fa)` 의 동작, 좌측 (Apply 경로) 이 `Lift2 / Lift3` 에서 본격적으로 살아나는 *Pure + Apply* 사슬의 가장 짧은 한 단계 (1인자) 입니다.
...
+ Applicative        ─►  + Pure, Apply, Lift2, Lift3, Lift4, LiftN,
                            sequence (8장 부분), traverse (8장 부분)        (중간)
+ Monad (6장)        ─►  + Bind, Kleisli >=>, liftM, mapM, foldM, ...      (다수)
```

`Pure + Apply` 두 멤버의 정의 위에 `Lift1 ~ LiftN`, `Lift2Curried`, 그리고 8장 Traversable 에서 `sequence` / `traverse` 까지 자라납니다. 새 자료 타입이 Applicative trait 한 개 만족으로 *어떤 인자 개수든* 끌어올림이 공짜로 따라옵니다.

---

## §4.6 Applicative 의 다섯 법칙 — 기능 (목적의 보장)

### 4.6.1 시그니처만으로는 부족합니다

`Applicative<F>` 인터페이스를 구현했다고 진짜 Applicative 가 되는 것이 아닙니다. 다섯 법칙을 만족해야 합니다. **컴파일러는 강제하지 못합니다** — 독자가 직접 검증합니다.

> *elevated-world 글 인용* — "`apply` 와 `return` 의 올바른 구현은 Elevated World 마다 다르지만, 항상 만족해야 할 성질이 있습니다." Functor 의 두 법칙 (3장 §3.6) 의 자연스러운 확장이 Applicative 의 다섯 법칙입니다.
...
*Normal 함수 + Normal 값을 각각 끌어올린 뒤 결합* 한 것이 Normal 자리에서 먼저 결합한 뒤 끌어올림 과 같습니다. **`Pure` 가 Normal 의 함수 적용을 Elevated 로 그대로 옮긴다** 는 약속의 형식화 — `Pure` 가 Elevated World 의 *기준점* 임을 보장합니다.
...
Elevated 함수 + 끌어올린 Normal 값 의 결합이, 그 값을 미리 묶은 적용 함수 + Elevated 함수 의 결합과 같습니다. **`Apply` 의 좌우 비대칭이 사라지는 자리** — 함수 측과 값 측이 어디 있든 결과가 일치합니다.
...
여기서 `compose = g => f => x => g(f(x))` — 함수 합성을 끌어올린 형태입니다. 두 Elevated 함수의 합성 이 각각 적용한 합성 과 같습니다. Functor 의 합성 법칙 (`Map(g, Map(f, fa)) ≡ Map(g ∘ f, fa)`) 의 다인자 확장입니다.
...
`Map ≡ Apply ∘ Pure` 등식 (§4.2.4) 의 형식화입니다. Applicative 가 Functor 를 상속하므로 *두 trait 의 `Map` 이 같은 의미를 가져야* 합니다 — `Apply` 와 `Pure` 만 정의해도 `Map` 이 자동 일관성을 갖는다는 보장.
...
두 경로의 결과가 같습니다. 이 법칙이 깨지면 `Pure + Apply` 만으로 정의한 `Map` 의 동작이 자료 타입이 직접 구현한 `Map` 과 달라져, *§4.7.1 의 상속 사슬* 자체가 무너집니다.

> *참고 (LanguageExt v5 의 다섯 법칙 정렬)* — LanguageExt `ApplicativeLaw<F>.validate(...)` 가 검증하는 다섯 법칙 중 마지막이 바로 이 *functorLaw*. 이 책의 다섯 절 (`Identity / Homomorphism / Interchange / Composition / Functor 정합`) 이 라이브러리의 다섯 법칙과 시그니처·의미 단계에서 완전히 정렬됩니다.
...
값은 같지만 몇 번 들었는지 가 다릅니다 — 동형 사상 법칙 위반. 부작용. `Lift2` 의 결과 크기가 예상과 다릅니다.

> *흔한 함정* — Apply 가 외부 카운터를 건드리거나 임의 부풀림을 추가하면 법칙이 깨집니다. Applicative 의 `Apply` 는 순수 결합 이어야 합니다. 부수 효과가 필요하면 그 부수 효과를 캡슐화하는 Monad (6장 Bind / Kleisli) 의 자리.
...
시그니처가 같은 모양인데 결과 의미가 달라지는 자리 — Identity 는 `Pure(id)` 의 단순화를, Homomorphism 은 `Pure` 의 기준점을, Interchange / Composition 은 `Apply` 의 결합 형태 자유도를, Functor 정합은 상속한 `Map` 의 일관성을 각각 책임집니다. **다섯 법칙 모두 만족할 때만** `Apply` 가 **진짜 Applicative**. 시그니처를 통과한 모든 추론이 안전한 trait.

---

## §4.7 Functor 와 Applicative — 기능 (상속 정합)
...
Applicative   — Map + Pure + Apply          (N인자 끌어올림 + 사다리)
   ↑
Monad         — Map + Pure + Apply + Bind   (6장. World-crossing 합성)
```

`Applicative<F> : Functor<F>` 의 상속이 시그니처 단계 에서 의미하는 것 — `Map` 이 자동으로 따라옵니다. `Map ≡ Apply ∘ Pure` 의 등식 (§4.2.4) 이 그 한 줄 증명. 새 자료 타입에 `Pure + Apply` 를 정의하면 `Map` 의 동작이 자동으로 일관됩니다.
...
*Apply 가 Map 의 일반화* — 함수가 Elevated 한 칸 위로 올라간 모양. *Pure 가 Apply 의 좌측 입력 (`K<F, Func<A, B>>`) 을 만들 수 있게 해주는 보조 도구*. 세 시그니처가 *한 가족* 입니다.
...
`Pure` 가 *0인자 lift* — 끌어올릴 함수가 없는 자리. 값 자체 만 끌어올립니다.

---

## §4.8 Applicative 가 아닌 변환 — 예제 (경계)
...
| `Bind(fa, f)` | `E<a> → (a → E<b>) → E<b>` | ✗ | Monad (6장) — `a → E<b>` 가 World-crossing |
| `Fold(step, seed, fa)` | `(...) → E<a> → b` | ✗ | Foldable (5장) — `E<a> → b` 끌어내림 |
| `Filter(p, fa)` | `(a → bool) → E<a> → E<a>` | ✗ | Filterable — 원소 개수 감소 |
| `Sequence(fas)` | `T<E<a>> → E<T<a>>` | ✗ | Traversable (8장) — 두 Elevated 의 층 swap |
...
| Map | Functor | 3장 (Applicative 가 상속) |
| Bind / Kleisli `>=>` | Monad | 6장 |
| Fold / FoldRight / FoldLeft | Foldable | 5장 |
| Traverse / Sequence | Traversable | 8장 |
...
>
> `MyValidation` 이 만약 `Bind` 를 정의한다면, 첫 Invalid 에서 *단락* 하여 누적이 사라집니다. 사용자에게 왜 네 자리에서 어떻게 실패했는지 가 한 자리 에러로 줄어듭니다. 7장 Validation 실전의 핵심 결정. Validation 은 *의도적으로* Bind 를 정의하지 않고 Applicative 의 누적만 유지합니다. 누적이 필요한 자리에서 실수로 `Bind` 를 쓰는 것 자체가 *언어 수준 차단*.
...
3장에서 `MyList<A>` 에 Functor 를 부착했습니다. 이제 같은 자료 타입에 Applicative 를 부착합니다 — `Apply` 의 결합 의미는 *cartesian product*.
...
`Lift2`, `Lift3` 의 패턴을 따라 `Lift4` 를 직접 작성합니다. 정답은 §4.5.3 에 있지만, *먼저 손으로* 작성한 뒤 비교하는 것이 학습 목적.
...
`MyValidation` 과 달리 *첫 실패에서 단락* 하는 Applicative. `Apply` 의 분기에서 (Left fe, Left ae) 일 때 *첫 fe 만* 돌려줍니다 — 누적 안 함. 두 인스턴스의 같은 시그니처가 다른 의미 를 가짐을 확인합니다.

**C. `Pair<W, A>` (Writer 의 단순형) Applicative**

`Pair<W, A>` 가 부수 로그 W 와 값 A 의 쌍. `Pure` 가 `(W.Empty, value)`, `Apply` 가 `(w1 <> w2, f(a))` — 로그를 *monoid 로 누적* 합니다. 8장 Reader / 10장 Writer 의 출발점.

이 세 챌린지 중 하나 이상 을 직접 작성·검증합니다. 정답 코드는 `code/Part1-Foundations/Ch04-Applicative/Challenges/` 안에 있습니다.

---

## §4.10 Elevated World 어휘로 다시 읽기 — 마무리

이 장의 결정을 Wlaschin 시리즈 흐름과 정렬합니다.

| 시리즈 흐름 | 이 책의 자리 |
|---|---|
| Part 1 — Toolbox (`map`, `return`, `apply`, `bind`) | 3장 (Map), 4장 (Pure + Apply), 6장 (Bind) |
| Part 2 — Understanding apply | **이 장. Pure + Apply + Lift2/3/4** |
| Part 3 — Understanding bind | 6장 Monad |
| Part 5 — Lift, sequence, traverse | 8장 Traversable |

4장이 다루는 4분면 자리는 `E<a> → E<b>` 의 *다인자 확장* + ⓟ 사다리 (`a → E<a>`). 두 Elevated 시민의 결합이 본격적으로 가능해지는 자리 — Wlaschin 의 용어로는 *unpacking a wrapped function*, *combining lifted values* 입니다.

---

## §4.11 Q&A — 마무리 (자주 묻는 질문)

> *Q1. `map` 으로 두 Elevated 값의 덧셈은 정말 못 합니까?*

부분적으로는 가능합니다. `Map(add, two)` 의 결과 `K<F, Func<int, int>>` 에 *세 번째 함수* (`Apply`) 를 한 번 더 적용하면 됩니다. 그 한 번 더가 정확히 `Apply` 의 시그니처 — 컨테이너 안 함수와 컨테이너 안 값의 결합입니다. 즉 `Lift2(f, fa, fb) = Map(f, fa).Apply(fb)` 가 같은 등식입니다. `Map` 만으로는 끌어올림이 *반쪽* 입니다. `Apply` 가 그 반쪽을 닫는 도구입니다.

> *Q2. `Pure` 와 `Map(_, ...)` 가 비슷해 보이는데 시그니처가 어떻게 다릅니까?*

`Pure : a → E<a>` 와 `Map : (a → b) → E<a> → E<b>` 는 입력이 다릅니다 — `Pure` 는 Normal 값 만, `Map` 은 *변환 함수 + Elevated 값*. 0인자 끌어올림 과 1인자 끌어올림 의 차이로 보면 정렬됩니다. `Pure(a) = ???` 의 자리에서 `Map` 을 쓰려면 Elevated 값을 이미 만들어야 합니다. 닭과 달걀의 자리입니다. `Pure` 가 그 시작점을 만듭니다.

> *Q3. `Lift1` 이 `Map` 과 같다면 왜 별도 이름을 둡니까?*

두 어휘가 같은 자리를 다르게 부른다 는 사실 자체가 학습 자리입니다. `Map(f, fa)` 는 *컨테이너 안 값 변환* 시각, `Lift1(f)(fa)` 는 *Normal 함수의 Elevated 판* 시각. **인자 개수 N 으로 일반화할 때 자연스러운 어휘가 `LiftN`** — `Lift1 / Lift2 / Lift3` 의 가족이 한 줄로 정렬됩니다. 코드 측면에서는 `Lift1` 의 제약이 `Functor<F>` 만이고, `Lift2` 이상이 `Applicative<F>` 가 필요한 분리점 도 보입니다.
...
새 인스턴스가 `Applicative<F>` 를 만족하려면 `Pure`, `Apply`, *그리고 `Map`* 세 멤버를 모두 구현해야 합니다 (interface 상속의 직접 결과). 단, `Map ≡ Apply ∘ Pure` 의 등식 (§4.2.4) 이 Map 의 일관성 을 보장 — 두 정의가 의미적으로 같아야 법칙이 깨지지 않습니다. 즉 `Map` 을 직접 구현해도 되지만, 최적화 / 결합 의미 보존 을 위해 `Apply` 와 같은 분기를 따르도록 손으로 작성합니다 (3장 §3.4 의 MyMaybe 의 Map 이 그 예).

> *Q6. `Lift2` 의 `Curry → Pure → Apply × 2` 패턴이 왜 *Curry 먼저* 입니까?*

`Apply` 가 한 번에 한 인자 만 결합하기 때문입니다. 다인자 함수 `(a, b) → c` 의 Elevated 판은 `K<F, Func<A, Func<B, C>>>` — 즉 *함수 안에 또 함수가 든* 모양. `Curry.Of` 가 그 변환을 합니다. `Curry` 없이 `Pure((a, b) => a + b)` 를 그대로 끌어올리면 결과는 `K<F, Func<A, B, C>>` 인데, `Apply` 의 시그니처가 받지 않습니다. **Curry 가 Applicative 의 전제 조건**. 다인자가 한 인자씩 받는 체인으로 바뀌어야 `Apply` 의 사슬이 시작됩니다.

> *Q7. `Apply` 의 누적 (Validation) 과 단락 (Maybe) 이 시그니처 단계 에서 어떻게 갈립니까?*

시그니처는 똑같습니다 — `K<F, Func<A, B>> → K<F, A> → K<F, B>`. 갈리는 자리는 *Apply 의 분기 구현* 입니다. `MyMaybe` 의 분기는 `(Just, Just) → Just(f(a))`, *그 외* → `Nothing`. `MyValidation` 의 분기는 `(Valid, Valid) → Valid(f(a))`, `(Invalid, Invalid) → Invalid(누적)`, 한 측만 Invalid → 그 에러 보존. 같은 시그니처가 자료 구조의 분기 에 따라 다른 결합 의미를 가집니다. 6장 Monad 의 `Bind` 와는 시그니처 자체 가 다릅니다. `Bind` 의 두 번째 인자가 `a → E<b>` (World-crossing) 라 *왼쪽 결과가 오른쪽 호출을 결정* 하므로 첫 실패에서 단락이 *시그니처적으로 강제* 됩니다. 7장 Validation 실전의 핵심.

> *Q8. `Applicative<F>` 가 `IMonoid<E>` 같은 추가 제약을 받는 자리는 어디입니까?*

`MyValidation<E, A>` 의 누적 결합 `[..fe.Errors, ..ae.Errors]` 가 사실 *List monoid 의 Combine*. 일반화하면 `MyValidation<MonoidE, A>` — `E` 가 *임의 monoid* 라면 두 실패의 결합이 그 monoid 의 Combine 으로. 라이브러리 LanguageExt 의 `Validation<MonoidE, A>` 가 이 일반화의 직접 표현. 학습용 코드는 List 의 Concat 만 다루지만, §4.9.3.A 의 `ConstApplicativeF<M>` 챌린지가 그 일반화의 첫 단계.

> *Q9. `Applicative` 의 법칙이 Functor 의 두 법칙과 어떤 관계입니까?*

Identity / Composition 두 법칙이 Functor 의 두 법칙의 *다인자 확장* 입니다. Applicative 의 Identity 가 Functor 의 `Map(id) ≡ id` 의 다인자 판, Applicative 의 Composition 이 `Map(g ∘ f) ≡ Map(g) ∘ Map(f)` 의 다인자 판. Homomorphism / Interchange 두 법칙이 *Pure 와 Apply 가 새로 등장하면서 추가된* 약속. Pure 가 *기준점* 임 (Homomorphism), Apply 의 좌우가 *대칭적* 임 (Interchange). 마지막 다섯 번째 (Functor 정합) 는 상속한 `Map` 이 `Apply ∘ Pure` 로 정의한 Map 과 같은 의미임을 보장 (`Map ≡ Apply ∘ Pure`). 다섯 법칙이 함께 Applicative 의 결합 의미가 일관됨 을 보장합니다.

> *Q10. `LiftN` 의 한도는 어디까지입니까?*

이론적 한도는 없습니다. `Apply` 를 N 번 호출하면 N 인자 함수가 끌어올려집니다. 실무 한도는 *가독성* 입니다. `Lift5` 이상은 인자 5 개 이상의 생성자 / 함수입니다. 일반적으로 도메인 객체 생성자의 자리입니다. 그 자리에서는 4장의 `Lift5 / Lift6` 보다 8장 Traversable 의 `sequence / traverse` 가 더 자연스럽습니다 — 인자가 *같은 종류 N개* 일 때 `K<F, IEnumerable<A>>` 로 묶어 한 번에 처리합니다. 4장의 `Lift2-Lift4` 가 실무에서 가장 자주 쓰이는 범위입니다.

> *Q11. C# 의 LINQ `from-from-select` 가 Applicative 와 어떤 관계입니까?*

LINQ `from a in fa from b in fb select f(a, b)` 는 *Monad 의 SelectMany 의 syntactic sugar* — `Bind` 의 어법이지 `Apply` 가 아닙니다. *Validation* 처럼 *Apply 만 정의* 한 자료 타입은 LINQ `from-from-select` 로 합성할 수 *없습니다*. 명시적 `Lift2 / Lift3 / Lift4` 호출만 가능합니다. 이 차이가 언어 수준 에서 누적과 단락을 가르는 자리입니다. 누적이 필요하면 `Lift`, 단락이 필요하면 LINQ 입니다. 6장 Monad / 7장 Validation 실전에서 본격적으로 다룹니다.

---

## §4.12 다음 장으로 — 마무리 (5장 Foldable 다리)

4장에서 본 `Pure + Apply + LiftN` 이 *Normal → Elevated* 의 *N 인자 lift* 였다면, 5장 Foldable 은 *반대 방향* 입니다. *Elevated World 의 구조를 Normal World 의 한 값으로 끌어내리는 (lower) 함수가 `fold`* 입니다. 3장 / 4장이 *lift 가족* 이었다면 5장이 *lower* 의 자리입니다.

```
3장 — Functor:       (a → b) → (E<a> → E<b>)                map        (1인자 lift)
이 장 (4장) — Applicative:  (a, ..., z) → r → E<a> → ... → E<z> → E<r>   pure + apply (N 인자 lift)
다음 장 (5장) — Foldable:   E<a> → b                          fold        (반대 방향 lower)
6장 — Monad:        a → E<b> 의 합성 회복                     bind / Kleisli >=>
```

[5장 — Foldable](./Ch05-Foldable.md) 로 넘어갑니다.

> *§4 → §5 의 다리* — Applicative 가 *N 인자 Normal 함수* 를 *Elevated* 에서 쓸 수 있게 *끌어올렸다면*, Foldable 은 *Elevated 의 구조* 를 *Normal 의 한 값* 으로 *끌어내립니다*. 두 추상이 *두 평행 세계 사이 두 방향 사다리* 의 *위→아래* 자리입니다.
