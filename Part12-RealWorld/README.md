...
- 효과 기반 애플리케이션 — `Eff<RT>` + `Has<RT, Trait>` DI + `LanguageExt.Sys` (Console / File / Time) 로 부수 효과를 격리 한 앱.
...
- `Eff<RT>` + `Has<RT, Trait>` 로 의존성을 주입하고, `LanguageExt.Sys` 로 콘솔 / 파일 / 시간 부수 효과를 격리한 애플리케이션 작성.
...
| 의존성 주입 + 부수 효과 격리 | `Eff<RT>` / `Has` / `LanguageExt.Sys` | Part 4 ~ 5 |
...
| 40장 | 부수 효과를 격리한 앱 | `Eff<RT>` + `Has` DI + `LanguageExt.Sys` |
...
`Eff<RT>` + `Has<RT, ConsoleIO>` / `Has<RT, FileIO>` / `Has<RT, TimeIO>` 로 의존성을 주입하고, `LanguageExt.Sys` 의 라이브 구현으로 부수 효과를 앱 경계로 격리 한 콘솔 / HTTP 워크플로를 작성합니다. 7부의 런타임 주입과 11부의 테스트 더블이 같은 설계의 양면임을 실제 앱에서 확인합니다.
