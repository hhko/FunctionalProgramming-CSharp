# Part 10 — Real-world (실무 예제 — 실전 적용)

## 12부의 배경

기초 ~ 11부에서 우리는 함수형의 어휘 (Functor ~ Traversable), 효과 (Reader ~ Eff), 견고함 (Schedule / Resource / 동시성 / 스트리밍), 테스트를 각각 직접 구현하며 익혔습니다. 하지만 실무는이 도구들을 하나씩 쓰지 않습니다. 한 기능 안에서 검증 · 의존성 주입 · 오류 처리 · 자원 · 동시성 · 스트리밍 · 테스트가 동시에 얽힙니다.
...
- 효과 기반 애플리케이션 — `Eff<RT>` + `Has<RT, Trait>` DI + `LanguageExt.Sys` (Console / File / Time) 로 부수 작용을 격리 한 앱.
- 데이터 파이프라인 — Pipes / Schedule / Resource 를 결합한 스트리밍 처리.
...
- `Eff<RT>` + `Has<RT, Trait>` 로 의존성을 주입하고, `LanguageExt.Sys` 로 콘솔 / 파일 / 시간 부수 작용을 격리한 애플리케이션 작성.
- Pipes + Schedule + Resource 를 결합한 실전 데이터 파이프라인 구성.
...
|---|---|---|
| 도메인 모델링 + 검증 | Validation / Applicative / Monoid | Part 1 |
| 의존성 주입 + 부수 작용 격리 | `Eff<RT>` / `Has` / `LanguageExt.Sys` | Part 4 ~ 5 |
| 재시도 · 자원 · 추적 | Schedule / Resource / Activity | Part 6 |
| 동시 · 스트리밍 처리 | STM / Pipes / StreamT | Part 7 ~ 8 |
| 검증 | 합법칙 / property-based / 테스트 더블 | Part 9 |
...
| 40장 | 부수 작용을 격리한 앱 | `Eff<RT>` + `Has` DI + `LanguageExt.Sys` |
| 41장 | 데이터 파이프라인 | Pipes + Schedule + Resource 결합 |
...
`Eff<RT>` + `Has<RT, ConsoleIO>` / `Has<RT, FileIO>` / `Has<RT, TimeIO>` 로 의존성을 주입하고, `LanguageExt.Sys` 의 라이브 구현으로 부수 작용을 앱 경계로 격리 한 콘솔 / HTTP 워크플로를 작성합니다. 7부의 런타임 주입과 11부의 테스트 더블이 같은 설계의 양면임을 실제 앱에서 확인합니다.

### 41장 — 동시성·스트리밍 실전

10부의 Pipes 와 8부의 Schedule (주기적 트리거) / Resource (소스·싱크의 안전한 개폐) 를 결합해 대용량 입력 → 변환 → 적재 데이터 파이프라인을 구성합니다. 필요한 자리에 9부의 STM / Atom 을 끼워 동시 처리를 더합니다. 효과 · 자원 · 스트림 · 동시성이 한 파이프라인으로 합성되는 자리입니다.

### 42장 — 종합 capstone

기초 ~ 11부의 도구를 모두 동원하는 소형 애플리케이션을 한 편으로 만듭니다 (도메인 모델 + 효과 + 자원 + 스트리밍 + 테스트). 코드와 테스트를 함께 작성하며, 마지막으로 LanguageExt v5 샘플 (`Newsletter` / `CardGame` / `BlazorApp`) 을 펼쳐 이 책에서 손으로 만든 패턴의 변형으로 읽어내는 것으로 책 전체를 마무리합니다.
...
LanguageExt v5 의 `Samples/`(Newsletter·CardGame·BlazorApp·EffectsExamples)를 펼치면이 책에서 손으로 만든 패턴(`Eff<RT>`/`Has`/`Validation`/Pipes)의 변형으로 읽힙니다.
