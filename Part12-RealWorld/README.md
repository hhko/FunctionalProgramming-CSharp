...
기초 ~ 11부의 도구를 모두 동원하는 소형 애플리케이션을 한 편으로 만듭니다 (도메인 모델 + 효과 + 자원 + 스트리밍 + 테스트). 코드와 테스트를 함께 작성하며, 마지막으로 LanguageExt v5 샘플 (`Newsletter` / `CardGame` / `BlazorApp`) 을 펼쳐이 책에서 손으로 만든 패턴의 변형으로 읽어내는 것으로 책 전체를 마무리합니다.
