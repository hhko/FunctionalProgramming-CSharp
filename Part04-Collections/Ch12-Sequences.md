...
시퀀스의 `Bind` 는 각 원소에 그런 함수를 적용해 작은 시퀀스를 얻은 다음, 그 작은 시퀀스들을 순서대로 이어 붙입니다. 7장에서 `MyMaybe` 의 `Bind` 는 없으면 단락, `MyList` 의 `Bind` 는 여러 갈래로 펼침이었습니다. 같은 `Bind` 라는 한 도구의 서로 다른 두 얼굴이었습니다. 시퀀스의 `Bind` 는 그중 펼침 쪽 얼굴, 즉 한 입력이 여러 갈래로 입니다. 원소 하나가 작은 시퀀스가 되고, 그 작은 시퀀스들이 하나로 합쳐집니다.
