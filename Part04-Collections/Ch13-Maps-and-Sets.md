...
trait 은 아무 컨테이너에나 자동으로 붙는 도구가 아니었습니다. 4장에서 본 Functor 의 약속 (모양 보존) 과 무제약 시그니처를 지킬 수 있는 컨테이너에만 붙습니다. `MySet` 은 그 약속을 지킬 수 없어 Functor 의 경계에 섭니다. 이 경계를 보는 것이 추상을 더 정확히 이해하는 길입니다.
...
집합은 한 걸음 더 나아갑니다. 집합에 `map` 을 붙이려고 하면 두 가지가 막힙니다. 결과를 다시 중복 제거하려면 결과 타입에 동등성 비교가 필요한데 `map` 의 시그니처는 그런 제약을 허용하지 않고, 서로 다른 원소가 같은 값으로 가면 개수가 줄어 모양 보존이 깨집니다. 이 막힘이 곧 trait 의 경계입니다. 그래서 이 장에서 그 경계를 직접 만나 봅니다.
...
**첫째, 무제약 시그니처를 어깁니다.** `Map<A, B>` 의 시그니처는 결과 타입 `B` 에 아무 제약을 달지 않습니다. 그런데 집합은 결과를 다시 중복 제거해야 하고, 중복 제거에는 `B` 의 동등성 비교 (`EqualityComparer<B>`) 가 필요합니다. `K<F, A>` 인코딩으로는 `A` 나 `B` 에 제약을 달 수 없으므로, 무제약 `Map` 시그니처와 충돌합니다. Functor 의 약속은 `Map` 이 어떤 `A`·`B` 든 받는다는 것입니다. 한 컨테이너만 `B` 에 동등성 비교를 요구하면 그 `Map` 은 `Option` 이나 `List` 가 부착한 것과 더는 같은 trait 이 아니고, 모두가 같은 무제약 약속을 지켜야 trait 한 번 정의로 모든 시민이 능력을 얻는 4장의 이득이 성립합니다.
...
`MyMap` 의 `Map` 은 값만 바꾸고 키는 보존했습니다. 그렇다면 키 자체를 바꾸는 연산은 무엇일까요. 그것은 Functor 의 `map` 이 아닙니다. 키 변환은 두 가지를 부르기 때문입니다. 서로 다른 키가 같은 키로 가는 **충돌**, 그리고 키 순서가 바뀌는 **재정렬** 입니다.
...
상품 이름을 길이로 바꾸면 `apple` (5) 과 `cherry` (6) 처럼 길이가 다르면 살아남지만, 같은 길이의 키가 있으면 하나가 다른 하나를 덮어씁니다. 충돌이 일어나면 "어느 값이 이기는가" 라는 정책을 호출자가 정해야 하고, 이는 모양 보존을 깨는 연산입니다. 재정렬은 개수를 바꾸지 않아 그 자체로 모양 보존을 깨지는 않지만, 정렬 Map 에서는 키 순서가 출력 구조의 일부라 변환이 그 구조를 흔듭니다. 핵심 이유는 충돌입니다. 그래서 키 변환은 Functor 의 `map` 이 아니라 정책이 필요한 별도 함수로 둡니다.
...
가격 맵의 각 값을 검증한다고 합시다. 값 하나를 `Validation` 으로 검증하는 함수 `V → Validation<V>` 가 있을 때, `traverse` 는 `Map<K, V>` 를 `Validation<Map<K, V>>` 로 뒤집습니다. **모든 값이 통과해야 전체 Map 이 성공** 이고, 키는 그대로 보존됩니다.

```text
{apple: 1000, banana: -50}  traverse (값이 양수인지 검증)
  → apple: 1000  ✓
  → banana: -50  ✗
  → 전체 Invalid   (하나라도 실패하면 Map 전체 실패, 키는 보존)
```
