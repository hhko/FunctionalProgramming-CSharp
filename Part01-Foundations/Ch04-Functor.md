...
> 이 장에서 다룰 주제 — 1부의 첫 정통 추상. Normal World 의 함수 `a → b` 한 개를 받아 Elevated World 의 함수 `E<a> → E<b>` 로 끌어올리는 도구, 곧 `map` 의 일반화. 1장의 `E<a> → E<b>` 유형 함수 (1장 4 가지 함수 유형의 `E<a> → E<b>`) 를 어떤 Elevated World 든 만들 수 있게 하는 trait.
...
4장의 핵심은 한 줄로 압축됩니다. Normal World 의 1인자 함수를 Elevated World 에서 그대로 사용할 수 있도록 제공하는 lift 함수가 `map` 입니다. `(a → b) → (E<a> → E<b>)` 의 시그니처가 그 약속을 정확히 표현합니다. 5장 Applicative 의 `pure / apply` 가 N 인자 lift 로 확장되고, 4장은 그 사다리의 1인자 원형을 다룹니다.

> 4장의 3 부 구조 — 본문은 왜 이 함수가 필요한가 (목적) → 그 목적을 달성하기 위한 함수의 기능들 (기능) → 그 목적을 달성한 예제 (예제) 의 narrative arc 로 구성됩니다.
>
> - **§4.1 목적** — §1.6.1 의 1인자 lift 자리. 컨테이너마다 변환 코드를 다시 쓰는 반복을 *map* 한 번 정의로 해소합니다.
> - **§4.2 기능** — `Functor<F>` trait 의 시그니처와 `Map` 멤버. 시그니처가 모양 보존을 컴파일 타임에 강제합니다.
> - **§4.3 ~ §4.5 예제** — `MyList` / `MyMaybe` 인스턴스 + 어떤 Functor 든 받는 일반 함수 + 문자열 파싱 결과의 변환 예제 (§4.4.5).
> - **§4.6 ~ §4.7 기능 (법칙·합성)** — 두 법칙 (항등 + 합성) 이 목적의 성립을 보장하는 약속입니다.
> - **§4.8 ~ §4.9 예제 (경계·챌린지)** — Functor 가 아닌 변환들 + 직접 풀어 보는 챌린지입니다.
> - **§4.10 ~ §4.12 마무리** — §1.6.1 framing 정합 한 줄 + 5장 Applicative 다리입니다.

### 4.1.1 고통의 체험 — 컨테이너마다 다시 쓰는 변환

map 이 없으면 무엇이 아픈지 먼저 손으로 겪어 봅니다. 컨테이너에 담긴 값을 그저 두 배로 바꾸는 일을 생각해 봅니다. 값이 그냥 `int` 라면 `n * 2` 한 줄이면 끝납니다. 그런데 같은 값이 `MyMaybe<int>` (있을 수도 없을 수도 있는 값) 나 `MyList<int>` (여러 개의 값) 에 담겨 있으면, 두 배라는 연산 앞에 컨테이너를 여는 코드가 먼저 붙습니다.
...
같은 통증은 연산이 늘어나는 순간 분명해집니다. 세 배로 바꾸려면 `* 3` 한 글자만 다른 함수를 `MyMaybe` 용으로 한 번, `MyList` 용으로 한 번 또 적습니다. 문자열로 바꾸기, 1 더하기처럼 연산이 늘 때마다 같은 분기·순회 코드가 컨테이너 종류 × 연산 가짓수만큼 복제됩니다. 2장에서 본 N×M 격자가 바로 이 자리입니다.
...
1장 §1.7 에서 두 세계 사이의 함수 유형을 정리했습니다. 그중 `E<a> → E<b>` 유형 (4 가지 함수 유형의 `E<a> → E<b>`) 의 시그니처는 E 가 입출력에 같이 등장합니다.
...
이 *map* 이 **함수형의 본질** (합성 가능한 Elevated World 로 lift, §1.6.1) 의 lift 가 가장 단순한 형태로 등장하는 자리입니다. `Map : (a → b) → (E<a> → E<b>)` 는 lift 의 1인자 형태입니다. Normal World 의 함수 한 개를 Elevated World 에서 그대로 쓸 수 있게 끌어올리는 사다리입니다. 5장 Applicative 의 `Lift2 / Lift3 / Lift4` 가 다인자 확장이고, 4장은 그 사다리의 원형을 다룹니다.
...
Normal → Elevated 로 옮기는 도구 자체가 Lifting function 입니다. `map`, `pure`, `apply` 가 모두 lifting function 입니다. 4장의 `map` 은 가장 단순한 형태로, 1인자 함수 (`a → b`) 한 개를 끌어올립니다. 2인자 / 3인자 함수의 lifting 은 5장 (Applicative) 의 `Lift2`, `Lift3` 에서 다룹니다.
...
| 세 번째 — Lifted **functions** | `(a → b) → (E<a> → E<b>)` | **4장의 `map`** (= 5장 §5.5 의 `lift1`) |
...
4장은 세 번째 (Lifted functions) 중에서도 1인자 함수만 다룹니다. 5장에서 2인자, 3인자 함수의 lift (`Lift2`, `Lift3`) 로 확장됩니다.
...
└── Tests/FunctorLaws.cs               ← 두 법칙 검증 헬퍼 (§4.6)
```
...
> 참고 (어느 어법을 언제 쓰나) — 학습 코드는 ① (trait 정적) 으로 적으면 F 가 무엇인지 한눈에 보입니다. 일반 함수 안에서는 ② (모듈) 이 F 가 generic 매개변수일 때 자연스럽습니다 (§4.5 의 `MapAnyFunctor.Run` 이 그 예). 호출 측에서는 ③ (확장) 이 C# 의 점 호출 관습과 가장 잘 맞습니다. 세 어법은 동등하므로 어느 한쪽을 반드시 써야 하는 자리는 없습니다.
...
같은 Functor 메서드가 완전히 다른 자료 구조 (시퀀스 vs 옵셔널) 에서 각자 의미 있는 동작을 합니다. 세 어법은 호출 형태의 차이일뿐, Functor 의 동작은 자료 구조의 분기가 결정합니다.
...
문자열을 정수로 파싱하는 자리는 효과 인코딩 + 일반 연산 합성의 정통 예제입니다. *Map* 의 결정적 가치는 함수 변환이라는 문법적 형태가 아니라 **세 책임의 깔끔한 분리** 에 있습니다. 효과는 타입이, 연산은 함수가, 합성은 Map 이 책임집니다.
...
- **합성 — `Map(f, …)`** — *Map* 한 줄이 일반 연산 `f` 를 효과 인코딩 위로 승격시킵니다. 있음 분기 (`Just`) 에는 `f` 적용, 없음 분기 (`Nothing`) 에는 그대로 통과합니다. 분기 코드 `if (parsed != null) …` 자체가 사라집니다. 합성은 Map 의 책임입니다.
...
명령형 코드라면 효과 검사 (null 비교, try-catch) 와 일반 연산이 함수 본문 안에 섞여서 작성됩니다. 함수형은 **효과는 타입, 연산은 함수, 합성은 Map** — 세 책임이 깔끔히 나뉩니다. 독자가 작성하는 건 늘 효과를 모르는 일반 함수뿐, 분기 어법은 trait 의 메서드가 자동으로 입혀 줍니다. `E<a> → E<b>` 자리의 결정적 통찰이 구체 예제로 작동하는 자리입니다.

![효과 인코딩과 일반 연산의 분리 — Map 의 합성](./images/Ch04-Functor/04-parse-pipeline.svg)

**그림 4-4. 효과 인코딩 (`MyMaybe`) 과 일반 연산 (`f`) 의 분리** — 아래 행 Normal World 에 효과를 모르는 일반 연산 `f = (n => n * 2) : int → int`. 위 행 Elevated World 에 그 연산을 `Map` 으로 끌어올린 함수 `Map(f) : MyMaybe<int> → MyMaybe<int>`. 가운데 파란 lift 화살표 (`Map`) 가 Normal 의 `f` 를 Elevated 의 함수로 승격시킵니다. `int → int` 한 줄이 `MyMaybe<int> → MyMaybe<int>` 가 되면서 효과 (있음 / 없음 분기) 처리는 시그니처가 자동으로 책임집니다. **Map 의 가치는 함수 변환이 아니라 세 책임 (효과 / 연산 / 합성) 의 깔끔한 분리에 있습니다**.

`MyMaybeF.Map(f, …)` 한 줄이 1인자 lift 의 가장 단순한 실현입니다. Elevated World 의 값을 일반 연산 함수에 통과시켜 같은 Elevated World 의 새 값으로 만들어 줍니다. 효과 인코딩의 분기 처리는 시그니처가 책임지고, 변환 로직은 효과를 모르는 평범한 함수로 남습니다.
...
> 참고 (`Func<E, A>` 도 Functor) — `(Func<E, A>).Map(g) = e => g(f(e))` 가 함수 합성 자체입니다. 함수가 Functor 라는 점이 Reader Monad (3 부) 의 출발점입니다.
...
public static class FunctorHelpers
{
    public static K<F, A> MapTwice<F, A>(K<F, A> fa, Func<A, A> f)
        where F : Functor<F> =>
        F.Map(f, F.Map(f, fa));         //  Map(f ∘ f) 와 같음 (합성 법칙 §4.6.3)
...
trait 시그니처가 모양 보존을 강제하지만, 어떤 모양으로 보존되는가는 시그니처가 답하지 않습니다. 그 답이 두 법칙입니다. 두 법칙이 4 장의 목적 (1 인자 lift 가 합성을 보존한다) 이 실제로 성립한다는 약속입니다.
...
너무 자명해 보이지만, 이 자명함이 법칙으로 강제되어야 하는 이유는 §4.6.5 에서 `BogusListF` 와 함께 봅니다. 시그니처는 통과하지만 이 자명함을 어기는 가짜 Functor 가 존재합니다.

![Functor 항등 법칙](./images/Ch04-Functor/02-functor-law-identity.svg)
...
![Functor 합성 법칙](./images/Ch04-Functor/03-functor-law-composition.svg)
...
> ```
>
> `Compose.Of(g, f)(x)` 는 `g(f(x))` 와 같습니다. `Functions/MapAnyFunctor.cs` 안에 정의돼 있고 합성 절 (§4.7) 에서도 같은 헬퍼를 씁니다.

### 4.6.5 두 법칙이 함께 필요한 이유

Functor 의 목표는 한 줄입니다. Normal World 의 함수 `a → b` 를 Elevated World 의 함수 `E<a> → E<b>` 로, 컨테이너의 **모양** 을 보존하며 끌어올리는 것입니다. 여기서 **모양** 이란 컨테이너 종류 (`F` 가 같음) + 원소 개수 + 내부 배치 (Tree 노드 위치, Maybe 의 Just/Nothing variant 등) 의 묶음을 가리킵니다 (분류 어휘 §4.8.1 과 동일). trait 시그니처는 그중 컨테이너 종류만 강제하고 원소 개수·내부 배치 보존은 강제하지 못하므로, 두 법칙이 그 빈자리를 채웁니다.
...
> 흔한 함정 — `Map` 이 호출 횟수를 기록하거나 외부 상태를 건드린다면 합성 법칙이 깨집니다. Functor 의 `Map` 은 **순수 변환**이어야 합니다. 부수 효과가 필요하면 그 부수 효과를 캡슐화하는 Monad (Try · IO · Reader, 3 부 · 5 부) 의 자리입니다.
...
**그림 4-7. 순수 합성과 Functor 합성의 정렬** — 좌측은 Normal World 의 두 화살표. 우측은 Elevated World 의 두 화살표. 두 그림의 구조가 같습니다 (Functor 가 함수 합성의 모양을 Elevated World 에 옮겨 놓은 것).
...
`a → E<b>` 유형 (`a → E<b>`) 을 Functor 의 `Map` 으로 다루려는 시도가 겹친 Elevated 를 만들어 내는 과정과 그 평탄화가 `bind` 의 일이라는 점은 7장 §7.2.3 에서 본격적으로 다룹니다.
...
>
> `MyList<int>.Map(...)` 의 결과는 반드시 `MyList<…>`. `MyMaybe<…>` 로 변환할 수 없습니다. 컨테이너의 변환은 NaturalTransformation 의 자리입니다 (11장).

### 4.8.4 Functor 의 제한이 가치

만약 `Map` 이 모양도 바꿀 수 있다면 독자는 매 `Map` 호출마다 결과의 컨테이너가 무엇인지를 추적해야 합니다. 인지 부담이 결정적으로 커집니다.

함수형 trait 의 우아함은 각 trait 가 좁은 능력만 약속하는 데 있습니다. 좁은 약속 → 강한 보장 → 추론 단순화.
...
> **본문 어느 자리의 이해도를 묻는가**
>
> 1. 3-tuple 패턴 (§4.3·§4.4) 을 재귀 자료에 그대로 적용할 수 있는가.
> 2. 분기 처리 (§4.4.2, Maybe 의 Just/Nothing case 분기) 를 Tree 의 Leaf/Branch case 로 자연스럽게 이행할 수 있는가.
> 3. 두 법칙 (§4.6) 이 직접 작성한 `Map` 에서 성립함을 코드로 검증할 수 있는가.
>
> **해보기**
>
...
>
> 정답 코드: `code/Part1-Foundations/Ch04-Functor/Challenges/TreeFunctor.cs`.
...
> **본문 어느 자리의 이해도를 묻는가**
>
> 1. 시그니처 분류표 (§4.8.1) 를 새 메서드들에 직접 적용할 수 있는가.
> 2. `Map` 시그니처 (§4.2.1, `(Func<A, B>, K<F, A>) → K<F, B>`) 와 비교해 어떤 자리가 어긋나는가 짚을 수 있는가.
> 3. 좁은 약속 (§4.8.4, 모양 보존, 컨테이너 종류·개수 보존) 의 의미를 각 후보에 적용해 판정할 수 있는가.
>
> **판단 기준 (본문 §4.8.4)**
>
...
>
> 네 기준에 모두 예 면 Functor 의 `Map`. 하나라도 아니오 면 어느 trait 의 자리인지 표 (§4.8.2) 를 참고.
>
> **해보기**
>
...
> **검증 포인트**
>
> - 오답 한 개는 본문의 한 자리를 다시 읽어야 한다는 신호입니다. Reverse 가 막힌다면 Filter / Take 행 (§4.8.1), Lookup 이 막힌다면 `a → E<b>` 유형 (§4.8) 또는 7장 §7.2 미리보기로 갑니다.
>
> 정답: `code/Part1-Foundations/Ch04-Functor/Challenges/FunctorClassify.md`.
...
1. **새 자료 타입에 Functor 를 부착하고 그것이 진짜 Functor 임을 직접 검증할 수 있습니다** — 첫 번째 챌린지의 목표. 재귀 자료 (Tree) 까지 손에 익히면 Either, Try, Reader, Task 같은 1부 다음 인스턴스들이 같은 패턴의 변형으로 보입니다.
...
- `Functor<F>` trait 는 `E<a> → E<b>` 유형 함수의 일반화입니다 (1장 4 가지 함수 유형의 `E<a> → E<b>`). *함수형의 본질 — 합성 가능한 Elevated World 로 lift* (§1.6.1) 의 lift 가 가장 단순한 형태로 등장하는 자리입니다.
- `map` 은 elevated-world 글의 Lifting function 중 가장 단순한 형태입니다. 1인자 함수 (`a → b`) 를 lifted function (`E<a> → E<b>`) 으로 변환합니다. 같은 함수의 Function transformer 시각인 `lift1` 은 5장 `*Curry + Lift*` (§5.5) 에서 다룹니다.
- 두 법칙 (항등·합성) 이 모양 보존 약속을 구현 사이의 공통성으로 못 박습니다. 셋째 축 합성 가능성 (§1.6.1) 이 코드에 박히는 자리입니다.
- 5장의 Applicative 는 2인자 이상 함수 (`Lift2`, `Lift3`) 로 확장합니다. 7장의 Monad 는 World-crossing 함수 (`a → E<b>`) 를 `E<a> → E<b>` 유형으로 끌어올리는 `bind` 를 추가합니다.

네 줄이 4 장이 1 부 안에서 차지하는 위치의 요약입니다. lift 의 1 인자 원형이 4 장, 다인자 확장이 5 장, 반대 방향 (lower) 이 6 장, World-crossing 합성 살리기가 7 장입니다. 한 동사 `*lift*` 가 1 부 모든 추상의 공통 지도입니다.
...
- **trait 정적 호출** (`MyListF.Map<int, int>(f, nums)`) — 학습·명세 코드. `F` 가 무엇인지 한눈에 보임. 첫 줄 (§4.3.4 / §4.4.3).
- **모듈 어법** (`Functor.map<F, A, B>(f, fa)`) — generic 함수 본체. `F` 가 타입 매개변수일 때 자연 (§4.5 의 `MapAnyFunctor.Run`).
...
Functor 의 목표는 `a → b` 를 `E<a> → E<b>` 로 컨테이너의 모양을 보존하며 끌어올리는 것입니다. 모양 보존은 두 가지 약속의 묶음입니다. 항등 (값에 변환이 없으면 결과도 그대로) 과 합성 (두 변환을 차례로 적용한 결과가 합성 변환을 한 번에 적용한 결과와 같음) 입니다. 둘 중 하나가 빠지면 Map 이 보존해야 할 구조의 한쪽이 사라집니다. `BogusListF` (항등만 깨짐) 와 `CountingListF` (합성만 깨짐) 가 각자 다른 추론을 무너뜨립니다. §4.6.5 가 이 짝의 필연성을 전 절로 다룹니다.
...
Functor 의 `Map` 은 순수 변환이어야 합니다. 부수 효과가 필요하면 그 부수 효과를 캡슐화하는 Monad (Try · IO · Reader, 3 부 · 5 부) 의 자리입니다.

> Q8. `Map` 안에서 예외가 발생하면 어떻게 되나?

`Map` 자체는 예외 처리를 하지 않습니다. 독자의 변환 함수 `f` 가 예외를 던지면 그대로 호출자에게 전파됩니다. 함수형 패러다임에서는 예외도 부수 효과로 봅니다. Try Monad (2 부) 같은 부수 효과 캡슐화 Monad 로 다룹니다.

> Q9. Functor 가 왜 첫 정통 trait 입니까?

`Map` 한 메서드만 있으면 만족합니다. 가장 적은 약속의 trait 입니다. 그러면서 수많은 자료 구조가 인스턴스가 됩니다 (List, Maybe, Either, Tree, Function, Task, IO, ...). 낮은 부착 비용 + 높은 일반성을 가집니다. 7장의 Monad 가 더 많은 약속을 요구하는 상위 trait 임을 생각하면, Functor 가 가장 기초적인 sound 약속의 위치에 있습니다.

> Q10. `Map` 이 어떤 컨테이너에 적용 가능한가 — 실용적 판단 기준은?

모양 보존 정의 (§4.6.5, **컨테이너 종류** + **원소 개수** + **내부 배치** 의 묶음) 의 세 조건을 모두 만족해야 Functor 가 됩니다.
1. 컨테이너 종류 (`F`) 가 입출력에서 같은가?
2. 원소 개수가 입출력에서 같은가?
3. 각 원소에 변환 함수 `f : a → b` 가 적용됩니까?

셋 다 예 면 Functor. 하나라도 아니오 면 Filterable / Foldable / Monad 등 다른 trait 의 자리 (분류표 §4.8 참고). 다섯 후보에 직접 적용해 보는 자리가 §4.9.2 두 번째 챌린지입니다.

> Q11. Functor 와 람다 (Lambda) 의 관계는?

람다는 익명 함수의 syntax 이고, `Map` 인자에 자주 등장합니다 (`list.Map(n => n * 2)`). 람다 자체는 Functor 가 아니지만, 함수도 Functor 인스턴스이므로 람다가 다른 람다와 합성되는 모양이 Functor 의 합성과 같습니다. ReaderT (3 부) 에서 자세히 다룹니다.

---

## 4.12 다음 장으로 — 마무리 (5장 Applicative 다리)
...
4장의 `map` 이 1인자 함수의 lift 였다면, 5장 Applicative 는 같은 발상을 N 인자 함수의 lift 로 확장합니다. Normal World 의 다인자 함수를 Elevated World 에서 그대로 사용할 수 있도록 제공하는 lift 함수가 `pure` 와 `apply` 두 멤버입니다. 두 멤버 위에서 `Lift2`, `Lift3`, `Lift4` 가 자랍니다. [5장 — Applicative](./Ch05-Applicative.md) 로 넘어갑니다.

> 4장 → 5장 다리 — Functor 가 1인자 Normal 함수를 Elevated 에서 그대로 쓸 수 있게 한다면, Applicative 는 N 인자 Normal 함수를 Elevated 에서 그대로 쓸 수 있게 합니다. 같은 *lift* 동사의 1인자 원형 → 다인자 확장 사다리입니다.

> **실무 디딤돌** — Functor 의 `Map` 은 후속 trait (Applicative / Monad / Foldable / Traversable) 의 디딤돌입니다. 5 부 (Eff / IO) 의 효과 모나드도 `Map` 을 가장 기초 연산으로 사용합니다.
>
> **테스트 디딤돌** — §4.6 의 두 법칙 (항등 / 합성) 검증은 9 부 (Functional Testing) 의 *property-based 테스트* (CsCheck / FsCheck) 로 임의 입력에 자동 반복됩니다. 이 챕터의 콘솔 `bool` 검증이 표준 테스트로 옮겨가는 디딤돌입니다.