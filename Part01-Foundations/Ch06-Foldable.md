...
> 참고 — 이 장은 4장 (Functor) 의 짝입니다. Functor 가 `E<a> → E<b>` 유형 `E<a> → E<b>` (가로 이동) 의 일반화라면, Foldable 은 `E<a> → b` 유형 `E<a> → b` (끌어내림) 의 일반화. 두 trait 이 합쳐지면 8장의 Traversable 이 됩니다.
...
이 *fold* 가 함수형의 본질 — 합성 가능한 Elevated World 로 lift (§1.6.1) 의 반대 방향입니다. 4장 *map* 이 *lift* (끌어올림, Normal → Elevated) 의 가장 단순한 형태였다면, 6장 *fold* 는 *lower* (끌어내림, Elevated → Normal) 의 일반화입니다. 두 추상이 두 평행 세계 사이의 두 방향 사다리가 됩니다.
...
**그림 5-2. `map` 끌어올림 vs `fold` 끌어내림** — 두 추상이 격자 위에서 서로 반대 방향의 화살표. `map` 은 수직 끌어올림 (Normal 의 함수 `a → b` 를 Elevated 의 함수 `E<a> → E<b>` 로). `fold` 는 대각선 끌어내림 (Elevated 의 컨테이너 `E<a>` 를 Normal 의 한 값 `b` 로). 방향과 시그니처만 다른 두 추상이 같은 격자 위에 자리잡습니다. `fold` 의 step 함수와 seed 의 세부는 §6.1.4 의 mental model 과 §6.2 의 trait 시그니처에서 다룹니다 (그림 5-1 참고). 8장의 Traversable 은 두 방향을 동시에 다루는 추상이 됩니다.
...
> 핵심 — 두 그림이 같은 자료 + 같은 step + 같은 seed 에서 같은 결과 6 에 도착하지만 내부 계산 순서가 다릅니다. 가환·결합 step (덧셈, 곱셈, AND, OR) 이면 결과가 같고, 비가환 step (문자열 연결, 뺄셈) 이면 다릅니다. §6.6 의 결과 일관성 법칙이 언제 같은가의 조건을 형식화합니다.
...
**핵심** — `reduce` 는 모든 타입이 같을 때만 동작. `fold` 는 초기값으로 타입을 자유롭게 변환. 그래서 `Sum`, `Count`, `ToList`, `Average` 같은 자유 함수가 모두 `fold` 의 인스턴스 (§6.7 의 카탈로그). `reduce` 는 `fold` 의 특수한 경우 (= `seed = 자료의 첫 원소`, 결과 타입 = 원소 타입).
...
> 참고 (어느 어법을 언제 쓰나) — 학습 코드는 ① (trait 정적) 으로 적으면 F 가 무엇인지 한눈에 보입니다. 일반 함수 안에서는 ② (모듈) 이 F 가 generic 매개변수 일 때 자연스럽습니다 (§6.5 의 `SumAnyFoldable.Run` 이 그 예). 호출 측 에서는 ③ (확장) 이 C# 의 점 호출 관습과 가장 잘 맞습니다. 세 어법은 동등합니다. 어느 한쪽을 반드시 써야 하는 자리는 없습니다.
...
**그림 5-5. MyList Foldable 의 3-tuple 구조** — 위 행의 두 interface (`K<F, A>`, `Foldable<F>`) 가 아래 행의 두 구현 (`MyList<A>`, `MyListF`) 으로 각각 끌어내려집니다. `MyListF.FoldRight` / `MyListF.FoldLeft` 안에서 `K<MyListF, A>` 가 `.As()` 로 `MyList<A>` 가 되고, 그 `Items` 를 두 방향으로 순회합니다. 4장 §4.3.2 의 MyList Functor 다이어그램과 완전히 같은 구조. 같은 자료 타입에 다른 능력만 부착됐습니다.
...
§6.2.6 의 세 호출 어법이 MyList 에서 어떻게 보이는지 확인합니다. 세 줄 모두 결과가 동일합니다. 같은 `MyListF.FoldLeft` 로 디스패치됩니다.
...
>
> 자료 구조와 step 함수의 수학적 성질이 선택을 결정합니다. 두 방향이 같은 결과를 내는 조건은 §6.6 의 일관성 법칙에서 형식화합니다.
...
MyMaybe 도 §6.2.6 의 세 어법을 그대로 받습니다. `MyMaybeF` 가 `Foldable<MyMaybeF>` 를 만족하기 때문에 MyMaybe 전용 코드 없이 세 어법 모두 자동 디스패치됩니다.
...
4장의 ParseInt + Map(f) (§4.4.5) 와 같은 자료 (MyMaybe) 위의 평행 예제입니다. 4장이 Elevated 안에서의 1인자 lift (`E<a> → E<b>`) 였다면, 6장은 Elevated 를 Normal 로 회수하는 lower (`E<a> → b`) 자리 — 효과 인코딩 + 일반 누적 합성의 정통 lower 예제. *Fold* 의 결정적 가치는 반복 처리라는 문법적 형태가 아니라 **세 책임의 깔끔한 분리** 에 있습니다 — 효과는 타입 / 누적은 함수 / 합성은 Fold.
...
임의 자료 구조가 원소 순회 + 누적을 만족하면 Foldable 이 됩니다. 각 Elevated World 는 고유한 `fold` 구현을 갖지만, 시그니처는 같습니다. 표의 `MyTree<A>` 는 §6.9 의 첫 챌린지에서 직접 부착해 봅니다.
...
새 자료 타입이 Foldable trait 한 개 만족으로 수십 개 함수가 자동 적용 됩니다. 4장 §4.5.4 의 표가 한 줄 더 길어진 모양 — 함수형 prelude 의 누적 효과입니다.

특히 Foldable 은 abstract 가 두 개 (FoldRight + FoldLeft) 라 기여 두 개에 산출 수십의 결정적 ROI 를 보여 줍니다. §6.7 에서 자유 함수 누적의 전체 모습을 다시 봅니다.
...
>
> step 함수가 호출 횟수를 기록 하거나 외부 상태를 건드린다면 자유 함수의 두 정의가 부수 효과를 다르게 누적해 관찰 가능한 차이가 생깁니다. Foldable 의 step 은 **순수 함수** 여야 합니다 — 부수 효과가 필요하면 그 부수 효과를 캡슐화하는 Monad (Try · IO · Reader — 5부 · 3부) 의 자리.
...
시그니처가 같은 모양인데 결과의미가 달라지는 자리 — 결과 일관성은 두 방향의 동등성을, 부수 효과 없음은 호출 자리 무관성을 각각 책임집니다. **둘 다** 만족할 때만 `Foldable` 이 **진짜 Foldable**. 시그니처를 통과한 모든 자유 함수 추론이 안전한 trait. 한쪽이라도 빠지면 나머지 자리에서 결과가 바뀝니다. 함수형 trait 의 일관된 추론이 두 법칙의 짝 위에서 비로소 성립합니다.
## §6.7 Foldable 의 자유 함수 누적 — 기능 (abstract 2개에 산출 수십)
...
### 5.7.5 LanguageExt v5 의 abstract 선택 — FoldWhile 로 일반화

이 책은 `FoldRight` / `FoldLeft` 를 `static abstract` 로 두었습니다. 순수 catamorphism 정통 — Haskell 의 `foldr` / `foldl` 어휘를 그대로 매핑한 자리입니다. 학습 단계에서는 fold = 재귀 방향의 본질을 어휘로 정착시키는 것이 우선입니다.
...
| `Parse` | `string → Option<int>` | ✗ | `a → E<b>` (`a → E<b>` 유형) — Monad / `bind` (9장) |
| `Traverse` | `(Func<a, E<b>>, T<a>) → E<T<b>>` | ✗ | 양쪽 모두 Elevated — Traversable 의 자리 (8장) |
...
| Parse / FindUser | Monad 의 `bind` | 9장 |
| Traverse | Traversable | 8장 |
...
>
> Foldable 의 약속은 컨테이너를 소비 합니다. 결과 타입에 F 가 없는 것이 그 형식화 (§6.2.1). `Sum`, `Count` 의 결과는 Normal 의 한 값이고, 그 결과로 원래 컨테이너를 다시 만들 수 없습니다. Foldable 은 단방향 끌어내림입니다. 컨테이너 위에서 다시 변환이 필요하면 Functor (4장), 양방향 이동이 필요하면 Traversable (8장) 으로 갑니다.
...
함수형 trait 의 우아함은 각 trait 가 좁은 능력만 약속 하는 데 있습니다. Foldable 의 약속은 컨테이너 소비 + 한 값 누적 한 줄. 좁은 약속 → 강한 보장 → 추론 단순화. 그 좁은 약속이 §6.7 의 수십 개 자유 함수의 토대가 됩니다.
## §6.9 직접 해보기 — 예제 (챌린지)

본문을 읽은 것과 손으로 작성·분류할 수 있는 것의 차이를 만듭니다. 두 챌린지는 본문의 결정적 자리를 직접 묻습니다. 챌린지 ① 은 §6.3·§6.4·§6.6 (3-tuple 부착 + 일관성 법칙) 의 재귀 자료 구조 적용, 챌린지 ② 는 §6.8 (Foldable 인지 아닌지) 의 시그니처 분류 능력입니다. 더 깊은 챌린지 (Const, Pair) 는 §6.9.3 에서 별도로 다룹니다.
...
> 2. §6.4.2 의 분기 처리 (Maybe 의 Just/Nothing case 분기) 를 Tree 의 Leaf/Branch case 로 자연스럽게 이행할 수 있는가.
> 3. §6.6 의 일관성 법칙이 자기가 작성한 `FoldRight`/`FoldLeft` 에서 성립함을 코드로 검증할 수 있는가.
>
> **해보기**
>
...
> **본문 어느 자리의 이해도를 묻는가**
>
> 1. §6.8.1 의 시그니처 분류표를 새 메서드들에 직접 적용할 수 있는가.
> 2. §6.2.1 의 `Fold` 시그니처 `(Func, seed, K<F, A>) → B` 와 비교해 어떤 자리가 어긋나는가 짚을 수 있는가.
> 3. §6.8.4 의 좁은 약속 (컨테이너 소비 + 한 값 누적) 의 의미를 각 후보에 적용해 판정 할 수 있는가.
>
...
>
> 세 기준에 모두 예 면 Foldable 의 `Fold`. 하나라도 아니오 면 어느 trait 의 자리 인지 §6.8.2 의 표를 참고.
>
> **해보기**
>
...
두 능력 모두 본문을 읽기만 해서는 굳지 않습니다. 손으로 작성하고 분류해 보아야 본문이 자기 어휘가 됩니다. 심화 챌린지 (§6.9.3) 는 두 능력 위에서 경계 사례를 만지는 자리입니다. 본 능력이 단단해진 뒤 도전합니다.
...
- 8장의 Traversable 은 Functor + Foldable 의 합으로, `T<E<a>> ↔ E<T<a>>` 양방향 변환을 다룹니다. 두 trait 의 능력이 한 추상으로 묶입니다.
- 5장의 pure (lift) 와이 장의 fold (lower) 가 두 세계 사이의 두 방향 사다리입니다. 두 추상이 합쳐져야 Elevated World 사이의 자유로운 이동이 완성됩니다.

이 다섯 줄이 6장이 1부 안에서 차지하는 자리의 요약입니다. lift 와 lower 가 두 평행 세계의 두 방향 사다리라는 §1.6.1 의 그림이 코드에 박히는 자리입니다.
...
그렇습니다. `IEnumerable<T>.Aggregate(seed, f)` 가 List Foldable 의 FoldLeft 의 인스턴스 구현입니다. C# 표준에는 오른쪽 접기의 친숙한 이름이 없는데, Foldable trait 은 두 방향 (`FoldRight` + `FoldLeft`) 을 대칭으로 제공합니다. §6.2.5 의 확장 어법 (`fa.FoldLeft(f, seed)`) 이 `xs.Aggregate(seed, f)` 와 같은 자리이고, `K<F, A>` 위 점 호출입니다.

> Q3. 세 가지 호출 어법 (trait 정적 / 모듈 / 확장) 중 언제 어느 것을 쓰나?

셋 모두 같은 `Foldable<F>.FoldRight` / `FoldLeft` 로 디스패치되어 결과는 동일합니다 (§6.2.6). 자리에 따른 선호:
- **trait 정적 호출** (`MyListF.FoldLeft<int, int>(f, 0, nums)`) — 학습·명세 코드. F 가 무엇인지 한눈에 보임. §6.3.4 / §6.4.3 의 첫 줄.
- **모듈 어법** (`Foldable.foldLeft<F, A, B>(f, 0, fa)`) — generic 함수 본체. F 가 타입 매개변수일 때 자연 (§6.5 의 `SumAnyFoldable.Run`).
...
§6.3.5 의 함정 박스에서 두 방향의 선택 기준을 정리.

> Q5. 시그니처는 통과하지만 일관성은 깨진 "가짜 Foldable" 이 정말 존재할 수 있나?

존재합니다. §6.6.5 의 두 예가 그 증명.
...
두 abstract 가 같은 자료의 두 시각으로 짝으로 존재해야 Foldable 의 full power 가 나옵니다. §6.6 의 일관성 법칙이 두 시각의 정합성을 약속합니다.
...
4장 Functor 가 `E<a> → E<b>` 유형의 일반화라면, 이 장 Foldable 은 `E<a> → b` 유형의 일반화입니다. 4분면 격자의 두 인접 자리를 두 trait 이 채웁니다. 두 trait 의 합 (9장 Traversable) 이 복합 변환의 정통 자리입니다. 1부의 기초 두 정통이 나란히 자리잡는 이유입니다.

또한 Foldable 의 abstract 2 → virtual 다수 ROI 는 독자에게 trait 패턴의 가장 큰 가치를 보여 주는 결정적 예. §6.7 의 자유 함수 카탈로그가 그 증명.
...
셋 다 예 면 Foldable. 하나라도 아니오 면 Functor / Filterable / Monad / Traversable 등 다른 trait 의 자리 (§6.8 의 분류표 참고). 다섯 후보에 직접 적용해 보는 자리가 §6.9.2 챌린지 ② 입니다.
...
6장까지 두 평행 세계 사이의 두 방향 사다리 (lift / lower) 가 완성되었습니다. 7장 Monad 는 그 사다리 위에서 또 다른 자리를 답합니다. 출력 타입만 Elevated 인 함수 (`a → E<b>`) 끼리는 직접 합성이 안 됩니다 (1장 §1.7.4 의 어법 어긋남 자리). `bind` 가이 World-crossing 함수의 합성을 회복합니다. [7장 — Monad](./Ch07-Monad.md) 로 넘어갑니다.

> §6 → §7 의 다리 — 6장의 *fold* 가 *Elevated → Normal* 의 lower 였다면, 7장의 *bind* 는 `a → E<b>` 같은 어법이 어긋난 함수의 합성을 Elevated 안에서 회복 하는 자리입니다. 1장 §1.7.4 의 합성 어긋남이 7장 *Monad* 의 `bind` 한 메서드로 해소 됩니다.