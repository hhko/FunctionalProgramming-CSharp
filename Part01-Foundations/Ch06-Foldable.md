...
> 참고 — 이 장은 4장 (Functor) 의 짝입니다. Functor 가 `E<a> → E<b>` 유형 `E<a> → E<b>` (가로 이동) 의 일반화라면, Foldable 은 `E<a> → b` 유형 `E<a> → b` (끌어내림) 의 일반화. 두 trait 이 합쳐지면 9장의 Traversable 이 됩니다.
...
> - **§6.10 ~ §6.12 마무리** — §1.6.1 framing 정합 + 7장 Monad (합성 회복) 다리.
...
4장 §4.1.3 의 Lifting 3 종류 표는 Normal → Elevated 방향이었습니다. Foldable 의 `fold` 는 그 반대 방향, Elevated → Normal 의 Lowering 정통입니다.
...
같은 결과에 도착하는 step 도 있고 (가환·결합이면) 결과가 갈리는 step 도 있습니다 (예: 문자열 연결, 뺄셈). §6.6 에서 언제 두 결과가 같은가를 다룹니다.
...
#### 3-tuple 다이어그램 — MyMaybe Foldable
...
### 6.4.4 MyList 와 MyMaybe 의 Fold 비교
...
| Filter / Where | Filterable | §6 (Foldable) 언급 |
...
> 2. 분기 처리 (§6.4.2, Maybe 의 Just/Nothing case 분기) 를 Tree 의 Leaf/Branch case 로 자연스럽게 이행할 수 있는가.
...
- **trait 정적 호출** (`MyListF.FoldLeft<int, int>(f, 0, nums)`) — 학습·명세 코드. F 가 무엇인지 한눈에 보임. 첫 줄 (§6.3.4, §6.4.3).
...
다음 장 (7장) — Monad:    a → E<b> 의 합성 회복            bind / Kleisli >=>
```

6장까지 두 평행 세계 사이의 두 방향 사다리 (lift / lower) 가 완성되었습니다. 7장 Monad 는 그 사다리 위에서 또 다른 자리를 답합니다. 출력 타입만 Elevated 인 함수 (`a → E<b>`) 끼리는 직접 합성이 안 됩니다 (어법 어긋남 자리, 1장 §1.7.4). `bind` 가 이 World-crossing 함수의 합성을 회복합니다. [7장 — Monad](./Ch07-Monad.md) 로 넘어갑니다.
...
>
> **테스트 디딤돌** — 6 장의 Foldable 법칙 (Monoid 일치) 은 9 부의 property-based 테스트로 자연 확장됩니다. 임의의 `List<int>`, 임의의 Monoid 인스턴스에 대한 자동 검증이 출발점입니다.
