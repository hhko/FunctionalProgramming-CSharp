...
> 이 장에서 다룰 주제 — 1부의 두 번째 정통 추상. Elevated World 의 컨테이너 `E<a>` 를 받아 Normal World 의 한 값 `b` 로 끌어내리는 도구, 곧 `fold` 의 일반화. 1장의 `E<a> → b` 유형 함수 (1장 4 가지 함수 유형의 `E<a> → b`) 를 어떤 Elevated World 든 만들 수 있게 하는 trait. `Sum`, `Count`, `All`, `Any` 같이 친숙한 메서드의 정통 뿌리.
...
> - [ ] abstract 2 개 위에 자유 함수 수십 개가 자라나는 ROI 를 설명할 수 있습니다.
...
6장의 핵심은 한 줄로 압축됩니다. 4장 / 5장이 Normal → Elevated 로의 lift (1인자 / N 인자) 였다면, 6장은 반대 방향입니다. Elevated World 의 구조를 Normal World 의 한 값으로 돌아오게 (끌어내림 / lower) 하는 함수가 `fold` 입니다. `E<a> → b` 의 시그니처가 그 끌어내림을 정확히 표현합니다. 3·5장의 *lift* 와 6장의 *lower* 가 두 평행 세계 사이의 두 방향 사다리가 됩니다.

> 6장의 3부 구조 — 4장 / 5장과 같은 narrative arc 로 구성됩니다.
>
> - **§6.1 목적** — §1.6.1 의 lower (반대 방향) 자리. 컨테이너의 구조를 Normal 한 값으로 압축하는 패턴을 *fold* 한 번 정의로 해소.
> - **§6.2 기능** — `Foldable<F>` trait 의 시그니처와 `FoldRight` + `FoldLeft` 두 멤버. 시그니처가 F 가 결과에서 사라짐을 컴파일 타임에 강제.
> - **§6.3 ~ §6.5 예제** — `MyList Foldable` / `MyMaybe Foldable` + 어떤 Foldable 든 받는 일반 함수.
> - **§6.6 ~ §6.7 기능 (법칙·일관성)** — 두 법칙 (결과 일관성 + 부수 효과 없음) + 자유 함수의 누적. 목적의 성립을 보장 하는 약속.
> - **§6.8 ~ §6.9 예제 (경계·챌린지)** — Foldable 이 아닌 변환들 + 직접 풀어 보는 챌린지.
> - **§6.10 ~ §6.12 마무리** — §1.6.1 framing 정합 + 7장 Monad (합성 되살리기) 다리.

### 6.1.1 1장 4 가지 함수 유형의 `E<a> → b` 자리

1장 §1.7 에서 두 세계 사이의 함수 유형을 정리했습니다. 그중 `E<a> → b` 유형 (4 가지 함수 유형의 `E<a> → b`) 의 시그니처는 입력에는 E 가 있고 출력에는 사라지는 모양입니다.
...
이 *fold* 가 함수형의 본질, 곧 합성 가능한 Elevated World 로 lift 의 반대 방향입니다. 4장 *map* 이 *lift* (끌어올림, Normal → Elevated) 의 가장 단순한 형태였다면, 6장 *fold* 는 *lower* (끌어내림, Elevated → Normal) 의 일반화입니다. 두 가지 추상이 두 평행 세계 사이의 두 방향 사다리가 됩니다.
...
**그림 6-1. `fold`: Elevated 의 구조를 Normal 의 한 값으로 끌어내림** — 위 행 Elevated World 의 컨테이너 `E<a>` 안에 여러 원소 (`a₁, a₂, a₃, …`) 가 들어 있습니다. `fold` 가 step 함수를 들고 모든 원소를 차례로 누적해 컨테이너 모양은 소비 되고 아래 행 Normal World 의 한 값 `b` 만 남습니다. Functor 의 `map` 이 같은 행 안에서의 가로 이동이라면, `fold` 는 위에서 아래로 향하는 끌어내림입니다.

### 6.1.2 4 가지 함수 유형 — 시그니처와 1부 매핑
...
**그림 6-2. `map` 끌어올림 vs `fold` 끌어내림** — 두 가지 추상이 두 세계 위에서 서로 반대 방향의 화살표. `map` 은 수직 끌어올림 (Normal 의 함수 `a → b` 를 Elevated 의 함수 `E<a> → E<b>` 로). `fold` 는 대각선 끌어내림 (Elevated 의 컨테이너 `E<a>` 를 Normal 의 한 값 `b` 로). 방향과 시그니처만 다른 두 가지 추상이 같은 두 세계 위에 자리잡습니다. `fold` 의 step 함수와 seed 의 세부는 mental model (§6.1.4) 과 trait 시그니처 (§6.2) 에서 다룹니다 (그림 6-1 참고). 9장의 Traversable 은 두 방향을 동시에 다루는 추상이 됩니다.
...
### 6.1.3 Lifting 의 반대 방향 — Lowering

4장 §4.1.4 의 Lifting 3 종류 표는 Normal → Elevated 방향이었습니다. Foldable 의 `fold` 는 그 반대 방향, Elevated → Normal 의 Lowering 정통입니다.
...
### 6.1.4 fold 의 mental model — 함수형의 for-loop
...
| 부수 효과 가능성 | 자유 (`Console.WriteLine` 등) | step 이 순수해야 일관성 보존 (§6.6) |
...
이 mental model 위에서 §6.2 의 trait 시그니처를 보면 왜 step 함수와 seed 두 부품으로 구성되었는지가 자명해집니다. for-loop 의 두 핵심 (반복 본체 + 시작 상태) 을 함수로 명시화 한 것입니다.
...
같은 결과에 도착하는 step 도 있고 (가환·결합이면) 결과가 갈리는 step 도 있습니다 (예: 문자열 연결, 뺄셈). 두 결과가 같은 조건은 결과 일관성 법칙 (§6.6) 에서 다룹니다.
...
> 핵심 — 두 그림이 같은 자료 + 같은 step + 같은 seed 에서 같은 결과 6 에 도착하지만 내부 계산 순서가 다릅니다. 가환·결합 step (덧셈, 곱셈, AND, OR) 이면 결과가 같고, 비가환 step (문자열 연결, 뺄셈) 이면 다릅니다. 결과 일관성 법칙 (§6.6) 이 언제 같은가의 조건을 형식화합니다.
...
**핵심** — `reduce` 는 모든 타입이 같을 때만 동작합니다. `fold` 는 초기값으로 타입을 자유롭게 변환합니다. 그래서 `Sum`, `Count`, `ToList`, `Average` 같은 자유 함수가 모두 `fold` 의 인스턴스입니다 (자유 함수 카탈로그는 §6.7). `reduce` 는 `fold` 의 특수한 경우 (`seed = 자료의 첫 원소`, 결과 타입 = 원소 타입) 입니다.

> David Raab 의 통찰 — "`reduce` 는 모든 타입이 같아야 한다는 제약 때문에 표현력이 좁습니다. `fold` 의 초기값은 그 제약을 풀어 어떤 누적 타입으로든 변환 가능하게 만듭니다." ([Understanding fold](https://davidraab.github.io/posts/understanding-fold/)). 이 자유도가 Foldable trait 의 ROI (§6.7) 의 토대.

### 6.2.5 변성 (variance) — `K<in F, A>`

4장과 동일. `K<F, A>` 의 정확한 정의는 `K<in F, A>` (F 는 contravariant). 학습용 코드에서는 그대로 따라가면 됩니다.

### 6.2.6 라이브러리 4-파일 패턴 정렬
...
└── Tests/FoldableLaws.cs            ← 일관성 법칙 검증 헬퍼 (§6.6)
```
...
> 참고 (어느 어법을 언제 쓰나) — 학습 코드는 ① (trait 정적) 으로 적으면 F 가 무엇인지 한눈에 보입니다. 일반 함수 안에서는 ② (모듈) 이 F 가 generic 매개변수 일 때 자연스럽습니다 (`SumAnyFoldable.Run` (§6.5) 이 그 예). 호출 측 에서는 ③ (확장) 이 C# 의 점 호출 관습과 가장 잘 맞습니다. 세 어법은 동등합니다. 어느 한쪽을 반드시 써야 하는 자리는 없습니다.
...
**그림 6-5. MyList Foldable 의 3-tuple 구조** — 위 행의 두 interface (`K<F, A>`, `Foldable<F>`) 가 아래 행의 두 구현 (`MyList<A>`, `MyListF`) 으로 각각 끌어내려집니다. `MyListF.FoldRight` / `MyListF.FoldLeft` 안에서 `K<MyListF, A>` 가 `.As()` 로 `MyList<A>` 가 되고, 그 `Items` 를 두 방향으로 순회합니다. 4장 §4.3.2 의 MyList Functor 다이어그램과 완전히 같은 구조. 같은 자료 타입에 다른 능력만 부착됐습니다.
...
`fa.As()` 는 추상 신호 타입 `K<MyListF, A>` 를 진짜 자료 타입 `MyList<A>` 로 변환합니다. 3-tuple 패턴의 결정적 비용입니다. 컴파일러가 F = MyListF 면 자료 타입이 `MyList<A>` 라는 연결을 모르기 때문에 사람이 알려 줍니다. 4장 §4.3.3 과 같은 패턴입니다.
...
List 의 모양 (5 개 원소) 은 모두 소비 되고 Normal 의 한 값 (15) 만 남았습니다. Foldable 의 첫 동작입니다. 책의 이후 예제에서는 어법이 결정적이지 않으면 ③ 확장 어법을 우선합니다 (가장 간결).
...
>
> 자료 구조와 step 함수의 수학적 성질이 선택을 결정합니다. 두 방향이 같은 결과를 내는 조건은 일관성 법칙 (§6.6) 에서 형식화합니다.
...
4장의 ParseInt + Map(f) 와 같은 자료 (MyMaybe) 위의 평행 예제입니다. 4장이 Elevated 안에서의 1인자 lift (`E<a> → E<b>`) 였다면, 6장은 Elevated 를 Normal 로 끌어내리는 lower (`E<a> → b`) 자리, 곧 효과 인코딩 + 일반 누적 합성의 정통 lower 예제입니다. *Fold* 의 결정적 가치는 반복 처리라는 문법적 형태가 아니라 **세 책임의 깔끔한 분리** 에 있습니다 (효과는 타입, 누적은 함수, 합성은 Fold).
...
- **일반 누적 — `step = (n, acc) => n + acc`** — 원소를 누적에 더함이라는 평범한 2인자 함수는 효과를 모릅니다. 시그니처는 `(int, int) → int` 한 줄이고 `MyMaybe` 어휘는 들어가지 않습니다. 데이터가 있다는 가정 하의 단순 누적 함수입니다. 누적은 함수의 책임입니다.
...
```csharp
// 문자열을 int 로 파싱 — 효과 인코딩 (있음/없음을 MyMaybe 타입에 감춤). §4.4.5 와 동일.
...
1단계의 효과 인코딩 (있음/없음 분기 가능성) 위에서 2단계의 일반 누적 (`(n, acc) => n + acc`) 이 자유롭게 합성됩니다. 두 단계가 어법이 어긋납니다. 1단계는 `MyMaybe<int>` 로 끌어올리고, 2단계는 같은 `MyMaybe<int>` 를 `int` 로 끌어내립니다. 4장 Map (E → E) 과 반대 방향입니다.
...
명령형 코드라면 효과 검사 (`null` 비교, `TryParse` 분기) 와 일반 누적이 함수 본문 안에 섞여서 작성됩니다. 함수형은 효과는 타입, 누적은 함수, 합성은 Fold 입니다. 세 책임이 깔끔히 나뉩니다. 독자가 작성하는 건 늘 효과를 모르는 일반 누적 함수뿐, 분기 어법은 trait 의 `FoldLeft` 가 자동으로 입혀 줍니다. 결정적 통찰이 lower 자리로 확장되는 자리입니다.
...
새 자료 타입이 Foldable trait 한 개 만족으로 수십 개 함수가 자동 적용 됩니다. 4장의 자유 함수 누적 표가 한 줄 더 길어진 모양, 곧 함수형 prelude 의 누적 효과입니다.

특히 Foldable 은 abstract 가 두 개 (FoldRight + FoldLeft) 라 기여 두 개에 산출 수십의 결정적 ROI 를 보여 줍니다. 자유 함수 누적의 전체 모습은 §6.7 에서 다시 봅니다.
...
법칙은 두 가지입니다. 결과 일관성 (가환·결합 step 일 때 두 방향이 같은 결과) 과 부수 효과 없음 (step 함수가 순수) 입니다.
...
step 의 값 의미 (덧셈) 는 순수 인데, 카운터가 부수 효과입니다. `Sum` 자유 함수를 호출자가 직접 FoldLeft 로 또 계산 하면 counter 가 두 배가 됩니다. 어디서 누가 fold 했는지가 결과에 영향을 미칩니다. 추론이 불가능합니다.
...
부작용은 다음과 같습니다. 이 trait 위에서는 컴파일러 / 사용자의 자유 함수 재배치가 안전하지 않습니다. FoldLeft 정의 vs FoldRight 정의가 결과의미를 바꿀 수 있습니다.

> 흔한 함정 — step 함수가 부수 효과를 가지면 일관성이 깨집니다
>
> step 함수가 호출 횟수를 기록 하거나 외부 상태를 건드린다면 자유 함수의 두 정의가 부수 효과를 다르게 누적해 관찰 가능한 차이가 생깁니다. Foldable 의 step 은 **순수 함수** 여야 합니다. 부수 효과가 필요하면 그 부수 효과를 캡슐화하는 Monad (Try, IO, Reader, 5부 / 3부) 의 자리입니다.
...
이 책은 `FoldRight` / `FoldLeft` 를 `static abstract` 로 두었습니다. 순수 catamorphism 정통입니다. Haskell 의 `foldr` / `foldl` 어휘를 그대로 매핑한 자리입니다. 학습 단계에서는 fold = 재귀 방향의 본질을 어휘로 정착시키는 것이 우선입니다.

LanguageExt v5 의 실무 어법은 다릅니다. v5 의 `Foldable<T>` 는 `FoldWhile` / `FoldBackWhile` 두 개를 `static abstract` 로 두고, `FoldRight` / `FoldLeft` 는 그 위에서 default 로 파생됩니다.
...
v5 가 FoldWhile + predicate 를 가장 일반적 abstract 로 택한 동기는 함수형 정통의 여섯 자리에서 만납니다.

1. **lazy 자동 종료의 strict 대응** — Haskell 의 `foldr` 는 lazy 평가로 predicate 만족 시 자동 종료 합니다. 그것을 strict C# 에서 명시 predicate 로 옮긴 자리입니다.
2. **control flow as value** — `break` 같은 문법적 종료가 predicate 함수 값으로 표현됩니다. 합성 가능한 종료입니다.
3. **최소 abstract 원칙** — 한 abstract 가 모든 fold 변형을 흡수합니다 (`FoldLeft = FoldWhile (predicate ≡ true)`). 구현자가 `FoldWhile` 한 개만 정의하면 30+ helper 가 default 로 따라옵니다.
4. **short-circuit 통일** — `any`, `all`, `find`, `takeWhile` 이 모두 FoldWhile + 종료 조건의 특수 케이스로 자연스럽게 자라납니다.
5. **curried step** — `Func<A, Func<S, S>>` 의 curried 형태가 Haskell `a -> b -> b` 와 1:1 매핑됩니다. 부분 적용과 함수 합성이 자연스럽습니다.
6. **실무 효율** — 큰 컬렉션이나 무한 시퀀스에서 조건부 종료가 abstract 수준에서 보장됩니다.
...
이 책의 abstract 두 개는 재귀 방향이 fold 의 본질이라는 어휘를 1부에서 정착시키기 위한 선택입니다. early termination + 30+ default 파생은 실무 v5 코드에서 자연스럽게 만나는 layer 입니다. 한 abstract 의 일반화가 효율을 abstract 수준으로 끌어올린 자리이지, fold 의 본질을 바꾼 것은 아닙니다.
...
| Filter / Where | Filterable | 경계 언급 (§6.8) |
...
> **Filterable 의 자리** — `Filter(p) : E<a> → E<a>` 는 컨테이너 종류를 보존하면서 (Functor 처럼) 술어를 통과한 원소만 남겨 개수를 줄입니다. 원소를 순회해 (Foldable 처럼) 통과분만 같은 컨테이너로 다시 모으는, Foldable 과 Functor 가 결합된 자리라 둘 중 하나만으로는 표현되지 않습니다. 9장 Traversable 이 서로 다른 두 효과를 합치는 결합이라면, Filterable 은 같은 컨테이너 안에서 거르는 결합으로 구분됩니다.
...
>
> Foldable 의 약속은 컨테이너를 소비 합니다. 결과 타입에 F 가 없는 것이 그 형식화 (§6.2.1). `Sum`, `Count` 의 결과는 Normal 의 한 값이고, 그 결과로 원래 컨테이너를 다시 만들 수 없습니다. Foldable 은 단방향 끌어내림입니다. 컨테이너 위에서 다시 변환이 필요하면 Functor (4장), 양방향 이동이 필요하면 Traversable (9장) 으로 갑니다.
...
함수형 trait 의 우아함은 각 trait 가 좁은 능력만 약속 하는 데 있습니다. Foldable 의 약속은 컨테이너 소비 + 한 값 누적 한 줄입니다. 좁은 약속 → 강한 보장 → 추론 단순화. 그 좁은 약속이 수십 개 자유 함수의 토대가 됩니다.
## 6.9 직접 해보기 — 예제 (챌린지)
...
> **본문 어느 자리의 이해도를 묻는가**
>
> 1. §6.3·§6.4 의 3-tuple 패턴을 재귀 자료에 그대로 적용할 수 있는가.
> 2. 분기 처리 (§6.4.3, Maybe 의 Just/Nothing case 분기) 를 Tree 의 Leaf/Branch case 로 자연스럽게 이행할 수 있는가.
> 3. 일관성 법칙 (§6.6) 이 자기가 작성한 `FoldRight`/`FoldLeft` 에서 성립함을 코드로 검증할 수 있는가.
>
> **해보기**
>
...
>
> 정답 코드: `code/Part1-Foundations/Ch06-Foldable/Challenges/TreeFoldable.cs`.
...
> **본문 어느 자리의 이해도를 묻는가**
>
> 1. 시그니처 분류표 (§6.8.1) 를 새 메서드들에 직접 적용할 수 있는가.
> 2. §6.2.1 의 `Fold` 시그니처 `(Func, seed, K<F, A>) → B` 와 비교해 어떤 자리가 어긋나는가 짚을 수 있는가.
> 3. 좁은 약속 (§6.8.4, 컨테이너 소비 + 한 값 누적) 의 의미를 각 후보에 적용해 판정 할 수 있는가.
>
> **판단 기준 (본문 §6.8.4 + §6.2.1)**
>
> 1. 시그니처가 `E<a> → b` 형태입니까? (입력에 E, 출력에 E 없음)
> 2. 컨테이너의 모양이 소비 되습니까? (결과에 컨테이너가 다시 나타나지 않음)
> 3. step 함수로 누적 가능한가? (한 원소씩 차례로 결합)
>
> 세 기준에 모두 예 면 Foldable 의 `Fold`. 하나라도 아니오 면 어느 trait 의 자리 인지 분류표 (§6.8.2) 를 참고.
>
> **해보기**
>
...
> **검증 포인트**
>
> - 오답 한 개는 본문의 한 자리를 다시 읽어야 한다는 신호입니다 — Reverse 가 막힌다면 §6.8.1 의 Filter / Take 행, Lookup 이 막힌다면 §6.8 의 `a → E<b>` 유형 / 7장 §7.2 미리보기로 갑니다.
>
> 정답: `code/Part1-Foundations/Ch06-Foldable/Challenges/FoldableClassify.md`.
...
- **`Const<C, _>` Foldable** — 두 번째 타입 매개변수 `A` 가 phantom (시그니처에만 등장, 실제 값 없음). `FoldRight` / `FoldLeft` 의 step 은 호출되지 않습니다. step 호출 0회여도 일관성 법칙이 자동 성립 하는 흥미로운 경계 사례입니다. Applicative 효과 분리 (Const 패턴)의 출발점입니다. 정답: `Challenges/ConstFoldable.cs`.
...
## 6.10 Elevated World 어휘로 다시 읽기 — 마무리

이 장에서 익힌 도구를 1장의 4 가지 함수 유형과 elevated-world 시리즈의 흐름에 다시 매핑합니다.

- `Foldable<F>` trait 는 `E<a> → b` 유형 함수의 일반화입니다 (1장 4 가지 함수 유형의 `E<a> → b`). 함수형의 본질, 곧 합성 가능한 Elevated World 로 lift (§1.6.1) 의 반대 방향 (lower / 끌어내림) 이 등장하는 자리.
- `fold` 는 elevated-world 글의 Lowering 정통입니다. Elevated World 의 구조를 Normal World 의 한 값으로 끌어내립니다. 4장 `map` (lift / 끌어올림) 의 짝입니다.
- 두 법칙 (결과 일관성·부수 효과 없음) 이 두 abstract 의 정합성 약속을 구현 사이의 공통성으로 못 박습니다. 셋째 축 합성 가능성 (§1.6.1) 의 lowering 판본입니다.
...
이 다섯 줄이 6장이 1부 안에서 차지하는 자리의 요약입니다. lift 와 lower 가 두 평행 세계의 두 방향 사다리라는 그림이 코드에 박히는 자리입니다.

---

## 6.11 Q&A — 마무리 (자주 묻는 질문)
...
그렇습니다. `IEnumerable<T>.Aggregate(seed, f)` 가 List Foldable 의 FoldLeft 의 인스턴스 구현입니다. C# 표준에는 오른쪽 접기의 친숙한 이름이 없는데, Foldable trait 은 두 방향 (`FoldRight` + `FoldLeft`) 을 대칭으로 제공합니다. 확장 어법 (§6.2.5, `fa.FoldLeft(f, seed)`) 이 `xs.Aggregate(seed, f)` 와 같은 자리이고, `K<F, A>` 위 점 호출입니다.

> Q3. 세 가지 호출 어법 (trait 정적 / 모듈 / 확장) 중 언제 어느 것을 쓰나?

셋 모두 같은 `Foldable<F>.FoldRight` / `FoldLeft` 로 디스패치되어 결과는 동일합니다. 자리에 따른 선호:
- **trait 정적 호출** (`MyListF.FoldLeft<int, int>(f, 0, nums)`) — 학습·명세 코드. F 가 무엇인지 한눈에 보임. 첫 줄 (§6.3.4, §6.4.4).
- **모듈 어법** (`Foldable.foldLeft<F, A, B>(f, 0, fa)`) — generic 함수 본체. F 가 타입 매개변수일 때 자연 (`SumAnyFoldable.Run`, §6.5).
...
Foldable 의 step 은 순수 함수 여야 합니다. 부수 효과가 필요하면 그 부수 효과를 캡슐화하는 Monad (Try, IO, Reader, 5부 / 3부) 의 자리입니다.
...
4장 Functor 가 `E<a> → E<b>` 유형의 일반화라면, 이 장 Foldable 은 `E<a> → b` 유형의 일반화입니다. 4 가지 함수 유형의 두 인접 자리를 두 trait 이 채웁니다. 두 trait 의 합 (9장 Traversable) 이 복합 변환의 정통 자리입니다. 1부의 기초 두 정통이 나란히 자리잡는 이유입니다.

또한 Foldable 의 abstract 2 → virtual 다수 ROI 는 독자에게 trait 패턴의 가장 큰 가치를 보여 주는 결정적 예입니다. 자유 함수 카탈로그가 그 증명을 제공합니다.
...
셋 다 예 면 Foldable 가능합니다. 하나라도 아니오 면 Functor / Filterable / Monad / Traversable 등 다른 trait 의 자리입니다. 분류표는 §6.8 에 있고, 다섯 후보에 직접 적용해 보는 자리가 두 번째 챌린지입니다.

---

## 6.12 다음 장으로 — 마무리 (7장 Monad 다리)
...
다음 장 (7장) — Monad:    a → E<b> 의 합성 되살리기            bind / Kleisli >=>
```

6장까지 두 평행 세계 사이의 두 방향 사다리 (lift / lower) 가 완성되었습니다. 7장 Monad 는 그 사다리 위에서 또 다른 자리를 답합니다. 출력 타입만 Elevated 인 함수 (`a → E<b>`) 끼리는 직접 합성이 안 됩니다 (어법 어긋남 자리, 1장 §1.7.4). `bind` 가 이 World-crossing 함수의 합성을 되살립니다. [7장 — Monad](./Ch07-Monad.md) 로 넘어갑니다.

> **실무 디딤돌** — Foldable 의 `Fold` 는 컨테이너와 효과를 Normal World 의 값으로 끌어내리는 보편 도구입니다. 8 부 Streaming 의 `Conduit` / `Producer` 가 Foldable 의 fold 어법을 효과적 스트림으로 일반화합니다.
>
> **테스트 디딤돌** — 6 장의 Foldable 법칙 (결과 일관성·부수 효과 없음) 은 9 부의 property-based 테스트로 자연 확장됩니다. 임의의 `List<int>` 와 가환·결합 step 에 대해 FoldRight 와 FoldLeft 결과가 일치하는지 자동 검증하는 것이 출발점입니다.

> §6 → §7 다리. 6장의 *fold* 가 *Elevated → Normal* 의 lower 였다면, 7장의 *bind* 는 `a → E<b>` 같은 어법이 어긋난 함수의 합성을 Elevated 안에서 회복 하는 자리입니다. 합성 어긋남 (1장 §1.7.4) 이 7장 *Monad* 의 `bind` 한 메서드로 해소 됩니다.