...
> **이 장의 핵심 어휘** — 지금 외우는 목록이 아니라, 본문에서 하나씩 천천히 도입할 예고편입니다. 앞의 네 개가 본문의 도구이고, 뒤의 네 개는 한계의 정식 이름으로 깊이 보기에서 정리합니다.
>
...
> - **type constructor**: 타입을 받아 타입을 만드는 어휘. 자료가 안 채워진 `List`, `Option` 자체
> - **Order**: 값 또는 타입이 받는 인자의 단계. Order 0 은 값, Order 1 은 함수, Order 2 는 함수를 받는 고차 어휘
> - **kind**: 타입에도 분류 단계가 있다는 어휘 — 값을 분류하면 타입, 타입을 분류하면 kind
> - **Higher-Kinded Type (HKT)**: type constructor 자체를 매개변수로 받는 추상. C# 이 직접 지원하지 않는 자리

> 이 장을 마치면 할 수 있게 되는 것
> - [ ] 같은 `Map` 을 컨테이너마다 베끼는 N×M 비용을 시그니처로 보여줄 수 있습니다.
> - [ ] trait 다형성이 그 비용을 N+M 으로 줄이는 발상을 설명할 수 있습니다.
> - [ ] 왜 C# 제네릭만으로는 `Functor<F>` 의 `F<A>` 자리를 직접 쓸 수 없는지 두 에러 메시지로 답할 수 있습니다.
> - [ ] `K<F, A>` 의 `F` 와 `A` 가 각각 무엇을 뜻하는지 설명할 수 있습니다.
> - [ ] self-bound 제약과 static abstract 멤버가 각각 왜 필요한지 한 줄로 답할 수 있습니다.
...
2장은 1장과 마찬가지로 도구를 나열하는 장이 아닙니다. 먼저 **`F` 자리가 없으면 어떤 불편이 생기는지** 를 직접 체험하고, 그 불편을 C# 으로 풀려다 **두 에러에 부딪힌 뒤**, 막힌 자리를 우회하는 세 도구를 곧장 손에 쥐고, 실제 컨테이너 등록과 일반 함수로 가치를 확인합니다. 한계의 정식 이름 (Order / kind) 은 깊이 보기로 미뤄 두었습니다. 문제 → 막힘 → 우회 → 실전의 순서입니다.
...
`MyList<A>` 라는 컨테이너를 만들고 `Map` 능력을 부착합니다. 안의 값을 변환하되 컨테이너 모양은 그대로 두는 능력입니다. 1 장에서 본 `(a → b) → (E<a> → E<b>)` 끌어올림을, 컨테이너 `MyList` 의 C# 메서드로 적은 것입니다.
...
격자의 어느 칸 하나가 **미묘하게 다르게** 구현되어도 컴파일러는 알아채지 못합니다. 예를 들어 누군가 `MyOption.Map` 만 "`None` 에서 예외를 던지도록" 짜 놓고 떠나면, `MyList.Map` 과 `MyOption.Map` 이 "같은 Map 이어야 한다" 는 약속이 깨집니다. 그러나 그 약속은 **각 메서드의 본문에만** 존재할 뿐, 코드의 구조에는 적혀 있지 않습니다. 1장에서 `items` 와 `total` 의 약속이 절차의 본문에만 묻혀 있던 것과 정확히 같은 종류의 문제입니다. **같은 능력임을 코드가 어디에서도 강제하지 않습니다.**

![N×M 비용에서 N+M 비용으로](./images/Ch02-Higher-Kinds/02-nxm-vs-nplusm.svg)
...
가치는 `Functor` 한 trait 에 그치지 않습니다. 같은 발상이 기초의 추상 전체에서 반복됩니다.

| 현재 C# (앞 절의 격자) | `F` 자리가 가능하다면 |
|---|---|
| `List.Map` / `Option.Map` / `Task.Map` 각각 구현 | `Functor<F>` 의 `Map` 한 번 |
...
여기서 자연스러운 질문은 하나입니다. **막힌 자리를 어떻게 되찾는가.** 이 책은 한계의 타입 이론부터 풀지 않습니다. 막힌 자리를 직접 되찾는 세 도구 (`K<F, A>` 마커, self-bound, static abstract) 를 곧장 손에 쥡니다. 이 막힘의 정식 이름 (Order / kind / HKT) 은 도구를 모두 익힌 뒤 깊이 보기에 정리해 두었습니다. 이름을 몰라도 도구는 쓸 수 있습니다.
...
빈 인터페이스입니다. 멤버가 없습니다. 그런데 이 한 줄이 앞서 막힌 함수형 추상의 모든 가능성을 엽니다.

`K<F, A>` 는 마음속에서 `F<A>` 로 읽으면 됩니다. `F` 라는 컨테이너 안에 `A` 가 든 모양을, C# 이 직접 못 쓰는 `F<A>` 대신 완성 타입으로 인코딩한 신호입니다.

### 2.4.2 발상 — 막힌 자리를 한 단계 내려 되찾기

발상은 단순합니다. C# 이 `T<X>` 형태 (타입을 받아 타입을 만드는 type constructor) 를 직접 매개변수로 받지 못한다면, **자료가 다 채워진 완성 타입의 대체 신호를 만들어 거기 매개변수로 받자.**
...
C# 이 막힌 핵심은 type constructor (`List` 라는 컨테이너 종류 자체) 를 타입 인자로 전달 못 함입니다 (앞서 본 두 에러). 해결은 `F` 자리를 **완성 타입처럼** 사용하는 것입니다. 빈 태그 타입 (`MyListF`) 을 만들어 `List` 의 이름표 역할만 시킵니다.

```csharp
public sealed class MyListF { }   // 빈 태그 — 멤버 0개, 자료 자리가 없는 완성 타입
...
#### `K<F, A>` — 거부되는 자리를 받아들여지는 어법으로

```csharp
// K 가 받는 것 — F, A 모두 완성 타입
//   K               : 완성 타입 두 개를 받는 평범한 2인자 제네릭
//   K<MyListF, int> : 둘 다 채워진 완성 타입 — 변수에 담고 매개변수로 전달 가능

// 원하던 것 vs 우회 — 같은 "F 안에 A" 를 다른 자리에서 표현
//   F<A>     ← F 가 type constructor 매개변수   — C# 이 거부하는 자리 (2.3절)
//   K<F, A>  ← F 가 완성 타입 (빈 태그) 매개변수 — C# 이 받아들이는 어법
```

`F<A>` 는 `F` 가 `A` 를 감싸는 모양 (`F` 가 type constructor) 이고, `K<F, A>` 는 `K` 가 `F` 와 `A` 를 나란히 받는 모양 (`F` 가 완성 타입 태그) 입니다. 같은 "F 안에 A 가 있다" 를, C# 이 거부하는 자리에서 받아들이는 어법으로 **한 단계 낮춰** 인코딩한 것이 이 우회의 핵심입니다.

정리하면 이렇습니다. C# 이 컨테이너 종류 자체를 매개변수로 못 받으니, **빈 태그 (완성 타입) 로 그 종류를 가리켜** 막힌 자리를 되찾았습니다. 이 우회의 정식 이름 (kind 한 단계 내림) 과 다른 언어들의 같은 우회 패턴은 깊이 보기에서 봅니다.
...
`F` 가 **자기 자신을 타입 인자로 받는** 제약입니다. 이 한 줄이 "F 자리에 들어갈 타입은 `Functor` 의 구현체" 임을 컴파일러에 보장합니다. 즉 `F` 의 자리에 함수 호출이 가능한 어휘가 생깁니다. 구현은 `sealed class MyListF : Functor<MyListF> { … }` 형태로 강제됩니다.
...
>
> 둘은 **능력이 사는 자리** 가 다릅니다 (1장). `fa.Map(f)` 는 능력이 **값(객체)** 에 삽니다 — 1장의 객체 다형성. `MyListF.Map(f, fa)` 는 능력이 **타입(trait)** 에 삽니다 — 1장의 trait 다형성. 후자라야 앞서 본 "`F` 자리에 컨테이너 이름만 바꿔 끼움" 이 가능합니다. 값에는 이름을 "바꿔 끼울" 수 없지만, 타입 이름은 제네릭 인자로 바꿔 끼울 수 있기 때문입니다.
...
**그림 2-3. 함수형 trait 의 핵심인 두 도구** — 코드 한 블록에 두 도구 (첫 번째 `where F : Functor<F>` self-bound 제약 (주황), 두 번째 `static abstract` (초록)) 가 동시에 자리잡습니다. 두 도구가 합쳐져 `F` 라는 어휘 자체가 Functor 의 동작을 수행할 수 있는 자리가 됩니다. C# 의 제네릭만으로는 만들 수 없는 higher-kinded generic 을 두 도구가 가능하게 만듭니다.

---

## 2.6 가상 코드를 `K<F, A>` 로 재작성 — 최종 돌파

가상 코드의 가치와 막힘을 봤고, 세 우회 도구 (`K<in F, A>` 마커, self-bound, static abstract) 를 손에 쥐었습니다. 이제 컴파일되지 않던 가상의 한 줄이 **실제 빌드되는 한 줄** 로 어떻게 변환되는지 직접 비교합니다. 아래 가상 코드의 `F<_>` 는 "자료 자리가 빈 `F`" 를 뜻하는 표기입니다.

```csharp
// 2.3절 가상 — F 가 type constructor 매개변수 (C# 불가)
public interface Functor<F<_>>
{
    static abstract F<B> Map<A, B>(Func<A, B> f, F<A> fa);
}

// 2.6절 실제 — K<F, A> 마커 + self-bound + static abstract 로 변환 (컴파일 가능)
public interface Functor<F> where F : Functor<F>
{
    static abstract K<F, B> Map<A, B>(Func<A, B> f, K<F, A> fa);
}
...
| 자리 | 가상 | 실제 | 도구 |
|---|---|---|---|
| F 자리 | `F<_>` (type constructor 매개변수) | `F` (완성 타입 매개변수 — 빈 태그) | K 마커 |
| 컨테이너 어휘 | `F<A>`, `F<B>` | `K<F, A>`, `K<F, B>` | K 마커 |
| F 가 trait 구현체임을 보장 | (가상에서 암묵) | `where F : Functor<F>` | self-bound |
| 호출 입구 | `Functor<MyList>.Map(...)` | `MyListF.Map(...)` | static abstract |
...
> **최종 돌파** — 두 에러로 막혔던 한 줄이, 이제 **IDE 에서 실제로 빌드되는 한 줄** 이 됐습니다. 격자가 실제로 얼마나 줄어드는지는 일반 함수에서 코드로 확인합니다.

> **약간의 비용** — 변환에는 두 비용이 따릅니다. (a) 3-tuple 패턴 (자료 / 태그 / trait 의 세 조각) (b) 다운캐스트 한 줄 (`(MyList<A>)fa` 또는 LanguageExt 의 `.As()`, 정체는 뒤에서 봅니다). 대가로 Haskell / Scala 의 거의 모든 함수형 추상이 C# 어법으로 표현 가능해집니다.

두 에러로 막혔던 자리가 세 도구로 돌파됐습니다. 이제 이 한 줄을 실제 코드에 적용합니다.
...
앞서 우리는 `MyList<A>` 의 인스턴스 메서드 `Map` 으로 출발했습니다. 그 어법이 N×M 격자를 낳았습니다. 이제 같은 `MyList` 를 **세 우회 도구로 다시 써서** 격자를 없앱니다. 그 과정에서 인스턴스 메서드 어법의 세 약점 (모양 보존이 시그니처에 박히지 않음 / 체이닝 시 `F` 가 사라짐 / 어떤 `F` 든 받는 일반 함수 불가) 이 차례로 해소됩니다.

### 2.8.1 Before — `K<F, A>` 우회 없이

세 도구 없이 평범한 제네릭 인터페이스로 같은 시도를 하면 어떻게 되는지 먼저 봅니다. 1장식 어법 (능력을 인터페이스로 약속하되 `A` 만 매개변수로 둠) 으로 되돌린 `SimpleFunctor<A>` 입니다. 방금 만든 `Functor<F>` 와 달리 `F` 를 시그니처에서 가리킬 수단이 없습니다.

```csharp
public interface SimpleFunctor<A>
{
    SimpleFunctor<B> Map<B>(Func<A, B> f);
}

public sealed class MyList<A> : SimpleFunctor<A>
{
    private readonly List<A> _items;
    public MyList(IEnumerable<A> items) { _items = items.ToList(); }

    public SimpleFunctor<B> Map<B>(Func<A, B> f) =>
        new MyList<B>(_items.Select(f));
}
```
...
같은 `MyList` 를 `K<F, A>` 마커와 self-bound + static abstract 로 다시 씁니다. 아래 네 조각은 앞서 따로 본 도구를 한자리에 모은 것이라, 새 문법은 없습니다.
...
| 측면 | 제네릭 `SimpleFunctor<A>` 어법 (Before) | `K<F, A>` + 3-tuple (After) |
|---|---|---|
| F 자리 | 시그니처에서 사라짐 | `K<F, A>` 의 첫 매개변수로 부활 |
| 능력 호스트 | 자료 클래스 (`MyList<A>`) 의 인스턴스 메서드 | 태그 클래스 (`MyListF`) 의 정적 메서드 |
| 모양 보존 보장 | 시그니처에 없음 (`SimpleFunctor<B>` 만 보장) | `K<F, B>` ↔ `K<F, A>` 의 F 동일성으로 박힘 |
...
**그림 2-4. 3-tuple 패턴 (자료 / 태그 / trait 의 세 조각)** — 태그 타입 (`MyListF`) 이 trait (`Functor<F>`) 을 구현하고, 자료 타입 (`MyList<A>`) 은 `K<MyListF, A>` 의 옷을 입어 태그의 정적 메서드 (`Map`) 안에서 다운캐스트로 본 모습을 드러냅니다. 세 조각이 느슨하게 결합되어 책임이 분리됩니다.
...
값 `xs` 의 인스턴스 메서드가 아니라 `MyListF` 라는 어휘가 `Map` 을 부릅니다. `xs` 는 인자로 전달될 뿐입니다. 능력이 객체가 아니라 *trait* (태그 타입에 부착된 자리) 에 사는 함수형의 모양이 그대로 드러납니다.

> **한 줄 정리** — 호출 입구가 값이 아니라 **태그 타입의 정적 메서드 이름** 이라, `F` 라는 어휘가 능력을 부르는 자리가 됩니다.

### 2.8.5 세 약점이 어떻게 해결되는가

*Before* (제네릭 `SimpleFunctor<A>` 어법) 가 보였던 세 약점이 *After* (`K<F, A>` + 3-tuple) 에서 차례로 해소됩니다.

| 약점 | Before — 제네릭 `SimpleFunctor<A>` | After — `K<F, A>` + 3-tuple |
|---|---|---|
| **모양 보존** | `SimpleFunctor<B> Map<B>(...)` — 결과가 그저 "어떤 Functor". List 였는지 Option 이었는지 시그니처가 모름. | `static K<F, B> Map<A, B>(..., K<F, A> fa)` — 입력과 출력의 F 가 같은 F (컴파일러가 검증). 잘못된 구현 거부. |
| **체이닝 시 F 추적** | `SimpleFunctor<string> a = xs.Map(...)` → List 정보 손실, `.Items` 접근 불가. | `K<MyListF, string> a = MyListF.Map(...)` → F = MyListF 가 끝까지 살아 있어 마지막에 다운캐스트로 `Items` 접근. |
...
`F` 가 어떤 Elevated World 인지 함수는 몰라도, self-bound 제약 한 줄이 한 번 정의로 모든 Functor 인스턴스에 통하게 만듭니다. 이 패턴의 전체 사용 예는 뒤에서 봅니다.
...
![3-tuple 패턴](./images/Ch02-Higher-Kinds/04-K-marker.svg)

**그림 2-5. `K<F, A>` (빈 인터페이스가 만드는 어휘)** — 자료 (`MyOption<a>` / `MyList<a>` / `MyResult<a>`) → 태그 (`MyOptionF` / `MyListF` / `MyResultF`) → 공통 어휘 (`K<F, a>`) 의 3-tuple 을 세 자료 타입에 나란히 보입니다. 세 조각이 모두 갖춰져야 각 자료가 이 책의 Elevated World 어법 안에서 시민이 됩니다.
...
같은 함수 한 줄이 세 가지 다른 Elevated World 위에서 그대로 동작합니다. **추상을 한 번 정의하면 N 개 인스턴스가 자동으로 따라오는**, trait 의 진짜 이득이 여기서 드러납니다. 처음의 N×M 격자가 N+M 으로 실제 줄어드는 자리입니다.

이 일반 함수의 모양이 기초 3 ~ 9장의 모든 함수형 도구의 공통 골격입니다. Functor → Applicative → Monad 의 추상 사다리를 따라 함수 시그니처가 같은 모양으로 발전합니다.

> **3-tuple 의 이득** — 새 컨테이너를 만들 때 세 조각만 정의하면 모든 일반 함수가 자동으로 동작합니다. `Sum`, `Count`, `All`, `Any` 같은 자유 함수를 다시 짤 필요가 없습니다. 6장 Foldable 에서 이 이득을 직접 봅니다.

---

## 2.10 Elevated World 어휘로 다시 읽기

이 장은 N×M 격자의 불편에서 출발해 일반 함수 (N+M) 로 끝났습니다. 그 여정에서 익힌 도구를 1장의 두 평행 세계 / 4 가지 함수 유형에 다시 매핑하면, 2장이 1장 비유의 **C# 구현체** 였음이 드러납니다.

| 1장의 비유 | 2장이 준 C# 어휘 |
|---|---|
| Elevated World 의 시민 (`Option<int>`, `List<a>` ...) | `K<F, A>` — 어떤 Elevated World 인지 무관하게 "F 안에 A 가 든 자료" 라는 한 어법 |
| 능력은 trait 에 산다 | `Functor<F>`, `Monad<F>` — trait 의 static abstract 자리 |
| 4 가지 함수 유형 | `Functor` = `E<a> → E<b>` 의 일반화, `Monad` = `a → E<b>` 를 `E<a> → E<b>` 로 끌어올림 |
| trait 부착 | 3-tuple 패턴 — 각 Elevated World 가 공통 어휘의 시민이 되는 입회 절차 |
| 능력이 컴파일 타임에 해소 | self-bound + static abstract — 어떤 World 에 적용될지 컴파일러가 검증 |
...
> **선택적 심화** — 이 절은 앞서 본 막힘에 정식 이름 (Order / kind / HKT) 을 붙입니다. 세 도구를 쓰는 데 필수는 아니므로, 건너뛰고 Q&A 로 가도 4장 이후를 읽는 데 지장이 없습니다. "왜 하필 그 자리가 막히는가" 의 뿌리가 궁금할 때 돌아오면 됩니다.

### 2.11.1 값 차원의 Order 단계 (Order 0 / 1 / 2)

값을 만드는 어휘를 통틀어 **값 생성자** 라 부르겠습니다[^vc]. 리터럴 (`1`, `"hello"`) 도, 함수 (`int → int`) 도, 고차 함수도 모두 값을 만든다는 점에서 같은 묶음입니다. 이 절은 그 어휘들을 *Order* 라는 단계로 분류하고, 다음 절의 타입 생성자 (type constructor) 가 같은 분류를 타입 차원에서 반복합니다.

[^vc]: 학술 어휘의 *value constructor* (*data constructor*) 는 Haskell 대수적 데이터 타입에서 값을 만드는 함수입니다. `data Either a b = Left a | Right b` 의 `Left`, `Right` 가 그 예이고, 0-인자면 *nullary constructor* (상수) 라 부릅니다 (Haskell Wiki, *Constructor*). 이 책은 그보다 광의로, 값 차원에서 값을 만드는 모든 어휘를 가리키는 데 씁니다.
...
> ```
>
> `Negate` 는 인자가 하나이고 값 (int) 이라 Order 1 에 머뭅니다. `Apply` 는 같은 1-인자지만 인자가 함수 (`int → int`) 라 Order 2 로 올라갑니다. `Add` 는 인자가 둘이어도 모두 값이라 여전히 Order 1 입니다. **몇 개** 가 아니라 **어떤 어휘가 인자 자리에 들어오는가** 가 *Higher* 를 가릅니다. 이 구분은 타입 차원에서 다시 결정적으로 쓰입니다 (`Dictionary<K, V>` 가 2-인자지만 HKT 가 아닌 이유는 뒤에서 봅니다).
...
여기까지가 핵심입니다. **값 차원은 세 Order 모두 자유롭습니다.** 앞서 본 한계는 값 차원이 아니라 **타입 차원** 에서 나타납니다. 같은 Order 어휘를 타입 차원으로 옮기는 순간, C# 의 비대칭이 드러납니다.
...
kind 화살표는 함수 시그니처와 같은 법으로 읽습니다. 왼쪽이 입력, 오른쪽이 출력입니다. `List` 가 `int` 를 받아 `List<int>` 를 만드니 `* → *` 이고, 그 type constructor 전체를 다시 입력으로 받는 자리가 `(* → *) → *` (HKT) 입니다. 입력 자리가 두 개인 `* → * → *` (`Dictionary<K, V>`, `Either<L, R>`) 는 인자가 모두 완성 타입이라 HKT 가 아닙니다. 앞서 본 다인자 ≠ 고차의 판별 규칙이 타입 차원에도 그대로 적용됩니다. 결국 화살표의 **왼쪽 자리** 가 Order 를 가릅니다. 왼쪽이 `*` 면 Order 1, `(* → *)` 면 Order 2 (HKT) 로 올라갑니다.

C# 의 타입 차원이 어디까지 제공하는지 한 단계씩 점검하면 비대칭이 드러납니다. Order 0 (완성 타입) 은 **완전 제공**, Order 1 (type constructor) 은 **정의만 제공**, Order 2 (HKT) 는 **미지원** 입니다. 단계별로 살펴봅니다.
...
`int` 는 T 자리가 없는 완성 타입이고, `List<int>` 는 T 자리에 `int` 가 채워진 완성 타입입니다. 둘 다 kind `*` 에 해당하고, C# 이 모든 어법에서 자유롭게 다룹니다. **여기까진 앞서 본 한계가 없습니다.**
...
함수 시그니처 차원에서도 앞서 본 그 한계가 그대로 드러납니다.
...
타입 차원의 Order 2 가 **2장의 본 목표** 에 해당합니다. type constructor 를 매개변수로 받는 추상, 즉 앞서 시도했던 `Functor<F>` 의 `F` 자리가 여기입니다. 값 차원의 함수를 받는 함수 (HOF) 가 타입 차원에 평행 매핑된 모습입니다.

결론은 한 줄입니다. **`F` 가 이미 한 단계 위 어휘 (`* → *`) 라서, 그것을 매개변수로 받는 `Functor` 는 한 단계 더 위 (`(* → *) → *`) 로 올라갑니다.** 아래 세 단계는 그 한 줄의 형식 확인이므로, 결론만 들고 통합 비교로 건너뛰어도 됩니다.

앞서 본 가상 코드 `static F<B> Map<A, B>(Func<A, B> f, F<A> fa)` 의 각 자리가 어떤 kind 인지 세 단계로 따라갑니다.
...
`F` 의 kind 는 단순 `*` 가 아니라 **이미 한 단계 위 어휘 (`* → *`)** 입니다. 그것을 매개변수로 받는 `Functor` 의 kind 는 입력 자리를 괄호로 묶어 **`(* → *) → *`** 가 되고, 이 모양이 바로 **Order 2 (HKT, Higher-Kinded Type)** 입니다.
...
세 단계의 결론은 한 줄입니다. `Functor<F>` 의 kind `(* → *) → *` 가 **2 장의 핵심 목표** (Order 2, HKT) 자리이고, **C# 어법에는 바로 이 자리가 없습니다**. 앞서 본 두 에러가 정확히 이 자리에서 났습니다.

### 2.11.6 값 생성자 vs 타입 생성자 — 통합 비교

앞의 세 단계가 형식적 도출이라면, 이제 그 결론을 값 차원과 한 자리에서 통합 비교합니다. **type constructor 자체가 타입 차원의 함수** 입니다. kind 표기 `* → *` 의 `→` 가 함수 시그니처의 `→` 와 같은 어법 (왼쪽 입력, 오른쪽 출력) 이라, type constructor 도 타입을 받아 타입을 만드는 함수로 동작합니다. 즉 값 차원의 함수와 타입 차원의 type constructor 는 **같은 함수 어법의 두 차원** 에 놓입니다.
...
- **함수형 추상이 살고 싶은 곳은 타입 차원의 Order 2** — `Functor` 의 `F` 자리가 바로 **2장의 본 목표** 입니다. C# 으로 직접 못 다루는 이유는 Order 1 에서 type constructor 자체가 일급이 아니라서 한 단계 위 (Order 2) 도달이 어법적으로 막혀 있기 때문입니다.
...
![Order 단계의 값 차원/타입 차원 평행](./images/Ch02-Higher-Kinds/05-order-parallel.svg)

**그림 2-6. Order 단계로 본 Higher: 값 차원과 타입 차원의 평행** — 아래 행 값 차원 (Normal World 색) 에 Order 0 → Order 1 → Order 2 (HOF) 세 박스. 위 행 타입 차원 (Elevated World 색) 에 같은 Order 0 → Order 1 → Order 2 (HKT) 세 박스. 가운데 점선 매핑이 같은 Order 끼리 평행을 시각화합니다.

### 2.11.7 C# 의 한계 — 두 에러 메시지와 다른 언어

비대칭의 결론이 한 자리에 모입니다. **C# 의 핵심 제약은 제네릭 매개변수가 완성 타입 (Order 0) 만 받고, type constructor (Order 1) 자체를 매개변수로 받는 추상 (Order 2, HKT) 을 문법으로 표현할 수 없다는 것입니다.** 앞서 마주친 두 에러 메시지가 그 결론을 컴파일러의 입으로 직접 확정합니다.

```csharp
// 2.3절의 시도를 다시 — T<X> 형태 (Higher Kinds) 를 C# 으로 직접 표현
public interface Functor<F>
{
...
우회의 비용은 학습 곡선 (3-tuple 패턴 + 다운캐스트 + self-bound 제약) 입니다. 대가로 Haskell / Scala 의 거의 모든 함수형 추상이 C# 에서 표현 가능해집니다. 이 우회의 형식 이론적 기반도 논문으로 정립되어 있습니다[^moors08].
...
> **Q1. 객체지향의 N×M 비용과 함수형의 N+M 비용 차이는?** (2.2절)

객체지향에서 능력은 객체 (컨테이너 클래스) 의 인스턴스 메서드입니다. `List<A>.Map`, `Option<A>.Map`, `Task<A>.Map` 셋이 모양만 같고 자리가 달라 N 개 컨테이너 × M 개 능력 = N×M 개의 별개 구현이 필요합니다 (2.2절의 격자). 함수형은 능력이 사는 자리를 *trait* 으로 옮깁니다. `Functor<F>` 한 trait 에 `Map` 한 번 정의하고 `F` 자리에 컨테이너를 끼우면 N+M 비용으로 줄어듭니다.

> **Q2. C# 으로 `interface Functor<F>` 를 적고 `F<B> Map(...)` 을 쓰면 왜 막힙니까? 이 한계의 정식 이름은?** (2.3절)

**CS0307** (`F` 가 다시 인자를 받는 자리에서 사용 불가) 과 **CS0246** (`F` 를 완성 타입으로 해석하려 했으나 못 찾음) 두 에러가 납니다. 뿌리는 하나입니다. C# 의 제네릭 매개변수는 완성 타입만 받아, `F<...>` 처럼 `F` 가 다시 인자를 받는 어법이 없습니다. 이 한계의 정식 이름은 Order 2 (Higher-Kinded Type) 미지원, 학술 어휘로 *higher-kinded polymorphism* 미지원이며, 그 도출은 깊이 보기 (2.11절)에 정리했습니다.

> **Q3. `K<F, A>` 의 멤버 수와 발상은?** (2.4절)

0 개. 빈 인터페이스입니다. C# 이 거부하는 `F<A>` 자리를, 빈 태그 타입을 첫 매개변수로 받는 완성 타입 `K<F, A>` 로 끌어내려 인코딩하는 **type-level 신호** 입니다. 동작이 아니라 신호를 담는 그릇이라 멤버가 필요 없습니다. 동작은 trait 의 static abstract 에 따로 삽니다.

> **Q4. `where F : Functor<F>` (self-bound) 가 없으면 무엇이 안 되나? 순환 정의 아닌가?** (2.5절)
...
> **Q5. `static abstract` 호출이 인스턴스 호출과 다른 점은?** (2.5절)

값이 아니라 타입 이름이 호출 입구입니다. `MyListF.Map(f, xs)` 처럼 *trait* 에 사는 능력을 직접 부릅니다. 인스턴스 메서드 (`xs.Map(f)`) 가 능력이 **객체** 에 사는 모형이라면, static abstract 는 능력이 **trait** 에 사는 모형입니다 (1장 1.5절). 후자라야 F 자리에 컨테이너 이름을 바꿔 끼우는 일반화가 가능합니다.

> **Q6. 3-tuple 패턴의 세 조각은? `K<F, A>` 는 왜 4 번째 조각이 아닌가?** (2.8절)
...
- **불편에서 출발했습니다.** 같은 `Map` 을 컨테이너마다 베끼는 N×M 격자가, 1장 명령형의 약점 (약속이 코드 구조에 강제되지 않음) 과 같은 종류의 문제를 낳았습니다 (2.2절). 2장 전체는 그 격자를 없애는 도구 모음입니다.
- **C# 으로 직접 풀려다 두 에러에 부딪혔습니다.** `interface Functor<F>` 의 `F<A>` / `F<B>` 자리에서 CS0307, CS0246 이 났습니다 (2.3절). 제네릭 매개변수가 완성 타입만 받는 어법의 막힘입니다.
- **막힌 자리를 세 도구로 우회했습니다.** ① `K<in F, A>` 빈 인터페이스가 C# 이 거부하는 `F<A>` 자리를 완성 타입 어법으로 끌어내려 되찾고, ② self-bound (`where F : Functor<F>`) 가 F 자리에 함수 호출을 가능하게 하고, ③ static abstract (C# 11+) 가 호출 입구를 타입 이름에 둡니다 (2.4절 ~ 2.5절). 세 도구는 Lightweight Higher-Kinded Polymorphism (Yallop & White, 2014) 의 brand 인코딩의 C# 표현입니다.
- **가상 한 줄이 실제 한 줄로 변환됐습니다.** F 자리 / 컨테이너 어휘 / 호출 입구 세 곳이 일대일 매핑되어, 2.2절의 N×M 비용이 실제 컴파일 가능한 N+M 비용으로 돌파됩니다 (2.6절).
- **새 컨테이너의 입회 절차는 3-tuple 패턴** (자료 / 태그 / trait 의 세 조각) 입니다 (2.8절). 한 번 등록되면 어떤 Functor 든 받는 일반 함수 (`FunctorOps.Run`) 가 그 컨테이너에 자동 적용됩니다 (2.9절).
- **한계의 정식 이름은 깊이 보기에 정리했습니다.** 값 차원의 Order 0/1/2 가 타입 차원의 kind (`*` / `* → *` / `(* → *) → *`) 에 평행이고, C# 은 타입 차원의 Order 2 (HKT) 만 지원하지 않습니다. 2.3절 두 에러의 정체입니다 (2.11절).
...
4장부터 그 추상들이 하나씩 무대 위에 올라갑니다. 가장 단순한 추상 (`E<a> → E<b>` 유형 함수의 일반화) 인 Functor / map 부터 [4장](../Part02-CoreTraits/Ch04-Functor.md) 에서 만납니다.
