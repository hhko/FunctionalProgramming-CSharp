...
>
> **이 장의 주제**. 1 장의 두 평행 세계 비유 (Normal / Elevated World) 를 C# 코드로 표현하는 도구를 만듭니다. 같은 `Map` 을 컨테이너마다 베끼는 N×M 고통에서 출발해, 그 고통을 한 번에 풀려다 부딪히는 C# 컴파일러의 벽을 값 차원의 *Order* 어휘로 짚습니다. 그 벽은 type constructor 를 매개변수로 못 받는 Order 2 (HKT) 미지원입니다. 이 한계를 `K<in F, A>` 마커와 self-bound, static abstract 세 도구로 우회해, 한 능력을 한 번 정의하면 모든 Elevated World 위에서 자동 동작하는 자리 (N×M → N+M) 를 C# 어법 안에 실현합니다. 4 장 이후 모든 trait 이 올라설 무대 장치입니다.
...
이 목표의 실체는 1장에서 본 함수형의 본질, 즉 **합성 가능한 Elevated World 로 lift** 의 둘째 축 *type class 다형성* 입니다. 그 어휘를 컴파일 타임에 강제하는 도구가 2장의 주제입니다.

2장은 1장과 마찬가지로 도구를 나열하는 장이 아닙니다. 먼저 **`F` 자리가 없으면 어떤 고통이 생기는지** 를 직접 체험하고 (§2.2), 그 고통을 C# 으로 풀려다 **벽에 부딪힌 뒤** (§2.3), 그 벽이 정확히 무엇인지 어휘로 정착시키고 (§2.4 ~ §2.5), 벽을 우회하는 세 도구를 차례로 놓습니다 (§2.7 ~ §2.9). 고통 → 벽 → 우회의 순서입니다.

---

## 2.2 고통의 체험 — 같은 Map 을 N 번 베끼는 코드

### 2.2.1 첫 컨테이너 — 잘 동작하는 Map

`MyList<A>` 라는 컨테이너를 만들고 `Map` 능력을 부착합니다. 안의 값을 변환하되 컨테이너 모양은 그대로 두는 능력입니다 (1장 §1.7.6 의 `E<a> → E<b>` 자리).
...
public sealed class MyList<A>
{
    ...
    public MyList<B> Map<B>(Func<A, B> f) =>      // ← List 의 Map
...
격자의 어느 칸 하나가 **미묘하게 다르게** 구현되어도 컴파일러는 알아채지 못합니다. 예를 들어 누군가 `MyOption.Map` 만 "`None` 에서 예외를 던지도록" 짜 놓고 떠나면, `MyList.Map` 과 `MyOption.Map` 이 "같은 Map 이어야 한다" 는 약속이 깨집니다. 그러나 그 약속은 **각 메서드의 본문에만** 존재할 뿐, 코드의 구조에는 적혀 있지 않습니다. 1장 §1.2.3 에서 `items` 와 `total` 의 약속이 절차의 본문에만 묻혀 있던 것과 정확히 같은 종류의 고통입니다. **같은 능력이라는 사실이 코드 어디에도 강제되지 않습니다.**
...
1. **`F<...>` 가 정확히 왜 막히는가?** — 이 벽의 이름과 깊이를 알아야 합니다 (§2.4 ~ §2.5).
2. **벽을 어떻게 우회하는가?** — 막힌 자리를 되찾는 도구가 필요합니다 (§2.7 ~ §2.9).

먼저 첫 질문입니다. 벽의 정체를 이해하려면 *Higher* 라는 어휘의 직감을 **값 차원** 에서 먼저 잡고, 같은 어휘를 **타입 차원** 으로 평행 매핑해야 합니다. 그 과정에서 C# 의 핵심 제약이 또렷이 드러납니다.
...
여기까지가 핵심입니다. **값 차원은 세 Order 모두 자유롭습니다.** §2.3 의 벽은 값 차원이 아니라 **타입 차원** 에서 나타납니다. 같은 Order 어휘를 타입 차원으로 옮기는 순간, C# 의 비대칭이 드러납니다.
...
> | 타입 생성자 | (값 없음) | `List` | `* → *` |
>
> 읽는 방향은 왼쪽 → 오른쪽입니다. 한 칸 오른쪽으로 갈 때마다 "그 왼쪽 칸을 분류하는 어휘" 가 됩니다. `5` 를 분류하면 `int`, `int` 를 분류하면 `*`. 주의할 자리는 셋째 칸입니다. `int` 도 `List<int>` 도 자료가 채워진 **완성 타입** 이라 kind 가 똑같이 `*` 이고, T 자리가 빈 **type constructor `List`** 라야 kind 가 한 단계 위 `* → *` 가 됩니다 (`int → int` 같은 함수 타입도 완성 타입이라 kind 는 `*` 입니다 — `int → int` ∥ `* → *` 는 분류 단계가 아니라 §2.5.4 의 *Order 평행*).
>
...
`int` 는 T 자리가 없는 완성 타입이고, `List<int>` 는 T 자리에 `int` 가 채워진 완성 타입입니다. 둘 다 kind `*` 에 해당하고, C# 이 모든 어법에서 자유롭게 다룹니다. **여기까진 §2.3 의 벽이 없습니다.**
...
함수 시그니처 차원에서도 같은 한계가 그대로 드러납니다 — 이게 §2.3 에서 본 그 벽입니다.
...
§2.3 의 가상 코드 (`static F<B> Map<A, B>(Func<A, B> f, F<A> fa)`) 의 각 자리가 어떤 kind 인지 세 단계로 따라갑니다.
...
세 단계의 결론은 한 줄입니다. `Functor<F>` 의 kind `(* → *) → *` 가 **2 장의 핵심 목표** (Order 2, HKT) 자리이고, **이것이 C# 어법에 없는 자리** 입니다. §2.3 의 두 에러가 정확히 이 자리에서 났습니다.
...
두 에러 메시지의 정확한 의미는 다음과 같습니다. **CS0307** 은 "`F` 가 다시 인자를 받는 자리에서 사용 불가", **CS0246** 은 "`F` 를 완성 타입으로 해석하려 했으나 찾을 수 없다" 는 뜻입니다. 두 에러의 뿌리 원인은 하나입니다 — Order 1 에서 type constructor 자체가 일급 시민이 아니라서, `Functor<F>` 의 `F` 자리에 type constructor 가정이 어법으로 불가능하고, 그 결과 `F<...>` 어법 모두가 거부됩니다. 학술 어휘로는 *higher-kinded polymorphism* 미지원입니다.
...
벽의 정체를 확정했습니다 — Order 2 미지원. 그렇다면 **만약 C# 이 직접 지원했다면 어떤 가치가 있었을까요?** 가상의 C# 어법으로 그 가치를 먼저 확인하고, 그 가치를 §2.7 부터 실제 도구로 얻습니다.

C# 이 **Generic 매개변수 자체가 다시 generic** 일 수 있다면 (즉 Higher Kinds 직접 지원), §2.3 의 가상 `Functor<F>` 가 그대로 컴파일됩니다. 그러면 다음 호출이 가능합니다.
...
그래서 함수형의 핵심 가치가 **`Map` 함수 한 번 정의 → N 개 컨테이너 자동 적용** 입니다. 새 컨테이너 (예: `MyEither`) 가 등장해도 `Map` 을 다시 짤 필요가 없습니다. `F` 자리에 `MyEither` 만 넣어주면 됩니다. 이게 함수형의 둘째 축 *type class 다형성* 으로, `Map` 같은 능력이 컨테이너마다 따로가 아니라 trait (`Functor`) 하나에 한 번 자리잡는 형태입니다.
...
trait 한 번 정의 → 모든 컨테이너 자동 적용 이라는 발상이 1부의 추상 사다리 전체에서 반복됩니다. 즉 HKT 의 가치는 한 trait 의 편리가 아니라 **함수형 어휘 전체의 통합** 입니다.
...
> **`in F` 의 함수형 의미** — `F` 자리에 붙은 `in` (contravariant) 표기의 직감은 단순합니다. brand `F` 는 컨테이너 종류를 분류하는 type-level dispatch 어휘이고, **분류는 입력받는 자리** 라 contravariant 가 자연입니다. 지금은 **반공변** 의 직감만 가져가도 충분합니다. 더 결정적인 효과 (trait variance 반전) 는 자연 변환 (11장) 에서 풀이합니다.
...
`F` 자리의 가치를 가상 코드로 봤고 (§2.6), 세 우회 도구 (`K<in F, A>` 마커, self-bound, static abstract) 도 봤습니다 (§2.7 ~ §2.8). 이제 §2.3 에서 컴파일되지 않던 가상의 한 줄이 **실제 빌드되는 한 줄** 로 어떻게 변환되는지 직접 비교합니다.
...
> **약간의 비용** — 변환에는 두 비용이 따릅니다. (a) 3-tuple 패턴 (자료 / 태그 / trait 의 세 조각, §2.11) (b) 다운캐스트 한 줄 (`(MyList<A>)fa` 또는 LanguageExt 의 `.As()`). 대가는 Haskell / Scala 의 거의 모든 함수형 추상이 C# 어법으로 표현 가능해진다는 점입니다.

§2.3 에서 두 에러로 막혔던 자리가 세 도구로 돌파됐습니다. 이제 이 한 줄을 실제 코드에 적용합니다.

---

## 2.10 모든 trait 가 따르는 공통 모양

실제 적용으로 들어가기 전에, 한 가지 안심거리를 짚습니다. 1부의 trait 들 (`Functor`, `Foldable`, `Applicative`, `Monad`, `Traversable`) 은 모두 **같은 모양** 입니다.
...
§2.2 에서 우리는 `MyList<A>` 의 인스턴스 메서드 `Map` 으로 출발했습니다. 그 어법이 N×M 격자를 낳았습니다. 이제 같은 `MyList` 를 **세 우회 도구로 다시 써서** 격자를 없앱니다. 그 과정에서 인스턴스 메서드 어법의 세 약점 (모양 보존이 시그니처에 박히지 않음 / 체이닝 시 `F` 가 사라짐 / 어떤 `F` 든 받는 일반 함수 불가) 이 차례로 해소됩니다.

### 2.11.1 Before — `K<F, A>` 우회 없이

제네릭 `Functor<A>` 어법의 `MyList<A> : Functor<A>` 를 먼저 봅니다. `A` 만 매개변수로 둔 어법으로, `F` 가 시그니처에서 가리킬 수단이 없습니다.
...
같은 `MyList` 를 `K<F, A>` 마커와 self-bound + static abstract 로 다시 씁니다.
...
**그림 2-6. `K<F, A>` (빈 인터페이스가 만드는 어휘)** — 자료 (`MyOption<a>`) → 태그 (`MyOptionF`) → 공통 어휘 (`K<MyOptionF, a>`) 의 3-tuple. 이 셋이 모두 갖춰져야 `MyOption` 이 이 책의 Elevated World 어법 안에서 시민이 됩니다.
...
이 장은 §2.2 의 고통 (N×M 격자) 에서 출발해 §2.12 의 일반 함수 (N+M) 로 끝났습니다. 그 여정에서 익힌 도구를 1장의 두 평행 세계 / 4 가지 함수 유형에 다시 매핑하면, 2장이 1장 비유의 **C# 구현체** 였다는 사실이 드러납니다.
...
핵심을 한 줄로 누르면 이렇습니다. **1장의 그림 (두 평행 세계, trait 에 사는 능력) 을 C# 컴파일러가 실제로 검증하는 코드로 바꾼 것이 2장의 세 도구 (`K<F, A>`, self-bound, static abstract) 다.** 이 네 줄이 1부의 문법 기반이고, 4장부터 그 위에 진짜 추상들이 올라갑니다.
...
> Q6. C# 으로 `interface Functor<F>` 를 적고 `F<B> Map(...)` 을 쓰면 어떤 두 에러가 나나?
...
> Q8. 가상의 `Functor<F<_>>` 한 줄이 실제 한 줄로 어떻게 변환됩니까?
...
> Q10. 3-tuple 패턴의 세 조각은? `K<F, A>` 는 왜 4 번째 조각이 아닌가?
...
> Q11. `where F : Functor<F>` (self-bound) 가 없으면 무엇이 안 되나? 순환 정의 아닌가?
...
> Q12. `static abstract` 호출이 인스턴스 호출과 다른 점은?
...
> Q13. 어떤 Functor 든 받는 일반 함수 한 예는?
...
- **고통에서 출발했습니다.** 같은 `Map` 을 컨테이너마다 베끼는 N×M 격자가, 1장 명령형의 약점 (약속이 코드 구조에 강제되지 않음) 과 같은 종류의 고통을 낳았습니다 (§2.2). 2장 전체는 그 격자를 없애는 도구 모음입니다.
- **C# 으로 직접 풀려다 벽에 부딪혔습니다.** `interface Functor<F>` 의 `F<A>` / `F<B>` 자리에서 두 에러 (CS0307, CS0246) 가 났습니다 (§2.3). 그 벽의 정체는 **Order 2 (HKT) 미지원** 입니다.
- **벽을 어휘로 이해했습니다.** 값 차원의 Order 0/1/2 어휘가 타입 차원의 kind 어휘 (`*` / `* → *` / `(* → *) → *`) 에 평행입니다. 값 차원의 Order 2 가 고차 함수 (HOF) 이듯, 타입 차원의 Order 2 가 고차 타입 (HKT) 입니다. C# 의 비대칭은 값 차원 일급 시민 완비 vs 타입 차원 Order 1 부터 미완 (§2.4 ~ §2.5).
- **벽을 세 도구로 우회했습니다.** ① `K<in F, A>` 빈 인터페이스가 `F<A>` 의 Order 2 자리를 Order 0 어휘로 내려 되찾고, ② self-bound (`where F : Functor<F>`) 가 F 자리에 함수 호출을 가능하게 하고, ③ static abstract (C# 11+) 가 호출 입구를 타입 이름에 둡니다 (§2.7 ~ §2.8). 세 도구는 Lightweight Higher-Kinded Polymorphism (Yallop & White, 2014) 의 brand 인코딩의 C# 표현입니다.
