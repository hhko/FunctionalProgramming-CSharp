...
> **이 장의 주제**. 1장의 비유 (Normal / Elevated) 를 C# 코드로 표현하는 도구입니다. 객체지향에서 부딪히는 N×M 비용 문제로 시작해, 값 차원의 *Order* 단계 어휘로 *Higher* 의 직감을 잡고 같은 어휘를 타입 차원에 평행 매핑한 뒤, `K<in F, A>` 마커, self-bound, static abstract 두 우회 도구로 가상 한 줄을 실제 한 줄로 회수합니다. 언어가 직접 표현 못 하는 함수형 추상을 C# 안에서 가능하게 만드는 두 도구입니다.
...
객체지향에서 능력의 주체는 객체입니다. `Map`, `Bind`, `Fold` 같은 능력은 각 컨테이너 클래스의 인스턴스 메서드로 부착됩니다. 시그니처를 직접 보면 같은 능력 (`Map`) 이 컨테이너별로 세 번 별개로 구현되는 자리가 드러납니다.
...
세 시그니처가 모양만 같고 자리 (이름) 가 다른 어법입니다. `List<A>.Map` / `Option<A>.Map` / `Task<A>.Map` 이 세 별개 메서드라 컴파일러가 같은 능력으로 다루지 못합니다. 결과로 N 개 컨테이너에 M 개 능력 (`Map`, `Bind`, `Fold` ...) 을 부착하려면 **N×M 개의 구현** 이 필요합니다.
...
여기서는 이 정도 직감으로 충분합니다. 어휘 (Order / kind / type constructor 일급) 의 정착은 다음 두 절에서 단계적으로, 두 에러 메시지의 정밀 분석은 그 뒤 결정적 제약 자리에서 다룹니다. 이 한계가 결정적 제약이고, 한계를 우회하는 도구가 2장의 주제입니다.
...
세 단계의 제공 상태가 정확히 완전 / 부분 / 미지원의 비대칭을 그립니다. 단계별로 봅니다.
...
> - **`(* → *) → *`** — 왼쪽 자리에 type constructor 전체 (`* → *`) 가 들어와 오른쪽 `*` 를 만드는 HKT. 괄호가 왼쪽 `* → *` 가 한 자리임을 명시합니다. 값 차원의 `(int → int) → int` (함수를 받아 값을 만드는 고차 함수) 와 같은 어법으로, **2장의 본 목표 자리** 입니다.
...
타입 차원의 Order 0 은 완성 타입으로, 모든 자리가 채워진 구체적 타입입니다. kind 어휘로는 `*` 자리이고, 변수에 담거나 매개변수로 전달하거나 반환하는 데 어떤 제약도 없습니다.
...
C# 어법은 type constructor 의 정의는 제공합니다. `public class Box<T> { ... }` 같은 제네릭 정의가 그 자리입니다.
...
F<B> DoubleIt<F, A, B>(F<A> fa) { ... }     // ✗ F<A>, F<B> 두 자리 모두 CS0307 거부
//   ─┬─                  ─┬─
//   F<B>, F<A>: F 가 다시 인자를 받는 어법 — C# 어법에 없음
```

`DoubleIt<F, A, B>` 의 의도는 F 가 어떤 type constructor 든 받아 `F<A>` 를 `F<B>` 로 변환하는 것입니다. 즉 F 자체를 매개변수로 받아 그 안에 인자를 끼우는 자리, Order 2 의 자리 (kind `(* → *) → *`) 에 해당합니다. C# 어법에 그 자리가 없어 `F<A>` / `F<B>` 두 자리에서 CS0307 에러가 발생합니다. 이 어법이 사실은 §2.5.3 의 `Functor<F>` 인터페이스가 막히는 자리와 같은 패턴입니다. Order 1 의 type constructor 일급 미지원이 Order 2 의 미지원으로 이어지는 자리입니다.

C# 의 정의만 제공이 의미하는 자리가 바로 이것입니다. `Box<T>` 같은 제네릭 정의는 가능하지만, T 자리가 채워진 완성 타입 (`Box<int>`) 만 일급 시민이고, T 자리가 비어있는 type constructor 자체 (`Box`) 는 일급 시민이 아닙니다. 함수 시그니처 / interface 어법 어디서든 F 가 type constructor 매개변수라는 가정 자체가 어법으로 막혀 있습니다.
...
§2.3 의 함수형 발상 (N+M 비용 회복) 이 F 자리에 어떤 컨테이너든 끼움으로 달성되려면, F 가 type constructor 매개변수 자리에 들어와야 합니다. 가상 코드로 보면 다음과 같은 자리입니다.
...
위 가상 코드의 각 자리가 어떤 kind 의 자리인지 단계별로 따라가 보면, 왜 `Functor<F>` 자체가 Order 2 (HKT) 인지 직감이 잡힙니다.

#### 1 단계 — `F` 의 kind 가 왜 `* → *` 인가
...
핵심은 `F` 의 kind 가 단순 `*` 가 아니라 이미 한 단계 위 어휘 (`* → *`) 라서, 그것을 매개변수로 받는 `Functor` 의 kind 가 **`(* → *) → *`** 가 된다는 점입니다. 입력 자리가 합성 어휘라 괄호로 묶고, 출력 `*` 와 합성한 결과입니다. 이게 **Order 2 (HKT, Higher-Kinded Type)** 의 자리입니다.
...
| `F` (매개변수) | `* → *` | 1 |
| `F<A>`, `F<B>`, `Func<A, B>` | `*` | 0 |
| **`Functor<F>` 자체** | **`(* → *) → *`** | **2 (HKT) — 2장의 본 목표 자리** |
...
즉 `Functor<F>` 가 타입 차원의 한 단계 위 자리입니다. `F` (타입 차원의 함수 = type constructor) 를 매개변수로 받는 추상이라 `(* → *) → *` Order 2 (HKT) 입니다. 값 차원의 HOF 가 함수를 받는 함수인 자리와 어법은 같고 대상 (값 vs 타입) 만 다릅니다.

**컴파일 에러가 발생하는 정확한 자리** — C# 컴파일러는 제네릭 매개변수 `F` 를 완성 타입 (Order 0, kind `*`) 만 받습니다. 위 코드의 `F<B>` / `F<A>` 두 자리는 F 가 다시 인자를 받는 어법으로, F 가 type constructor (kind `* → *`, Order 1) 여야 가능한 자리입니다. 두 자리에서 컴파일러가 같은 에러를 냅니다.

- **CS0307** — *static `F<B>` Map<A, B>(Func<A, B> f, `F<A>` fa)* 의 두 `F<...>` 자리에서 발생. F 가 type parameter 인데 이 자리에서는 유효하지 않다는 뜻으로, 컴파일러는 `F` 를 완성 타입으로만 알고 있어 `F<...>` 어법을 F 가 type constructor 라는 가정의 표현으로 받아들이지 못합니다.

뿌리 원인은 §2.5.2 에서 본 자리입니다. Order 1 에서 type constructor 자체가 일급 시민이 아니라서, `Functor<F>` 의 F 자리에 type constructor 가정이 어법으로 불가능합니다. 그 결과 F 가 다시 인자를 받는 `F<...>` 어법 모두가 거부되고, 한 단계 위인 Order 2 (type constructor 를 매개변수로 받는 자리) 도 도달 불가능합니다.
...
Haskell 의 `f` 자리에 `[]` / `Maybe` / `IO` 같은 type constructor 자체가 직접 들어가고, 컴파일러가 kind `* → *` 인지 검증합니다. C# 어법에는 그 자리가 없습니다.
...
- **함수형 추상이 살고 싶은 자리는 타입 차원의 Order 2** — `Functor` 의 `f` 자리가 바로 그 자리이고, 이게 **2장의 본 목표** 입니다. C# 으로 직접 못 다루는 이유는 Order 1 에서 type constructor 자체가 일급이 아니라서 한 단계 위 (Order 2) 도달이 어법적으로 막혀 있기 때문입니다.

이 한계가 뒤이어 두 에러 메시지로 다시 검증되고, 한계를 우회하는 도구가 `K<F, A>` 마커 + self-bound + static abstract 입니다.
...
앞에서 Order 2 자리의 직접 표현이 막힌다는 사실을 두 번 봤습니다 (어법 분석 + 두 에러 메시지). 그렇다면 만약 직접 지원했다면 어떤 가치가 있을까요? 가상의 C# 어법으로 그 가치를 먼저 확인하고, 뒤이어 실제 도구로 어떻게 회수되는지의 동기로 삼습니다.
...
해결은 F 를 완성 타입으로 강등하는 것입니다. 빈 태그 타입 (`MyListF`) 을 만들어 List 의 이름표 역할만 시킵니다.
...
조각 수가 늘었지만 각 조각의 책임이 분명해집니다. 이게 3-tuple 패턴입니다.
...
세 조각이 느슨하게 결합되어 있습니다. 한 조각의 변경이 다른 조각에 최소한의 영향만 미칩니다. K 마커 인터페이스는 이 세 조각을 묶는 신호 어휘일뿐, 4 번째 조각이 아닙니다.
...
### 2.13.5 다운캐스트와 같은 패턴 — 보일러플레이트와 일반화

`(MyList<A>)fa` 의 다운캐스트가 어색해 보일 수 있습니다. `K<F, A>` 가 빈 인터페이스라 컴파일러는 "F = MyListF 면 자료 타입이 `MyList<A>`" 라는 연결을 모르고, 이 연결을 3-tuple 의 명시적 다운캐스트로 사람이 알려줍니다. LanguageExt 라이브러리는 확장 메서드 `.As()` 로 다운캐스트를 감춥니다 (`var items = fa.As().Items;`).
...
안의 타입 `A` 는 추적되지만 F 자체 (List / Option / Result) 가 시그니처에서 사라집니다. 모양 보존이 약속되지 않고, 체이닝 시 컨테이너 정보가 점점 모호해지며, 어떤 F 든 받는 일반 함수를 쓸 수 없습니다. 세 약점 모두가 §2.13.4 ~ §2.13.5 에서 해결됩니다.
