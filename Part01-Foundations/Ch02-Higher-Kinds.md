...
`F<A>` 는 `F` 가 `A` 를 감싸는 모양 (`F` 가 type constructor) 이고, `K<F, A>` 는 `K` 가 `F` 와 `A` 를 나란히 받는 모양 (`F` 가 완성 타입 태그) 입니다. 같은 "F 안에 A 가 있다" 를, C# 이 거부하는 자리에서 받아들이는 어법으로 **한 단계 끌어내려** 인코딩한 것이 이 우회의 핵심입니다.

정리하면 이렇습니다. C# 이 컨테이너 종류 자체를 매개변수로 못 받으니, **빈 태그 (완성 타입) 로 그 종류를 가리켜** 막힌 자리를 되찾았습니다. 이 끌어내림의 정식 이름 (kind 한 단계 내림) 과 다른 언어들의 같은 우회 패턴은 깊이 보기에서 봅니다.
