...
>
> **이 장의 흐름**. 1장의 비유 (Normal / Elevated) 를 C# 코드로 표현하는 도구입니다. 객체지향에서 부딪히는 N×M 비용 문제로 동기를 잡고 (동기), 값 차원의 *Order* 단계 어휘로 *Higher* 의 직감을 정착시킨 뒤 같은 어휘를 타입 차원에 평행 매핑해 C# 의 핵심 제약이 무엇인지 짚습니다 (정체). 그다음 `K<in F, A>` 마커와 self-bound, static abstract 우회 도구로 가상 한 줄을 실제 한 줄로 실현하고 (우회), `MyList` 예제와 일반 함수로 적용한 뒤 (적용), Elevated World 어휘로 다시 읽으며 마무리합니다 (정리).
...
비용 절감의 핵심은 F 라는 매개변수 어휘에 있습니다. 어떤 컨테이너든 받는 매개변수 자리, 즉 type constructor 를 매개변수로 받는 Order 2 의 자리입니다.

문제는 이 자리가 C# 어법에 직접 없다는 것입니다. 위의 가상 `Functor<F>` 코드를 그대로 IDE 에 적고 빌드하면 컴파일러가 `F<B>` / `F<A>` 자리에서 두 에러 (CS0307, CS0246) 를 냅니다. 비유 한 줄로 정리하면, C# 의 제네릭 자리는 **완성된 우편함** (자료가 채워진 형태) 만 받고, **우편함을 만드는 공장** (`List` 같은 type constructor 자체) 을 매개변수로 받는 어법이 없습니다. 그래서 `F<...>` 처럼 F 자체가 다시 매개변수를 받는 자리가 거부됩니다.

여기서는 이 정도 직감으로 충분합니다. 어휘 (Order / kind / type constructor 일급) 의 정착은 다음 두 절에서 단계적으로, 두 에러 메시지의 정밀 분석은 그 뒤 핵심 제약 절에서 한 번에 다룹니다. 이 한계가 핵심 제약이고, 한계를 우회하는 도구가 2장의 주제입니다.
...
| Order 1 | `int → int` (값 → 값) | 값 | 평범한 함수 — 값을 받아 값을 반환 | `int DoubleIt(int)`, `string Xxx(int)`, `Func<int, double>` |
| Order 2 | `(int → int) → int` (함수 → 값) | 함수 | 고차 함수 (HOF) — 함수를 받아 값을 반환 | `int DoubleIt(Func<string, int>)`, `xs.Aggregate(seed, f)` |
...
인자 개수는 Order 를 결정하지 않습니다. 다음 표는 인자 개수가 같아도 인자 자리의 어휘가 Order 를 가르는 자리를 보여줍니다.
...
판별 규칙은 다음과 같습니다. 인자 중 하나라도 함수 (`Func<...>` / 메서드 / 람다) 가 있으면 Order 2 이상이고, 모두 값이면 Order 1 입니다. `Add` 는 2 인자지만 모두 값이라 Order 1, `Apply` 는 1 인자지만 함수라 Order 2 인 자리입니다.
...
## 2.5 타입 차원의 kind 와 C# 의 벽

앞 절에서 값 차원의 Order 0 / 1 / 2 어휘를 봤습니다. 같은 어휘를 타입 차원으로 평행 매핑하면 *kind* (타입의 타입) 라는 어휘가 등장합니다. 값 차원의 Order 0 / 1 / 2 (`int` / `int → int` / `(int → int) → int`) 가 타입 차원의 kind 표기 `*` / `* → *` / `(* → *) → *` 에 그대로 대응합니다. 두 차원을 값 생성자 / 타입 생성자로 나란히 놓은 통합표는 §2.5.4 에 한 번에 모읍니다.

화살표 어법 (`→` 의 입력 / 출력) 은 두 차원에서 정확히 평행하지만 **이름은 분리** 됩니다. 값 차원의 `int → int` 는 *type signature*, 타입 차원의 `* → *` 는 *kind* 입니다. kind 는 type-level 어휘로 값에는 적용되지 않습니다 (Wikipedia *Kind (type theory)*). 그래서 앞 절 표가 값 차원의 정통 표기 (`int`, `int → int`) 를 쓰고, 이 절부터 kind 표기 (`*`, `* → *`) 가 등장합니다.
...
> - **`* → * → *`** — 입력 자리가 두 개인 다인자 type constructor. 왼쪽 `*` 와 가운데 `*` 두 자리에 완성 타입을 채워 오른쪽 `*` 를 만듭니다. 예: `Dictionary<K, V>`, `Either<L, R>`. 인자가 모두 완성 타입이라 HKT 가 아닙니다 (다인자 ≠ 고차, 앞 절과 같은 판별 규칙).
>
...
`DoubleIt<F, A, B>` 의 의도는 F 가 어떤 type constructor 든 받아 `F<A>` 를 `F<B>` 로 변환하는 것입니다. 즉 F 자체를 매개변수로 받아 그 안에 인자를 끼우는 자리, Order 2 의 자리 (kind `(* → *) → *`) 에 해당합니다. C# 어법에 그 자리가 없어 `F<A>` / `F<B>` 두 자리가 거부됩니다. Order 1 의 type constructor 일급 미지원이 Order 2 의 미지원으로 이어집니다.
...
앞 절의 함수형 발상 (N+M 비용 절감) 이 F 자리에 어떤 컨테이너든 끼움으로 달성되려면, F 가 type constructor 매개변수 자리에 들어와야 합니다. 가상 코드로 보면 다음과 같습니다.
...
먼저 `F` 가 무엇인지 정합니다. `Functor` 는 **컨테이너** (`List`, `Option`, `Task` 같은 어휘) 에 대한 추상이므로, `F` 자리에는 그 컨테이너들이 들어옵니다. 컨테이너 한 예제 (`List`) 로 모양을 봅니다.
...
다른 컨테이너도 같은 모양입니다. `Option`, `Task` 모두 자료 1 개를 받아 완성 타입을 만드는 어법, 즉 모두 kind `* → *`. 따라서 `Functor` 가 받기로 한 자리 (`F`) 의 kind 가 **`* → *`** 입니다. 시그니처 자체가 이 가정을 요구합니다. 위 코드의 `F<A>` / `F<B>` 어법은 `F` 가 type constructor (kind `* → *`) 일 때만 의미가 있고, 만약 `F` 의 kind 가 단순 `*` (완성 타입) 였다면 `F<A>` / `F<B>` 어법 자체가 불가능합니다.
...
앞의 세 단계 도출이 형식적 논증이라면, 이제 그 결론을 값 차원과 한 자리에서 통합 비교합니다. 핵심은 **type constructor 자체가 타입 차원의 함수** 라는 점입니다. kind 표기의 `* → *` 의 `→` 가 함수 시그니처의 `→` 와 같은 어법 (왼쪽 입력, 오른쪽 출력) 이라, type constructor 도 타입을 받아 타입을 만드는 함수입니다. 즉 값 차원의 함수와 타입 차원의 type constructor 는 같은 함수 어법의 두 차원입니다.
...
| **Order 2** | `(* → *) → *` | 고차 함수 (HOF) — `Apply` / 고차 생성자 — `new User(Func<int,int>)` | HKT — `Functor<F>` 의 `F` 자리 (**2장의 본 목표**) | 값 차원 ✓ / 타입 차원 ✗ **미지원** |

세 줄로 정리하면 다음과 같습니다.

- **Order 어휘는 두 차원에 동일** — `*` / `* → *` / `(* → *) → *` 세 단계가 값 차원과 타입 차원에 똑같이 적용됩니다. 값 차원의 고차 함수 (HOF, 함수를 받는 함수) 가 타입 차원의 고차 타입 (HKT, type constructor 를 받는 타입) 으로 평행 매핑됩니다. 같은 어법이고 대상 (값 vs 타입) 만 다릅니다.
...
### 2.5.5 C# 의 벽 — 두 에러 메시지와 다른 언어

비대칭의 결론은 한 자리에 모입니다. C# 의 핵심 제약은 제네릭 매개변수가 완성 타입 (Order 0) 만 받고, type constructor (Order 1) 자체를 매개변수로 받는 추상 (Order 2, HKT) 을 문법으로 표현할 수 없다는 것입니다. 이 절이 그 결론을 컴파일러가 직접 뱉는 두 에러 메시지로 한 번에 확정하고, 다른 언어들이 같은 한계를 어떻게 푸는지 정리합니다.

Order 2 가상 코드 (§2.5.3) 를 그대로 다시 보면, C# 컴파일러가 정확히 어떤 에러로 거부하는지 두 메시지로 드러납니다.
...
두 에러 메시지의 정확한 의미는 다음과 같습니다. **CS0307** 은 "F 가 다시 인자를 받는 자리에서 사용 불가" 라는 뜻이고, **CS0246** 은 "F 를 완성 타입으로 해석하려 했으나 찾을 수 없다" 는 뜻입니다. 두 에러 모두 C# 의 제네릭 매개변수가 완성 타입 (Order 0) 만 받는 어법에서 비롯됩니다. 뿌리 원인은 하나입니다. Order 1 에서 type constructor 자체가 일급 시민이 아니라서, `Functor<F>` 의 F 자리에 type constructor 가정이 어법으로 불가능하고, 그 결과 F 가 다시 인자를 받는 `F<...>` 어법 모두가 거부됩니다. 학술 어휘로는 *higher-kinded polymorphism* 미지원입니다.
...
Haskell 의 `f` 자리에 `[]` / `Maybe` / `IO` 같은 type constructor 자체가 직접 들어가고, 컴파일러가 kind `* → *` 인지 검증합니다. 다른 언어들의 풀이를 한 표로 정리하면 다음과 같습니다.
...
C# 의 핵심 제약을 확정했습니다. 그렇다면 만약 직접 지원했다면 어떤 가치가 있었을까요? 가상의 C# 어법으로 그 가치를 먼저 확인하고, 뒤이어 실제 도구로 어떻게 같은 가치를 얻는지의 동기로 삼습니다.

C# 이 **Generic 매개변수 자체가 다시 generic** 일 수 있다면 (즉 Higher Kinds 직접 지원) 앞서 본 가상 `Functor<F>` 인터페이스 (`static abstract F<B> Map<A, B>(Func<A, B> f, F<A> fa)`, §2.5.5) 가 그대로 컴파일됩니다. 그러면 다음 같은 호출이 가능합니다.
...
### 2.7.2 발상 — 빈 인터페이스가 신호 역할 + 충분한 이유
...
C# 이 막힌 핵심은 type constructor (`List`) 를 타입 인자로 전달 못 함입니다 (§2.5.5 의 미지원). 해결은 F 자리를 완성 타입처럼 사용하는 것입니다 (kind 한 단계 내림). 빈 태그 타입 (`MyListF`) 을 만들어 List 의 이름표 역할만 시킵니다.
...
## 2.8 두 번째 우회 — self-bound 제약과 static abstract 멤버
...
### 2.8.1 첫 번째 도구 — self-bound (`where F : Functor<F>`)
...
### 2.8.2 두 번째 도구 — static abstract (타입의 정적 멤버로서의 동작)
...
```csharp
// §2.6 가상 — F 가 type constructor 매개변수 (C# 불가)
public interface Functor<F<_>>
{
    static abstract F<B> Map<A, B>(Func<A, B> f, F<A> fa);
}

// §2.9 실제 — K<F, A> 마커 + self-bound + static abstract 로 변환
public interface Functor<F> where F : Functor<F>
{
    static abstract K<F, B> Map<A, B>(Func<A, B> f, K<F, A> fa);
}
...
> **N×M 문제의 최종 돌파 (실제)** — §2.6 의 가상 한 줄이 우회 도구 (K 마커 + self-bound + static abstract) 로 실제 컴파일 가능한 한 줄 (`static abstract K<F, B> Map<A, B>(Func<A, B>, K<F, A>)`) 로 실현됩니다. 가상에서 그쳤던 차이는 이제 IDE 에서 빌드되는 코드라는 점입니다. F 자리에 빈 태그 (`MyListF`, `MyMaybeF`, `MyTaskF`, ...) 를 바꿔 끼우면 같은 `Map` 이 모든 컨테이너 위에서 자동 동작해, 객체지향의 N×M 비용 (§2.2) 이 N+M 으로 실제 줄어듭니다.

> **약간의 비용** — 변환에는 두 가지 비용이 따릅니다. (a) 3-tuple 패턴 (자료 / 태그 / trait 의 세 조각, §2.11 에서 자세히) (b) 다운캐스트 한 줄 (`(MyList<A>)fa` 또는 LanguageExt 의 `.As()`). 가치는 Haskell / Scala 의 거의 모든 함수형 추상이 C# 어법으로 표현 가능해진다는 점입니다.
...
*Before* (제네릭 `Functor<A>` 어법, §2.11.1) 가 보였던 세 약점이 *After* (`K<F, A>` + 3-tuple) 에서 차례로 해소됩니다. 핵심을 표로 정리합니다.
...
`F` 가 어떤 Elevated World 인지 함수는 몰라도, self-bound 제약 한 줄이 한 번 정의로 모든 Functor 인스턴스에 통하게 만듭니다. 이 패턴의 전체 사용 예는 다음 절에서 봅니다.
...
| §2.11.3 네 조각 | 신호 `K<F, A>` / 자료 `MyList<A>` / 태그 `MyListF` / trait `Functor<F>` 의 책임 분리. |
...
같은 함수 한 줄이 세 가지 다른 Elevated World 위에서 그대로 동작합니다. 이게 trait 의 진짜 ROI 입니다. 추상 한 번 정의에 N 개 인스턴스가 자동으로 동작합니다. 객체지향의 N×M 비용 (§2.2) 이 N+M 비용으로 실제 동작하는 자리입니다.
...
C# 어법으로는 `List<T>` 만 가능합니다. type constructor 자체를 가리키는 어법이 없어 `K<F, A>` 우회 (§2.7) 가 필요합니다.
...
안의 타입 `A` 는 추적되지만 F 자체 (List / Option / Result) 가 시그니처에서 사라집니다. 모양 보존이 약속되지 않고, 체이닝 시 컨테이너 정보가 점점 모호해지며, 어떤 F 든 받는 일반 함수를 쓸 수 없습니다. 세 약점 모두가 §2.11.5 ~ §2.11.6 에서 해결됩니다.
