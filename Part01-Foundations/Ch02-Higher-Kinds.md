...
>
> **이 장의 흐름**. 1장의 비유 (Normal / Elevated) 를 C# 코드로 표현하는 도구입니다. 객체지향에서 부딪히는 N×M 비용 문제로 시작해, 값 차원의 *Order* 단계 어휘로 *Higher* 의 직감을 잡고 같은 어휘를 타입 차원에 평행 매핑한 뒤, `K<in F, A>` 마커와 self-bound, static abstract 우회 도구로 가상 한 줄을 실제 한 줄로 회수합니다. 언어가 직접 표현 못 하는 함수형 추상을 C# 안에서 가능하게 만드는 두 도구입니다.
...
이 목표의 실체는 함수형의 본질, 즉 합성 가능한 Elevated World 로 lift (§1.6.1) 의 둘째 축 type class 다형성입니다. 그 어휘를 컴파일 타임에 강제하는 도구가 2장의 주제입니다.
...
세 시그니처가 모양만 같고 이름이 다른 어법입니다. `List<A>.Map` / `Option<A>.Map` / `Task<A>.Map` 이 세 별개 메서드라 컴파일러가 같은 능력으로 다루지 못합니다. 결과로 N 개 컨테이너에 M 개 능력 (`Map`, `Bind`, `Fold` ...) 을 부착하려면 **N×M 개의 구현** 이 필요합니다.

비용의 결과는 두 가지 경우에서 드러납니다.
...
Haskell 의 `[]` / Scala 의 `F[_]` 가 type constructor 자체를 매개변수로 가리키는 어법입니다. C# 어법에는 그런 어법이 없습니다. 이 차이가 Order 2 (§2.5.3) 의 가능 여부를 가릅니다.
...
> **한 줄 회복** — kind `* → *` 는 한 타입 인자를 받아 완성 타입으로 가는 type constructor 이고, `F` 가 컨테이너 자체이므로 이 kind 를 가집니다.
...
> **한 줄 회복** — `F` 자리에 `A` 를 끼우면 완성 타입이 되어 kind 가 `*` 로 한 단계 내려갑니다.
...
> **한 줄 회복** — 입력 자리에 한 단계 위 어휘 (`* → *`) 가 들어오면 전체 kind 가 Order 2 로 올라갑니다.
...
**컴파일 에러가 발생하는 정확한 위치** — C# 컴파일러는 제네릭 매개변수 `F` 를 완성 타입 (Order 0, kind `*`) 만 받습니다. 위 코드의 `F<B>` / `F<A>` 두 자리는 F 가 다시 인자를 받는 어법으로, F 가 type constructor (kind `* → *`, Order 1) 여야 가능한 어법입니다. 두 자리에서 컴파일러가 같은 에러를 냅니다.
...
**C# 의 Order 단계 활용을 한 줄로 정리합니다.** C# 이 Order 1 (type constructor) 을 일급으로 못 다루니, Order 0 (완성 타입) 만으로 Order 2 의 다형성을 흉내냅니다. `F` (원래 Order 1) 자리를 빈 태그 (Order 0) 로 사용하고, `A` (Order 0) 와 나란히 `K<F, A>` (Order 0) 로 묶었습니다. **kind 한 단계 내림** 으로 어법 안의 자리를 회복한 셈입니다.
...
> **한 줄 회복** — F 가 시그니처에서 사라진 어법은 모양 보존을 시그니처에 박을 자리가 없어, 세 약점 (모양 보존 / F 추적 / 일반 함수) 이 동시에 발생합니다.
...
> **한 줄 회복** — F 자리가 `K<F, A>` 의 첫 매개변수로 부활해 모양 보존이 시그니처에 박힙니다.
...
> **한 줄 회복** — 네 조각이 서로 다른 책임을 지므로 한 조각의 변경 비용이 작습니다.
...
값 `xs` 의 인스턴스 메서드가 아니라 `MyListF` 라는 어휘가 `Map` 을 부릅니다. `xs` 는 인자로 전달될 뿐입니다. 이게 능력이 객체가 아니라 *trait* (태그 타입을 통해 부착) 에 사는 함수형의 모양입니다.

> **한 줄 회복** — 호출 입구가 값이 아니라 **태그 타입의 정적 메서드 이름** 이라, F 라는 어휘가 능력을 부르는 자리가 됩니다.
...
> **한 줄 회복** — F 가 시그니처에 박혀 끝까지 살아 있으므로 모양 보존 / F 추적 / 일반 함수 세 자리가 동시에 회복됩니다.
...
> **한 줄 회복** — 3-tuple 의 invariant (F 자리에는 단 하나의 자료 타입) 이 다운캐스트의 안전성을 보장합니다.
...
| §2.13.5 세 약점 해결 | 모양 보존 / F 추적 / 일반 함수가 한 자리에서 회복. |
...
가상 코드의 F 자리에 컨테이너 이름을 **바꿔 끼움** 이 실제 코드의 빈 태그 이름 (MyListF) 을 **바꿔 끼움** 으로 회수된 자리입니다.
