...
> **이 장의 핵심 어휘**. *Order* / *kind* / *type constructor* / *Higher-Kinded Type (HKT)* / `K<F, A>` 마커 / self-bound / static abstract / 3-tuple 패턴 (자료 / 태그 / trait).
>
> **이 장의 흐름**. 1장의 비유 (Normal / Elevated) 를 C# 코드로 표현하는 도구입니다. 객체지향에서 부딪히는 N×M 비용 문제로 시작해, 값 차원의 *Order* 단계 어휘로 *Higher* 의 직감을 잡고 같은 어휘를 타입 차원에 평행 매핑한 뒤, `K<in F, A>` 마커와 self-bound, static abstract 우회 도구로 가상 한 줄을 실제 한 줄로 실현합니다. 언어가 직접 표현 못 하는 함수형 추상을 C# 안에서 가능하게 만드는 두 도구입니다.
...
## 2.1 1장 비유로 출발 — F 자리의 목표

1장에서 Normal World 의 시민 (`int`, `string`) 과 Elevated World 의 시민 (`Option<int>`, `List<string>`) 두 가지 비유를 봤습니다. 이제 진짜 코드의 문제를 다룹니다. Elevated World 의 시민이 너무 많습니다.
...
여기서 1부의 목표가 등장합니다. 어떤 Elevated World 인지에 무관하게 `map`, `apply`, `bind` 같은 추상을 정의하고 싶습니다. 그러려면 F 라는 매개변수 자리가 필요합니다. F 는 Elevated World 의 이름입니다. `Option`, `List`, `Result` 같은 컨테이너 종류입니다.

이 목표의 실체는 함수형의 본질, 즉 합성 가능한 Elevated World 로 lift 의 둘째 축 type class 다형성입니다. 그 어휘를 컴파일 타임에 강제하는 도구가 2장의 주제입니다.

---

## 2.2 문제 배경 — 객체지향의 N×M 비용

객체지향에서 능력이 사는 자리는 객체입니다. `Map`, `Bind`, `Fold` 같은 능력은 각 컨테이너 클래스의 인스턴스 메서드로 부착됩니다. 시그니처를 직접 보면 같은 능력 (`Map`) 이 컨테이너별로 세 번 별개로 구현되는 모습이 드러납니다.
...
세 시그니처가 모양만 같고 이름이 다른 어법입니다. `List<A>.Map` / `Option<A>.Map` / `Task<A>.Map` 이 세 별개 메서드라 컴파일러가 같은 능력으로 다루지 못합니다. 그래서 N 개 컨테이너에 M 개 능력 (`Map`, `Bind`, `Fold` ...) 을 부착하려면 **N×M 개의 구현** 이 필요합니다.
...
이 N×M 비용 때문에 합성 가능한 Elevated World 로 lift 어휘가 컨테이너별로 따로 분리됩니다. 함수형 추상의 통합 가치 (한 능력이 모든 컨테이너 위에서 한 어휘로 살아야 함) 가 객체지향 어법으로는 달성되지 않습니다.

---

## 2.3 함수형의 발상 — N+M 비용 + F 자리의 필요

함수형의 발상은 능력이 사는 자리를 객체가 아니라 *trait* 으로 옮깁니다. `Functor` 라는 trait 한 곳에 `Map` 한 번을 정의하면, F 자리에 어떤 컨테이너든 끼워 같은 `Map` 이 자동 동작합니다.
...
비용 절감의 핵심은 F 라는 매개변수 어휘에 있습니다. 어떤 컨테이너든 받는 매개변수 자리, 즉 type constructor 를 매개변수로 받는 Order 2 의 자리입니다. C# 어법에는 그 자리가 직접 없습니다.

### C# 컴파일러가 위 가상 코드를 거부하는 이유

위의 가상 `Functor<F>` 코드를 그대로 IDE 에 적고 빌드하면 컴파일러가 `F<B>` / `F<A>` 자리에서 두 에러를 냅니다.
...
비유 한 줄로 정리하면, C# 의 제네릭 자리는 **완성된 우편함** (자료가 채워진 형태) 만 받습니다. **우편함을 만드는 공장** (`List` 같은 type constructor 자체) 을 매개변수로 받는 어법이 없습니다. 그래서 `F<...>` 처럼 F 자체가 다시 매개변수를 받는 자리가 거부됩니다.

여기서는 이 정도 직감으로 충분합니다. 어휘 (Order / kind / type constructor 일급) 의 정착은 다음 두 절에서 단계적으로, 두 에러 메시지의 정밀 분석은 그 뒤 핵심 제약 절에서 다룹니다. 이 한계가 핵심 제약이고, 한계를 우회하는 도구가 2장의 주제입니다.

본격적인 도구 (`K<in F, A>` 마커, self-bound, static abstract) 로 들어가기 전에, *Higher* 라는 어휘의 직감을 값 차원에서 먼저 정착시키고 같은 어휘를 타입 차원으로 평행 매핑합니다. 그 과정에서 C# 의 핵심 제약이 드러나고, HKT 가 직접 지원되었다면 어떤 가치가 있었을지가 우회의 동기가 됩니다.
...
>
> 이 책의 값 생성자는 그보다 광의로, 값 차원에서 값을 만드는 모든 어휘로 사용합니다. 리터럴 (`1`, `"hello"`) 도 0-인자 constructor 의 결과, 함수 (`int → int`) 도 값을 만드는 어휘, 고차 함수 ((`int → int`) → `int`) 도 값을 만드는 어휘로, 셋 모두 값을 만든다는 점에서 같은 어휘로 묶입니다. 이 절은 그 값을 만드는 어휘들을 *Order* 라는 단계로 분류합니다. §2.5 의 타입 생성자 (type constructor) 가 타입을 만드는 어휘를 같은 Order 어휘로 분류하는 평행 자리입니다.

Order 0 은 평범한 값, Order 1 은 값을 받아 값을 만드는 함수, Order 2 는 함수를 받아 값을 만드는 고차 함수 (HOF: Higher Order Function) 입니다.

| Order | 시그니처 (type signature) | 의미 | 예 |
|---|---|---|---|
| Order 0 | `int`, `string` 등 (구체 type) | 평범한 값 — 변수에 곧장 담음 | `1`, `2`, `new User()`, `new List<int>()` |
| Order 1 | `int → int` (값 → 값) | 평범한 함수 — 값을 받아 값을 반환 | `int DoubleIt(int)`, `string Xxx(int)`, `Func<int, double>` |
| Order 2 | `(int → int) → int` (함수 → 값) | 고차 함수 (HOF) — 함수를 받아 값을 반환 | `int DoubleIt(Func<string, int>)`, `xs.Aggregate(seed, f)` |

Order 0 의 자리는 값 자체로, C# 의 리터럴 / 인스턴스 / static 멤버가 모두 여기 해당합니다. Order 1 은 값을 받아 값을 산출하는 평범한 함수로, C# 의 메서드 / `Func<,>` / 람다가 여기 해당합니다. Order 2 는 함수를 받아 값을 산출하는 고차 함수로, LINQ 의 `Select`, `Where`, `Aggregate` 가 모두 여기 해당합니다.
...
같은 영역의 어휘 (`42`) 가 값 → 값을 다루는 함수 → 함수를 다루는 함수로 한 단계씩 *Higher* 로 올라갑니다.
...
> ```
>
> `Negate` 는 인자가 하나이고 값 (int) 이라 Order 1 입니다. `Apply` 는 같은 1-인자지만 인자가 함수 (`int → int`) 라 Order 2 입니다. `Add` 는 인자가 둘이지만 모두 값이라 여전히 Order 1 입니다. *Higher* 의 본질은 몇 개가 아니라 어떤 어휘가 인자 자리에 들어오는가입니다.

> **고차의 핵심 조건** (Wikipedia, *Higher-order function*) — *"a function that does at least one of the following: takes one or more functions as arguments, returns a function as its result"*. 즉 함수를 인자로 받거나 함수를 반환하면 고차 함수입니다. 같은 자료에 명시 — *"A function that merely returns a value (not a function) is classified as first-order"*. 함수가 단순히 값을 반환한다는 사실만으로는 *first-order* (1 차 함수, Order 1) 입니다. *higher* 가 되려면 **인자 또는 출력 자리에 함수가 등장** 해야 합니다.

| 값 생성자 | 시그니처 (type signature) | 인자 개수 | 인자 자리의 어휘 | Order |
|---|---|---|---|---|
| `42`, `new User()`, `new List<int>()` | `int`, `User`, `List<int>` 등 (구체 type) | 0 | (인자 없음) | Order 0 (값) |
| `int Negate(int n)` | `int → int` | 1 | 값 | Order 1 (기본형) |
| `int Add(int x, int y)` | `int → int → int` | 2 | 값, 값 | Order 1 (다인자) |
| `int Apply(Func<int, int> f)` | `(int → int) → int` | 1 | 함수 | Order 2 (고차 함수) |
| `new User(Func<int, int> f)` | `(int → int) → User` | 1 | 함수 | Order 2 (고차 생성자) |
...
제약은 타입 차원에서 등장합니다. 같은 Order 어휘를 타입 차원에 평행 매핑하면 C# 의 핵심 한계가 드러납니다.

> **어휘가 많다고 느껴진다면 자연스러운 반응입니다.** 이 장의 어휘 (*Order* / *kind* / *type constructor* / *HKT* / `K<F, A>` 마커 / self-bound / static abstract / 3-tuple) 는 **값 차원** → **타입 차원** 두 평행을 풀이하는 데 필요한 최소입니다. 한 번에 외울 필요는 없습니다. 같은 어휘를 이후 절들에서 반복 등장시켜 자연 정착시킵니다.

---

## 2.5 타입 차원의 Order — C# 의 제공 범위

§2.4 에서 값 차원의 Order 0 / 1 / 2 어휘를 봤습니다. 같은 어휘를 타입 차원으로 평행 매핑하면 *kind* (타입의 타입) 라는 어휘가 등장합니다. 두 차원의 평행은 다음 표가 한 자리에 모읍니다.

| Order | 값 차원 (type signature) | 타입 차원 (kind) | 어휘 의미 |
|---|---|---|---|
| Order 0 | `int`, `List<int>` (구체 type) | `*` | 입력 자리 없는 완성 어휘 |
| Order 1 | `int → int` | `* → *` | 한 자리를 받아 한 자리를 만듦 |
| Order 2 | `(int → int) → int` | `(* → *) → *` | 입력 자리에 한 단계 위 어휘가 들어옴 |

화살표 어법 (`→` 의 입력 / 출력) 은 두 차원에서 정확히 평행하지만 **이름은 분리** 됩니다. 값 차원의 `int → int` 는 *type signature*, 타입 차원의 `* → *` 는 *kind* 입니다. kind 는 type-level 어휘로 값에는 적용되지 않습니다 (Wikipedia *Kind (type theory)*). 그래서 §2.4 표가 값 차원의 정통 표기 (`int`, `int → int`) 를 쓰고, 이 절부터 kind 표기 (`*`, `* → *`) 가 등장합니다.

> **type theory 의 3 단 계층** — kind 가 값에 적용되지 않는 이유는 type theory 의 계층 구조에서 나옵니다.
>
> ```
> value : type : kind
>   ↑      ↑      ↑
...
>
> Wikipedia 표현은 다음과 같습니다. *"kinds classify types, just as types classify values"*. 즉 kind 가 값을 분류하지 않습니다. 그래서 값 차원의 평행 자리는 **구체 type 어휘** (`int`, `int → int`, `(int → int) → int`) 가 됩니다.

C# 의 타입 차원이 어디까지 제공하는지 한 단계씩 점검하면 비대칭이 드러납니다.
...
> - **`* → *`** — 왼쪽 `*` 를 입력으로 받아 오른쪽 `*` 를 출력으로 만드는 type constructor. 예: `List` 가 `int` 를 받아 `List<int>` 를 만듭니다. 화살표 왼쪽 입력 자리는 T 자리가 비어있는 형태 (예: `List<_>`) 로, 자료를 채우면 완성 타입이 됩니다.
...
> - **`* → * → *`** — 입력 자리가 두 개인 다인자 type constructor. 왼쪽 `*` 와 가운데 `*` 두 자리에 완성 타입을 채워 오른쪽 `*` 를 만듭니다. 예: `Dictionary<K, V>`, `Either<L, R>`. 인자가 모두 완성 타입이라 HKT 가 아닙니다 (다인자 ≠ 고차, §2.4 와 같은 판별 규칙).
>
> **핵심** — 화살표 어법의 왼쪽 자리가 어떤 kind 를 받는가가 *Order* 를 결정합니다. 왼쪽이 `*` 만이면 Order 1, 왼쪽이 `(* → *)` 면 Order 2 (HKT).
...
`int` 는 T 자리가 없는 완성 타입이고, `List<int>` 는 T 자리에 `int` 가 채워진 완성 타입입니다. 둘 다 kind `*` 에 해당하고, C# 이 모든 어법에서 자유롭게 다룹니다.
...
C# 어법은 type constructor 의 정의는 제공합니다. `public class Box<T> { ... }` 같은 제네릭 정의가 여기 해당합니다.
...
함수 시그니처 차원에서도 같은 한계가 그대로 드러납니다. `F<A>` / `F<B>` 어법 자체가 F 가 type constructor 라고 가정한 자리라 컴파일러가 거부합니다.

```csharp
// ✗ 함수 시그니처에서 F 가 type constructor 라고 가정한 자리도 거부
F<B> DoubleIt<F, A, B>(F<A> fa) { ... }     // ✗ 두 곳 모두 CS0307 — F<B> 어법이 C# 컴파일러를 통과 못함
//   ─┬─                  ─┬─
//   F<B>, F<A>: F 가 다시 인자를 받는 어법 — C# 어법에 없음
```

`DoubleIt<F, A, B>` 의 의도는 F 가 어떤 type constructor 든 받아 `F<A>` 를 `F<B>` 로 변환하는 것입니다. 즉 F 자체를 매개변수로 받아 그 안에 인자를 끼우는 자리, Order 2 의 자리 (kind `(* → *) → *`) 에 해당합니다. C# 어법에 그 자리가 없어 `F<A>` / `F<B>` 두 자리에서 CS0307 에러가 발생합니다. Order 1 의 type constructor 일급 미지원이 Order 2 의 미지원으로 이어집니다.

C# 의 정의만 제공이 의미하는 바가 바로 이것입니다. `Box<T>` 같은 제네릭 정의는 가능하지만, T 자리가 채워진 완성 타입 (`Box<int>`) 만 일급 시민이고, T 자리가 비어있는 type constructor 자체 (`Box`) 는 일급 시민이 아닙니다. 함수 시그니처 / interface 어법 어디서든 F 가 type constructor 매개변수라는 가정 자체가 어법으로 막혀 있습니다.
...
타입 차원의 Order 2 가 **2장의 본 목표** 에 해당합니다. type constructor 를 매개변수로 받는 추상, 즉 `Functor` 의 `f` 자리가 여기입니다. kind 어휘로는 `(* → *) → ...` 에 해당하고, 값 차원의 함수를 받는 함수 (HOF) 가 타입 차원에 평행 매핑된 모습입니다.

§2.3 의 함수형 발상 (N+M 비용 절감) 이 F 자리에 어떤 컨테이너든 끼움으로 달성되려면, F 가 type constructor 매개변수 자리에 들어와야 합니다. 가상 코드로 보면 다음과 같습니다.
...
위 가상 코드의 각 자리가 어떤 kind 의 자리인지 단계별로 따라가 보면, 왜 `Functor<F>` 자체가 Order 2 (HKT) 인지 직감이 잡힙니다. 세 단계로 kind 를 도출합니다. 첫째, `F` 자체의 kind 를 정합니다 (`* → *`). 둘째, `F<A>` / `F<B>` 의 kind 가 무엇인지 결정합니다 (`*`). 셋째, 둘을 합쳐 `Functor<F>` 자체의 kind 가 `(* → *) → *` 임을 도출합니다.

#### 1 단계 — `F` 의 kind = `* → *` (한 인자 type constructor)

먼저 `F` 가 무엇인지 정합니다. `Functor` 는 **컨테이너** (`List`, `Option`, `Task` 같은 어휘) 에 대한 추상이므로, `F` 자리에는 그 컨테이너들이 들어옵니다.
...
다른 컨테이너도 같은 모양입니다. `Option`, `Task` 모두 자료 1 개를 받아 완성 타입을 만드는 어법, 즉 모두 kind `* → *`. 따라서 `Functor` 가 받기로 한 자리 (`F`) 의 kind 가 **`* → *`** 입니다.

이 결정은 Functor 가 컨테이너 위에서 동작하는 추상이라는 의도에서 자연스럽게 나옵니다. 시그니처 자체가 이 가정을 **요구** 합니다. 위 코드의 `F<A>` / `F<B>` 어법은 F 가 다시 인자를 받는 자리이고, 이런 어법은 `F` 가 type constructor (kind `* → *`) 일 때만 의미가 있습니다. 만약 `F` 의 kind 가 단순 `*` (완성 타입) 였다면 `F<A>` / `F<B>` 어법 자체가 불가능합니다.

> **한 줄 정리** — kind `* → *` 는 한 타입 인자를 받아 완성 타입으로 가는 type constructor 이고, `F` 가 컨테이너 자체이므로 이 kind 를 가집니다.
...
> **한 줄 정리** — 입력 자리에 한 단계 위 어휘 (`* → *`) 가 들어오면 전체 kind 가 Order 2 로 올라갑니다.
...
세 단계의 결론은 다음과 같습니다. `Functor<F>` 의 kind `(* → *) → *` 가 **2 장의 핵심 목표** (Order 2, HKT) 자리이고, 이것이 C# 어법에 없는 자리입니다.
...
| `F` (매개변수) | `* → *` | 1 |
| `F<A>`, `F<B>`, `Func<A, B>` | `*` | 0 |
| **`Functor<F>` 자체** | **`(* → *) → *`** | **2 (HKT) — 2장의 본 목표** |

세 단계 도출과 두 표가 kind `(* → *) → *` 자리가 Order 2 (HKT) 임을 형식적으로 정합니다. 다음 절 (§2.5.4) 이 이 결론을 두 자리에서 한 번 더 검증합니다.

---

### 2.5.4 Order 2 의 검증 — 값 차원 평행과 C# 컴파일 에러

앞 절 (§2.5.3) 의 kind 도출이 형식적 논증이라면, 이 절은 그 결론을 두 자리에서 검증합니다. 첫째, 값 차원의 고차 함수 (HOF) 와 타입 차원의 HKT 가 같은 함수 어법의 두 차원임을 평행 비교합니다. 둘째, C# 컴파일러가 위 가상 코드를 실제로 어떤 에러로 거부하는지 확인하고, Haskell / Scala 가 같은 어법을 어떻게 일급으로 다루는지 비교합니다.

#### 값 차원과의 평행 — 둘 다 함수 어법

타입 차원의 `Functor` 가 값 차원의 무엇과 평행한지 보면 직감이 한 번 더 잡힙니다. 핵심은 **type constructor 자체가 타입 차원의 함수** 라는 점입니다. kind 표기의 `* → *` 의 `→` 가 함수 시그니처의 `→` 와 같은 어법 (왼쪽 입력, 오른쪽 출력) 이라, type constructor 도 타입을 받아 타입을 만드는 함수입니다. 즉 값 차원의 함수와 타입 차원의 type constructor 는 **같은 함수 어법의 두 차원** 입니다.

| 차원 | Order 1 — 평범한 함수 | Order 2 — 함수를 받는 함수 |
|---|---|---|
| **값 차원** | `int → int` — 값을 받아 값을 만드는 함수 | **`(int → int) → int`** — **함수를 받아** 값을 만드는 고차 함수 (HOF) |
| **타입 차원** | `* → *` — 타입을 받아 타입을 만드는 함수 (= type constructor) | **`(* → *) → *`** — **함수 (= type constructor) 를 받아** 타입을 만드는 HKT |
| **공통 어법** | 입력 1 개의 어휘 → 출력 1 개 | 입력 자리에 함수가 등장 → 한 단계 위 어휘 |

두 차원 모두 **함수를 받아 무엇을 만드는** 어법입니다. 다만:

- **값 차원** — 함수 (value constructor) 가 값을 받아 값을 만듦. 시그니처는 `int → int` 같은 **type signature**
- **타입 차원** — 함수 (type constructor) 가 타입을 받아 타입을 만듦. 시그니처는 `* → *` 같은 **kind**

> **어휘 구분 — type signature vs kind**. 두 차원의 시그니처 어법 (`→` 의 입력 / 출력) 은 동일하지만 **이름은 다릅니다**. 값 차원의 함수 시그니처는 *type signature* (예: `int → int`), 타입 차원의 type constructor 시그니처는 *kind* (예: `* → *`) 입니다. kind 는 **type-level 의 어휘** 로, 값에는 적용되지 않고 type 과 type constructor 에만 적용됩니다 (Wikipedia *Kind (type theory)*). value constructor 자체는 **type signature 만** 가지며 kind 는 갖지 않습니다 (Haskell Wiki *Constructor* — *a data or value constructor has a type; a type of a type constructor itself is called kind*). 즉 두 차원이 **어법은 평행**, **적용 차원은 분리**.

즉 `Functor<F>` 가 타입 차원의 한 단계 위에 해당합니다. `F` (타입 차원의 함수 = type constructor) 를 매개변수로 받는 추상이라 `(* → *) → *` Order 2 (HKT) 입니다. 값 차원의 HOF 가 함수를 받는 함수인 어법과 같고 대상 (값 vs 타입) 만 다릅니다.

#### C# 컴파일 에러 위치 + 다른 언어 비교

C# 컴파일러는 제네릭 매개변수 `F` 를 완성 타입 (Order 0, kind `*`) 만 받습니다. 위 코드의 `F<B>` / `F<A>` 두 자리는 F 가 다시 인자를 받는 어법으로, F 가 type constructor (kind `* → *`, Order 1) 여야 가능한 어법입니다. 두 자리에서 컴파일러가 같은 에러를 냅니다.

- **CS0307** — *static `F<B>` Map<A, B>(Func<A, B> f, `F<A>` fa)* 의 두 `F<...>` 자리에서 발생합니다. F 가 type parameter 인데 이 자리에서는 유효하지 않다는 뜻으로, 컴파일러는 `F` 를 완성 타입으로만 알고 있어 `F<...>` 어법을 F 가 type constructor 라는 가정의 표현으로 받아들이지 못합니다.

뿌리 원인은 §2.5.2 에서 이미 다뤘습니다. Order 1 에서 type constructor 자체가 일급 시민이 아니라서, `Functor<F>` 의 F 자리에 type constructor 가정이 어법으로 불가능합니다. 그 결과 F 가 다시 인자를 받는 `F<...>` 어법 모두가 거부되고, 한 단계 위인 Order 2 (type constructor 를 매개변수로 받는 자리) 도 도달 불가능합니다.
...
Haskell 의 `f` 자리에 `[]` / `Maybe` / `IO` 같은 type constructor 자체가 직접 들어가고, 컴파일러가 kind `* → *` 인지 검증합니다. C# 어법에는 그런 어법이 없습니다.

![Order 단계의 값 차원/타입 차원 평행](./images/Ch02-Higher-Kinds/04-order-parallel.svg)

**그림 2-5. Order 단계로 본 Higher — 값 차원과 타입 차원의 평행** — 아래 행 값 차원 (Normal World 색) 에 Order 0 → Order 1 → Order 2 (HOF) 세 박스. 위 행 타입 차원 (Elevated World 색) 에 같은 Order 0 → Order 1 → Order 2 (HKT) 세 박스. 가운데 점선 매핑이 같은 Order 끼리 평행을 시각화합니다.

---

## 2.6 값 생성자 vs 타입 생성자 — 통합 비교 + C# 의 비대칭

§2.4 와 §2.5 에서 값 차원과 타입 차원의 Order 어휘를 평행 매핑했습니다. 한 자리에 모아 보면 두 차원의 Order 어휘 평행과 C# 일급 시민 비대칭이 동시에 드러납니다.

| Order | kind | 값 차원 (값 생성자) | 타입 차원 (타입 생성자) | C# 제공 |
|---|---|---|---|---|
| **Order 0** | `*` | 값 자체 — `42`, `new User()`, `new List<int>()` | 완성 타입 — `int`, `List<int>` | ✓ **완전 제공** (두 차원 모두) |
| **Order 1** | `* → *` | 평범한 함수 — `Negate`, `Add` / 생성자 — `new User(int)` | type constructor — `List` (Haskell) / `List<_>` (Scala) | 값 차원 ✓ / 타입 차원 △ **정의만 제공** (`List<T>` 정의 OK, `List` 자체 일급 ✗) |
| **Order 2** | `(* → *) → *` | 고차 함수 — `Apply` / 고차 생성자 — `new User(Func<int,int>)` | HKT — `Functor<F>` 의 `F` 자리 (**2장의 본 목표**) | 값 차원 ✓ / 타입 차원 ✗ **미지원** |

세 줄로 정리하면 다음과 같습니다.

- **Order 어휘는 두 차원에 동일** — `*` / `* → *` / `(* → *) → *` 세 단계가 값 차원과 타입 차원에 똑같이 적용됩니다. 값 차원의 고차 함수 (HOF) 가 타입 차원의 고차 타입 (HKT) 으로 평행 매핑됩니다.
- **C# 의 비대칭은 타입 차원에만 등장** — 값 차원은 Order 0/1/2 모두 일급 시민으로 완비. 타입 차원은 Order 0 만 완전 일급, Order 1 은 정의만 제공 (`List<T>` 같은 제네릭 정의는 OK, `List` 자체 type constructor 는 일급 ✗), Order 2 는 미지원.
...
이 한계가 뒤이어 두 에러 메시지로 다시 검증되고, 한계를 우회하는 도구가 `K<F, A>` 마커 + self-bound + static abstract 세 가지입니다.

---

## 2.7 C# 의 핵심 제약 — Order 2 (HKT) 미지원

§2.5 에서 C# 의 타입 차원 비대칭 (Order 0 완전 제공 / Order 1 정의만 제공 / Order 2 미지원) 을 봤습니다. §2.6 에서 같은 비대칭을 값 차원과 평행으로 비교했습니다. 이 절에서는 그 결론을 코드 차원에서 한 번 더 검증하고, 다른 언어들이 같은 한계를 어떻게 푸는지 비교 표로 정리합니다. C# 의 핵심 제약을 **컴파일러가 직접 뱉는 두 에러 메시지** (CS0307 + CS0246) 와 **전체 언어 풀이 표** 둘로 마무리합니다.
...
두 에러 메시지의 정확한 의미는 §2.3 의 컴파일러 거부 절에서 풀었습니다. 한 줄로 다시 정리하면, **CS0307** 은 "F 가 다시 인자를 받는 자리에서 사용 불가" 라는 뜻이고, **CS0246** 은 "F 를 완성 타입으로 해석하려 했으나 찾을 수 없다" 는 뜻입니다. 두 에러 모두 C# 의 제네릭 매개변수가 완성 타입 (Order 0) 만 받는 어법에서 비롯되며, type constructor (Order 1) 자체를 매개변수로 받는 추상 (Order 2, HKT) 을 문법으로 표현할 수 없습니다. 학술 어휘로는 *higher-kinded polymorphism* 미지원입니다.
...
우회의 비용은 학습 곡선 (3-tuple 패턴 + 다운캐스트 + self-bound 제약) 입니다. 대가는 Haskell / Scala 의 거의 모든 함수형 추상이 C# 에서 표현 가능해진다는 점입니다. Higher Kinds 의 형식 이론적 기반과 *Higher* 의 정의도 이미 논문 어휘로 정립되어 있습니다[^moors08].
...
C# 의 핵심 제약을 살펴봤습니다. 이제 만약 C# 이 직접 지원했다면 어떤 가치가 있었을지를 가상 코드로 먼저 확인하고, 그 가치를 우회 도구로 실현하는 절차로 이어집니다.

---

## 2.8 가상 코드로 본 Order 2 의 가치 — §2.2 문제 돌파 확인

앞에서 Order 2 자리의 직접 표현이 막힌다는 사실을 두 번 봤습니다 (어법 분석 + 두 에러 메시지). 그렇다면 만약 직접 지원했다면 어떤 가치가 있었을까요? 가상의 C# 어법으로 그 가치를 먼저 확인하고, 뒤이어 실제 도구로 어떻게 같은 가치를 얻는지의 동기로 삼습니다.
...
**가치는 Functor 에서 그치지 않습니다** — 같은 발상이 1부의 다른 추상에서도 반복됩니다.
...
trait 한 번 정의 → 모든 컨테이너 자동 적용 이라는 발상이 1 부의 추상 사다리 전체에서 반복됩니다. F 자리의 일반화 한 번이 1 부 전체의 추상에 자유롭게 확장됩니다. 즉 HKT 의 가치는 한 trait 의 편리가 아니라 **함수형 어휘 전체의 통합** 입니다.

> **§2.2 문제 돌파 확인 (가상)** — §2.2 의 N×M 비용 (List.Map / Option.Map / Task.Map 따로 구현) 이 가상 코드 한 줄 (`Map<F, A, B>(F<A>, Func<A, B>) → F<B>`) 로 N+M 비용으로 줄어듭니다. 새 컨테이너 추가는 컨테이너 1 개 등록, 새 능력 추가는 trait 1 개 정의 만으로 끝납니다. 가치는 명확합니다. 그러나 C# 의 미지원으로 이 가상 코드는 직접 컴파일 불가입니다. 이제 우회 도구로 같은 가치를 실제 코드로 실현합니다.

---

## 2.9 첫 번째 우회 — `K<in F, A>` 마커 인터페이스

C# 의 핵심 제약과 만약 지원했다면 어떤 가치가 있었을지까지 봤습니다. 이제 LanguageExt v5 의 핵심 우회 도구인 `K<F, A>` 마커 인터페이스의 발상부터 봅니다. 실제 코드 적용은 MyList 진화와 3-tuple 패턴 예제로 이어집니다.
...
해결은 F 자리를 완성 타입처럼 사용하는 것입니다 (kind 한 단계 내림). 빈 태그 타입 (`MyListF`) 을 만들어 List 의 이름표 역할만 시킵니다.
...
**C# 의 Order 단계 활용을 한 줄로 정리합니다.** C# 이 Order 1 (type constructor) 을 일급으로 못 다루니, Order 0 (완성 타입) 만으로 Order 2 의 다형성을 흉내냅니다. `F` (원래 Order 1) 자리를 빈 태그 (Order 0) 로 사용하고, `A` (Order 0) 와 나란히 `K<F, A>` (Order 0) 로 묶었습니다. **kind 한 단계 내림** 으로 어법 안의 자리를 되찾은 셈입니다.

> **`in F` 의 함수형 의미** — `F` 자리에 붙은 `in` (contravariant) 표기의 직감은 단순합니다. brand `F` 는 컨테이너 종류를 분류하는 type-level dispatch 어휘이고, **분류는 입력받는 자리** 라 contravariant 가 자연입니다. 지금은 **반공변** 이 **왜 필요한가** 의 직감만 가져가도 충분합니다. 더 결정적인 효과인 **trait variance 반전** (`K<in F, A>` 의 contravariant context 가 후속 카테고리적 trait 의 어휘를 가능하게 만드는 자리) 은 자연 변환 (11장) 에서 자세히 풀이합니다.
...
### 2.10.2 두 번째 도구 — static abstract (타입의 정적 멤버로서의 동작)
...
F 가 값이 아니라 어휘라는 점이 핵심입니다. `MyListF` 라는 타입 이름이 "이 Elevated World 의 동작" 을 호출하는 입구가 됩니다.

C# 11 (2022) 이전에는 이 모양이 원리적으로 불가능했습니다. 객체지향의 인스턴스 메서드는 객체에 능력을 붙이는 도구였고, 클래스의 정적 메서드는 상속·다형성이 안 됐습니다. C# 11 의 `static abstract` 가 정적 자리에 강제되는 다형성을 가능하게 만들어 능력이 *trait* 에 사는 함수형의 모양이 C# 어법 안에 들어왔습니다.
...
F 자리의 가치를 가상 코드로 봤고, 두 우회 도구 (`K<in F, A>` 마커, self-bound, static abstract) 도 봤습니다. 이제 가상의 한 줄이 실제 동작 코드의 한 줄로 어떻게 변환되는지 직접 비교합니다.
...
> **§2.2 N×M 문제의 최종 돌파 (실제)** — §2.2 의 N×M 비용 (List.Map / Option.Map / Task.Map 따로 구현) 이 우회 도구 두 가지 (K 마커 + self-bound + static abstract) 로 실제 컴파일 가능한 한 줄 (`static abstract K<F, B> Map<A, B>(Func<A, B>, K<F, A>)`) 로 실현됩니다. F 자리에 빈 태그 (`MyListF`, `MyMaybeF`, `MyTaskF`, ...) 를 바꿔 끼우면 같은 `Map` 이 모든 컨테이너 위에서 자동 동작합니다. §1.6.1 의 trait 다형성의 N+M 비용이 C# 어법으로 실제 구현됩니다.
...
가상 코드의 F 자리가 실제 코드의 F 자리 (빈 태그) 로 실현됐고, 가상 코드의 호출 입구가 실제 코드의 태그 이름의 정적 메서드로 실현됐습니다. C# 의 핵심 제약이 두 도구로 돌파됩니다.
...
### 2.13.1 Before — `K<F, A>` 우회 없이

제네릭 `Functor<A>` 어법의 `MyList<A> : Functor<A>` 를 먼저 봅니다. A 만 매개변수로 둔 어법으로, F 가 시그니처에서 가리킬 수단이 없습니다. 모양 보존도, 체이닝의 F 추적도, 어떤 F 든 받는 일반 함수도 안 되는 자리입니다.
...
### 2.13.2 After — 3-tuple 도입

같은 `MyList` 를 `K<F, A>` 마커와 self-bound + static abstract 두 도구로 다시 씁니다.
...
### 2.13.3 자료 / 태그 / trait — 책임 세 조각 (+ K 묶는 신호)

After 코드의 네 조각이 어떤 책임을 지는지 정리합니다.
...
**그림 2-1. 3-tuple 패턴 (자료 / 태그 / trait 의 세 조각)** — 태그 타입 (`MyListF`) 이 trait (`Functor<F>`) 을 구현하고, 자료 타입 (`MyList<A>`) 은 `K<MyListF, A>` 의 옷을 입어 태그의 정적 메서드 (`Map`) 안에서 다운캐스트로 본 모습을 드러냅니다. 세 조각이 느슨하게 결합되어 책임이 분리됩니다.

> **한 줄 정리** — 네 조각이 서로 다른 책임을 지므로 한 조각의 변경 비용이 작습니다.

### 2.13.4 호출 모양 — 정적 자리에서 부르기
...
### 2.13.5 세 약점이 어떻게 해결되는가

§2.13.1 의 *Before* (제네릭 `Functor<A>` 어법) 가 보였던 세 약점이 *After* (`K<F, A>` + 3-tuple) 에서 차례로 해소됩니다. 핵심을 표로 정리합니다.
...
`F` 가 어떤 Elevated World 인지 함수는 모릅니다. 알 필요도 없습니다. `where F : Functor<F>` 가 F 가 Map 정적 메서드를 가진다는 약속을 컴파일러에 전달하면, 모든 Functor 인스턴스에 한 줄 호출이 통합니다. §2.14 의 `FunctorOps` 예제에서 이 패턴의 전체 사용 예를 봅니다.

> **한 줄 정리** — F 가 시그니처에 박혀 끝까지 살아 있으므로 모양 보존 / F 추적 / 일반 함수 세 자리가 동시에 해결됩니다.

### 2.13.6 다운캐스트 `(MyList<A>)fa` — 왜 안전한가

`(MyList<A>)fa` 의 다운캐스트가 어색해 보일 수 있습니다. 그러나 3-tuple 의 책임 분리 덕분에 이 자리에 들어올 수 있는 자료가 단 하나로 결정됩니다.
...
`K<F, A>` 가 빈 인터페이스라 컴파일러는 "F = MyListF 면 자료 타입이 `MyList<A>`" 라는 연결을 모르고, 이 연결을 3-tuple 의 명시적 다운캐스트로 사람이 알려줍니다. **책임 분리** 가 invariant 를 만들어 다운캐스트가 사실상 안전합니다. LanguageExt 라이브러리는 확장 메서드 `.As()` 로 다운캐스트를 감춥니다 (`var items = fa.As().Items;`).

이 3-tuple 패턴 (자료 + 태그 + trait) 이 1부의 모든 함수형 추상의 표준입니다.
...
`code/Part01-Foundations/Ch04-Functor/` 에 `MyList` 와 `MyMaybe` 의 3-tuple 코드가 있습니다. 한 번 익히면 새 추상 (Functor, Applicative, Monad, Foldable, Traversable) 을 만나도 세 조각이 어디 있는가만 찾으면 됩니다.
...
| 절 | 무엇을 정착시켰는가 |
|---|---|
| §2.13.1 Before | F 가 시그니처에서 사라진 어법의 세 약점을 한 자리에서 봤습니다. |
| §2.13.2 After | `K<F, A>` + self-bound + static abstract 로 F 가 시그니처에 박힙니다. |
| §2.13.3 네 조각 | 신호 `K<F, A>` / 자료 `MyList<A>` / 태그 `MyListF` / trait `Functor<F>` 의 책임 분리. |
| §2.13.4 호출 모양 | `MyListF.Map(f, xs)` — 호출 입구가 태그 타입의 정적 메서드. |
| §2.13.5 세 약점 해결 | 모양 보존 / F 추적 / 일반 함수가 한 자리에서 해결. |
| §2.13.6 다운캐스트 안전성 | 3-tuple invariant 가 `(MyList<A>)fa` 의 안전성 근거. |
...
> **3-tuple 의 ROI** — 새 컨테이너를 만들 때 3 조각만 정의하면 모든 일반 함수가 자동으로 동작합니다. `Sum`, `Count`, `All`, `Any` 같은 자유 함수를 다시 짤 필요가 없습니다. 6장 Foldable 에서 이 ROI 를 직접 봅니다.

---

## 2.15 Elevated World 어휘로 다시 읽기

이 장에서 익힌 도구를 1장의 4 가지 함수 유형에 다시 매핑하면 다음과 같습니다.
...
Order 가 올라가는 기준은 인자 개수가 아니라 인자 자리에 한 단계 위 어휘 (함수) 가 들어오는 자리입니다.
...
객체지향에서 능력은 객체 (컨테이너 클래스) 의 인스턴스 메서드입니다. `List<A>.Map`, `Option<A>.Map`, `Task<A>.Map` 셋이 모양만 같고 자리가 달라 N 개 컨테이너 × M 개 능력 = N×M 개의 별개 구현이 필요합니다. 함수형은 능력이 사는 자리를 *trait* 으로 옮깁니다. `Functor<F>` 한 trait 에 `Map` 한 번 정의 + F 자리에 컨테이너 끼움 = N+M 비용 (trait M 개 + 컨테이너 N 개). 비용 절감의 핵심이 F 자리이고, 그 자리가 Order 2 (type constructor 매개변수) 의 자리입니다.
...
0 개. 빈 인터페이스입니다. C# 이 Order 2 의 자리 (kind `(* → *) → ...`) 를 직접 표현 못하니, Order 0 의 어휘 (kind `*` 의 빈 태그 타입) 로 Order 2 의 다형성을 인코딩하는 type-level 신호입니다. 발상 한 줄로 정리하면 Order 2 의 다형성을 Order 0 의 인코딩으로 푸는 자리입니다.

> Q7. 가상의 `Functor<F<_>>` 한 줄이 실제 한 줄로 어떻게 변환됩니까?
...
안의 타입 `A` 는 추적되지만 F 자체 (List / Option / Result) 가 시그니처에서 사라집니다. 모양 보존이 약속되지 않고, 체이닝 시 컨테이너 정보가 점점 모호해지며, 어떤 F 든 받는 일반 함수를 쓸 수 없습니다. 세 약점 모두가 §2.13.5 ~ §2.13.6 에서 해결됩니다.
...
값이 아니라 타입 이름이 호출 입구입니다. `MyListF.Map(f, xs)` 처럼 *trait* 에 사는 능력을 직접 부릅니다. 인스턴스 메서드 (`xs.Map(f)`) 가 능력이 객체에 사는 모형이라면, static abstract 는 능력이 *trait* 에 사는 모형입니다 (1장 §1.5).
...
- 2장 전체는 함수형의 본질, 즉 합성 가능한 Elevated World 로 lift 의 둘째 축 type class 다형성을 C# 어법으로 구현하는 도구 모음입니다. 능력이 trait 의 정적 자리에 살고 컴파일 타임에 해소되도록 강제하는 두 도구를 다뤘습니다.
- 객체지향 어법의 N×M 비용 (컨테이너 별 인스턴스 메서드 따로 구현) 이 함수형의 N+M 비용 (trait 한 번 정의 + F 자리에 컨테이너 끼움) 으로 줄어듭니다. 절감의 핵심은 F 자리이고, 이것이 Order 2 (type constructor 매개변수) 의 자리에 해당합니다.
...
- C# 의 핵심 제약은 다음과 같습니다. 제네릭 매개변수가 완성 타입 (Order 0) 만 받고 type constructor (Order 1) 자체를 매개변수로 받는 추상 (Order 2, HKT) 은 문법으로 표현할 수 없습니다. Order 2 를 지원하는 가상 C# 에서는 Functor / Monad / Foldable / Traversable 의 trait 한 번 정의로 N 개 컨테이너에 자동 적용됩니다.
...
- 가상 한 줄이 실제 한 줄로 변환됩니다. F 자리 / 컨테이너 어휘 / 호출 입구 세 곳이 K 마커, self-bound, static abstract 로 일대일 매핑되어, §2.2 의 N×M 비용이 실제 컴파일 가능한 N+M 비용으로 돌파됩니다.
- 새 컨테이너의 입회 절차는 3-tuple 패턴 (자료 / 태그 / trait 의 세 조각) 입니다. 한 번 등록되면 어떤 Functor 든 받는 일반 함수가 그 컨테이너에 자동 적용됩니다 (`Sum`, `Count`, `Lift`, `Apply`, `Bind` …).
