...
> **이 장의 핵심 어휘** — 지금 외우는 목록이 아니라, 본문에서 하나씩 천천히 도입할 예고편입니다. 앞의 네 개가 본문의 도구이고, 뒤의 네 개는 한계의 정식 이름으로 깊이 보기 (§2.11) 에서 정리합니다.
>
...
격자의 어느 칸 하나가 **미묘하게 다르게** 구현되어도 컴파일러는 알아채지 못합니다. 예를 들어 누군가 `MyOption.Map` 만 "`None` 에서 예외를 던지도록" 짜 놓고 떠나면, `MyList.Map` 과 `MyOption.Map` 이 "같은 Map 이어야 한다" 는 약속이 깨집니다. 그러나 그 약속은 **각 메서드의 본문에만** 존재할 뿐, 코드의 구조에는 적혀 있지 않습니다. 1장 §1.2.3 에서 `items` 와 `total` 의 약속이 절차의 본문에만 묻혀 있던 것과 정확히 같은 종류의 문제입니다. **같은 능력임을 코드가 어디에서도 강제하지 않습니다.**
...
여기서 자연스러운 질문은 하나입니다. **막힌 자리를 어떻게 되찾는가.** 이 책은 한계의 타입 이론부터 풀지 않습니다. 막힌 자리를 직접 되찾는 세 도구 (`K<F, A>` 마커, self-bound, static abstract) 를 곧장 손에 쥡니다. 이 막힘의 정식 이름 (Order / kind / HKT) 은 도구를 모두 익힌 뒤 깊이 보기 (§2.11) 에 정리해 두었습니다. 이름을 몰라도 도구는 쓸 수 있습니다.
...
C# 이 막힌 핵심은 type constructor (`List` 라는 컨테이너 종류 자체) 를 타입 인자로 전달 못 함입니다 (§2.3 의 두 에러). 해결은 `F` 자리를 **완성 타입처럼** 사용하는 것입니다. 빈 태그 타입 (`MyListF`) 을 만들어 `List` 의 이름표 역할만 시킵니다.
...
//   F<A>     ← F 가 type constructor 매개변수   — C# 이 거부하는 자리 (§2.3)
...
정리하면 이렇습니다. C# 이 컨테이너 종류 자체를 매개변수로 못 받으니, **빈 태그 (완성 타입) 로 그 종류를 가리켜** 막힌 자리를 되찾았습니다. 이 끌어내림의 정식 이름 (kind 한 단계 내림) 과 다른 언어들의 같은 우회 패턴은 깊이 보기 (§2.11.7) 에서 봅니다.
...
>
> 둘은 **능력이 사는 자리** 가 다릅니다 (1장 §1.5). `fa.Map(f)` 는 능력이 **값(객체)** 에 삽니다 — 1장의 객체 다형성. `MyListF.Map(f, fa)` 는 능력이 **타입(trait)** 에 삽니다 — 1장의 trait 다형성. 후자라야 §2.3 의 "`F` 자리에 컨테이너 이름만 바꿔 끼움" 이 가능합니다. 값에는 이름을 "바꿔 끼울" 수 없지만, 타입 이름은 제네릭 인자로 바꿔 끼울 수 있기 때문입니다.
...
가상 코드의 가치와 막힘 (§2.3) 을 봤고, 세 우회 도구 (`K<in F, A>` 마커, self-bound, static abstract) 를 손에 쥐었습니다. 이제 컴파일되지 않던 가상의 한 줄이 **실제 빌드되는 한 줄** 로 어떻게 변환되는지 직접 비교합니다. 아래 가상 코드의 `F<_>` 는 "자료 자리가 빈 `F`" 를 뜻하는 표기입니다.

```csharp
// §2.3 가상 — F 가 type constructor 매개변수 (C# 불가)
public interface Functor<F<_>>
{
    static abstract F<B> Map<A, B>(Func<A, B> f, F<A> fa);
}

// §2.6 실제 — K<F, A> 마커 + self-bound + static abstract 로 변환 (컴파일 가능)
public interface Functor<F> where F : Functor<F>
{
    static abstract K<F, B> Map<A, B>(Func<A, B> f, K<F, A> fa);
}
...
| 자리 | 가상 (§2.3) | 실제 (§2.6) | 도구 |
|---|---|---|---|
| F 자리 | `F<_>` (type constructor 매개변수) | `F` (완성 타입 매개변수 — 빈 태그) | K 마커 (§2.4) |
| 컨테이너 어휘 | `F<A>`, `F<B>` | `K<F, A>`, `K<F, B>` | K 마커 (§2.4) |
| F 가 trait 구현체임을 보장 | (가상에서 암묵) | `where F : Functor<F>` | self-bound (§2.5) |
| 호출 입구 | `Functor<MyList>.Map(...)` | `MyListF.Map(...)` | static abstract (§2.5) |
...
> **최종 돌파** — 두 에러로 막혔던 §2.3 의 한 줄이, 이제 **IDE 에서 실제로 빌드되는 한 줄** 이 됐습니다. 격자가 실제로 얼마나 줄어드는지는 일반 함수 (§2.9) 에서 코드로 확인합니다.

> **약간의 비용** — 변환에는 두 비용이 따릅니다. (a) 3-tuple 패턴 (자료 / 태그 / trait 의 세 조각, §2.8) (b) 다운캐스트 한 줄 (`(MyList<A>)fa` 또는 LanguageExt 의 `.As()`, 정체는 §2.8.6). 대가로 Haskell / Scala 의 거의 모든 함수형 추상이 C# 어법으로 표현 가능해집니다.
...
세 도구 없이 평범한 제네릭 인터페이스로 같은 시도를 하면 어떻게 되는지 먼저 봅니다. 1장식 어법 (능력을 인터페이스로 약속하되 `A` 만 매개변수로 둠) 으로 되돌린 `SimpleFunctor<A>` 입니다. 방금 만든 `Functor<F>` (§2.6) 와 달리 `F` 를 시그니처에서 가리킬 수단이 없습니다.
...
같은 `MyList` 를 `K<F, A>` 마커와 self-bound + static abstract 로 다시 씁니다. 아래 네 조각은 §2.4~§2.6 에서 따로 본 도구를 한자리에 모은 것이라, 새 문법은 없습니다.
...
*Before* (제네릭 `SimpleFunctor<A>` 어법, §2.8.1) 가 보였던 세 약점이 *After* (`K<F, A>` + 3-tuple) 에서 차례로 해소됩니다.
...
`F` 가 어떤 Elevated World 인지 함수는 몰라도, self-bound 제약 한 줄이 한 번 정의로 모든 Functor 인스턴스에 통하게 만듭니다. 이 패턴의 전체 사용 예는 §2.9 에서 봅니다.
...
같은 함수 한 줄이 세 가지 다른 Elevated World 위에서 그대로 동작합니다. **추상을 한 번 정의하면 N 개 인스턴스가 자동으로 따라오는**, trait 의 진짜 이득이 여기서 드러납니다. 처음의 N×M 격자 (§2.2) 가 N+M 으로 실제 줄어드는 자리입니다.
...
이 장은 §2.2 의 불편 (N×M 격자) 에서 출발해 §2.9 의 일반 함수 (N+M) 로 끝났습니다. 그 여정에서 익힌 도구를 1장의 두 평행 세계 / 4 가지 함수 유형에 다시 매핑하면, 2장이 1장 비유의 **C# 구현체** 였음이 드러납니다.
...
> **선택적 심화** — 이 절은 §2.3 의 막힘에 정식 이름 (Order / kind / HKT) 을 붙입니다. 세 도구를 쓰는 데 필수는 아니므로, 건너뛰고 Q&A (§2.12) 로 가도 4장 이후를 읽는 데 지장이 없습니다. "왜 하필 그 자리가 막히는가" 의 뿌리가 궁금할 때 돌아오면 됩니다.
...
> ```
>
> `Negate` 는 인자가 하나이고 값 (int) 이라 Order 1 에 머뭅니다. `Apply` 는 같은 1-인자지만 인자가 함수 (`int → int`) 라 Order 2 로 올라갑니다. `Add` 는 인자가 둘이어도 모두 값이라 여전히 Order 1 입니다. **몇 개** 가 아니라 **어떤 어휘가 인자 자리에 들어오는가** 가 *Higher* 를 가릅니다. 이 구분은 타입 차원에서 다시 결정적으로 쓰입니다 (`Dictionary<K, V>` 가 2-인자지만 HKT 가 아닌 이유, §2.11.2).
...
kind 화살표는 함수 시그니처와 같은 법으로 읽습니다. 왼쪽이 입력, 오른쪽이 출력입니다. `List` 가 `int` 를 받아 `List<int>` 를 만드니 `* → *` 이고, 그 type constructor 전체를 다시 입력으로 받는 자리가 `(* → *) → *` (HKT) 입니다. 입력 자리가 두 개인 `* → * → *` (`Dictionary<K, V>`, `Either<L, R>`) 는 인자가 모두 완성 타입이라 HKT 가 아닙니다. 다인자 ≠ 고차의 판별 규칙 (§2.11.1) 이 타입 차원에도 그대로 적용됩니다. 결국 화살표의 **왼쪽 자리** 가 Order 를 가릅니다. 왼쪽이 `*` 면 Order 1, `(* → *)` 면 Order 2 (HKT) 로 올라갑니다.
...
결론은 한 줄입니다. **`F` 가 이미 한 단계 위 어휘 (`* → *`) 라서, 그것을 매개변수로 받는 `Functor` 는 한 단계 더 위 (`(* → *) → *`) 로 올라갑니다.** 아래 세 단계는 그 한 줄의 형식 확인이므로, 결론만 들고 통합 비교 (§2.11.6) 로 건너뛰어도 됩니다.
...
세 단계의 결론은 한 줄입니다. `Functor<F>` 의 kind `(* → *) → *` 가 **2 장의 핵심 목표** (Order 2, HKT) 자리이고, **C# 어법에는 바로 이 자리가 없습니다**. 앞서 본 두 에러 (§2.3) 가 정확히 이 자리에서 났습니다.
...
코드의 `static abstract` 는 §2.5 에서 이미 익힌 도구입니다. 지금 자리의 핵심은 하나입니다. `F<B>` / `F<A>` 어법이 거부됩니다.
...
> **Q1. 객체지향의 N×M 비용과 함수형의 N+M 비용 차이는?** (§2.2)

객체지향에서 능력은 객체 (컨테이너 클래스) 의 인스턴스 메서드입니다. `List<A>.Map`, `Option<A>.Map`, `Task<A>.Map` 셋이 모양만 같고 자리가 달라 N 개 컨테이너 × M 개 능력 = N×M 개의 별개 구현이 필요합니다 (§2.2 의 격자). 함수형은 능력이 사는 자리를 *trait* 으로 옮깁니다. `Functor<F>` 한 trait 에 `Map` 한 번 정의하고 `F` 자리에 컨테이너를 끼우면 N+M 비용으로 줄어듭니다.

> **Q2. C# 으로 `interface Functor<F>` 를 적고 `F<B> Map(...)` 을 쓰면 왜 막힙니까? 이 한계의 정식 이름은?** (§2.3)

**CS0307** (`F` 가 다시 인자를 받는 자리에서 사용 불가) 과 **CS0246** (`F` 를 완성 타입으로 해석하려 했으나 못 찾음) 두 에러가 납니다. 뿌리는 하나입니다. C# 의 제네릭 매개변수는 완성 타입만 받아, `F<...>` 처럼 `F` 가 다시 인자를 받는 어법이 없습니다. 이 한계의 정식 이름은 Order 2 (Higher-Kinded Type) 미지원, 학술 어휘로 *higher-kinded polymorphism* 미지원이며, 그 도출은 깊이 보기 (§2.11) 에 정리했습니다.

> **Q3. `K<F, A>` 의 멤버 수와 발상은?** (§2.4)
...
> **Q4. `where F : Functor<F>` (self-bound) 가 없으면 무엇이 안 되나? 순환 정의 아닌가?** (§2.5)
...
> **Q5. `static abstract` 호출이 인스턴스 호출과 다른 점은?** (§2.5)
...
> **Q6. 3-tuple 패턴의 세 조각은? `K<F, A>` 는 왜 4 번째 조각이 아닌가?** (§2.8)
...
- **C# 으로 직접 풀려다 두 에러에 부딪혔습니다.** `interface Functor<F>` 의 `F<A>` / `F<B>` 자리에서 CS0307, CS0246 이 났습니다 (§2.3). 제네릭 매개변수가 완성 타입만 받는 어법의 막힘입니다.
- **막힌 자리를 세 도구로 우회했습니다.** ① `K<in F, A>` 빈 인터페이스가 C# 이 거부하는 `F<A>` 자리를 완성 타입 어법으로 끌어내려 되찾고, ② self-bound (`where F : Functor<F>`) 가 F 자리에 함수 호출을 가능하게 하고, ③ static abstract (C# 11+) 가 호출 입구를 타입 이름에 둡니다 (§2.4 ~ §2.5). 세 도구는 Lightweight Higher-Kinded Polymorphism (Yallop & White, 2014) 의 brand 인코딩의 C# 표현입니다.
- **가상 한 줄이 실제 한 줄로 변환됐습니다.** F 자리 / 컨테이너 어휘 / 호출 입구 세 곳이 일대일 매핑되어, §2.2 의 N×M 비용이 실제 컴파일 가능한 N+M 비용으로 돌파됩니다 (§2.6).
- **새 컨테이너의 입회 절차는 3-tuple 패턴** (자료 / 태그 / trait 의 세 조각) 입니다 (§2.8). 한 번 등록되면 어떤 Functor 든 받는 일반 함수 (`FunctorOps.Run`) 가 그 컨테이너에 자동 적용됩니다 (§2.9).
- **한계의 정식 이름은 깊이 보기에 정리했습니다.** 값 차원의 Order 0/1/2 가 타입 차원의 kind (`*` / `* → *` / `(* → *) → *`) 에 평행이고, C# 은 타입 차원의 Order 2 (HKT) 만 지원하지 않습니다. §2.3 두 에러의 정체입니다 (§2.11).
