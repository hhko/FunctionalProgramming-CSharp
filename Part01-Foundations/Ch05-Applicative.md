...
5장의 핵심은 한 줄로 압축됩니다. Normal World 의 N 인자 함수를 Elevated World 에서 그대로 사용할 수 있도록 제공하는 lift 함수가 `pure` 와 `apply` 입니다. 4장 *map* 이 1 인자 lift 였다면, 5장 *pure + apply* 는 N 인자 lift 입니다. 두 멤버 위에서 `Lift2`, `Lift3`, `Lift4` 같은 다인자 헬퍼가 자랍니다.

> 5장의 3부 구조 — 4장과 같은 narrative arc 로 구성됩니다.
>
> - **§5.1 목적** — §1.6.1 의 N 인자 lift 자리. `map` 의 1 인자 한계 + 다인자 함수를 Elevated 결합하는 패턴을 *pure + apply* 한 번 정의로 해소.
...
#### 첫 번째 단계 — `Map` 의 시그니처는 1 인자 함수만 받습니다
...
**그림 5-2. `Pure`: Normal 값을 Elevated 의 가장 단순한 시민으로** — Wlaschin 의 `return` 다이어그램을 따라간 좌우 대비 시각. 좌측 (Pure 전). Normal World 에 평범한 값 `a` 가 활성, Elevated 의 `K<F, a>` 자리는 아직 비어 (흐릿) 있습니다. 가운데 파란 `Pure` 화살표 → 우측 (Pure 후) 에서 Elevated 의 `K<F, a>` 가 활성, Normal 의 `a` 는 흐릿. **`Pure : a → K<F, a>`** 가 끌어올림 사다리의 가장 짧은 한 칸입니다.
...
**그림 5-3. `Apply`: 컨테이너 안 함수를 lifted function 으로 풀어냄** — Wlaschin 의 `apply` 다이어그램을 따라간 unpacking 시각. 단일 Elevated World 띠 안에서 좌측 `K<F, a → b>` (컨테이너 안에 갇힌 함수) 한 박스 → 파란 `apply` 화살표 → 우측 `K<F, a> → K<F, b>` 의 *Elevated lifted function*. **시그니처는 curried 형. `apply : K<F, a → b> → ( K<F, a> → K<F, b> )`**. 함수 한 개를 받아 Elevated 의 함수 한 개를 돌려주는 구조가 `Apply` 의 본질입니다.
...
세 어법 모두 `where F : Applicative<F>` 제약으로 자동 디스패치됩니다. 호출 측은 어느 쪽을 골라도 같은 `MyMaybeF.Apply` 로 흘러갑니다. `r1`, `r2`, `r3` 의 값은 동일합니다.
...
> 참고 — 실전 코드는 `Lift2` 한 줄
>
> 위 2 단계 사슬은 학습 명료성을 위해 단계를 펼친 형태입니다. 실전에서는 `Lift2` 한 줄로 같은 결과를 냅니다. 2 인자 함수를 원래 모양 그대로 넘기고, Curry 와 Apply 사슬은 헬퍼가 알아서 합니다.
>
> ```csharp
> // 실전 압축형 — Curry 도 Apply 사슬도 헬퍼 안으로
> K<MyMaybeF, int> r = Lift.Lift2<MyMaybeF, int, int, int>(
>     (x, y) => x + y,    // 2 인자 Normal 함수 그대로 (수동 Curry 불필요)
>     five, three);
...
> | 사용자가 직접 curried 함수를 만들어 넘김 | 2 인자 함수 그대로 넘김 — Curry 가 내부 자동 |
> | 2 단계 (Map + Apply) 가 코드에 노출 | 1 단계 (Lift2) — Curry → Pure → Apply 사슬을 캡슐화 |
> | 각 단계의 시그니처 변화가 보임 | 결과만 보임 (단계는 헬퍼 안) |
>
> 두 코드 모두 시그니처와 결과 (`Just(8)`) 가 동일합니다. **학습 단계에서는 펼친 형태로 무엇이 일어나는지 보고, 실무 코드에서는 압축형을 씁니다**. `Lift2` 의 4 단계 분해 (§5.5.2) `Curry → Pure → Apply → Apply` 가 바로 이 압축의 내부입니다.
...
>
> **둘째, `mf` 안의 함수는 1 인자** — `Map(addC, five)` 의 결과인 `y => 5 + y` 는 매개변수 `y` 한 개, 출력 `5 + y` 한 개. 타입은 `int → int` (즉 `a → b`). `Apply` 의 시그니처 `K<F, a → b>` 에 정확히 맞습니다.
>
...
> ```
>
> **2 인자 `(a, b) → c` (§5.1.1) 는 어디로 갔나요?** — `addC` 가 그 `add` 의 curried 형태입니다. `Map(addC, five)` 가 첫 인자 `5` 를 소비해 2 인자가 1 인자로 줄어든 wrapped 함수 (`mf`) 를 만듭니다. 이 자리에서 시그니처가 `(a, b) → c` 에서 `a → b` 로 바뀐 셈입니다. `Apply(mf, three)` 가 마지막 1 인자를 소비해 결합 완성. **`Lift2` (§5.5.2) 는 이 2 단계 (`Map → Apply`) 를 `Pure → Apply → Apply` 의 3 단계로 일반화** 합니다. 어떤 N 인자 함수든 같은 패턴입니다.
...
두 필드 (`CustomerId`, `EmailAddress`) 의 검증 결과로 `CustomerInfo` 를 만드는 자리는 효과 인코딩 + 일반 연산 합성의 정통 N 인자 확장 예제입니다. *Lift2 / Lift3 / Lift4* 의 결정적 가치는 N 회 Apply 라는 문법적 형태가 아니라 **세 책임의 깔끔한 분리** 에 있습니다. 효과는 타입, 연산은 함수, 합성은 Lift 입니다. 4장 *Map* 이 하나의 효과 위에서 1 인자 일반 함수를 끌어올렸다면, 여기서는 두 (또는 N 개) 효과 위에서 N 인자 일반 함수를 끌어올립니다.

- **효과 인코딩 — `MyValidation<E, A>`** — 각 필드의 검증은 통과할 수도 있고 실패할 수도 있습니다 (`42` 는 통과, `-1` 은 실패). 이 통과/실패 가능성 + 에러 누적 자체를 **타입에 인코딩** 하는 자리가 `MyValidation<string, T>` 입니다. 호출자가 분기 검사를 잊을 수 없도록 시그니처가 강제합니다. 효과는 타입의 책임입니다.
...
- **합성 — `Lift2(createCustomer, …, …)`** — *Lift2* 한 줄이 2 인자 일반 함수를 효과 두 분기 위로 승격시킵니다. 둘 다 성공 (`Valid`) 이면 결합 결과 생성, 한 측이라도 실패 (`Invalid`) 면 에러 누적입니다. 분기 코드 `if (id.IsValid && email.IsValid) ...` 자체가 사라집니다. 합성은 Lift 의 책임입니다.
...
| **일반 연산은 N 인자 평범한 함수** | `createCustomer = (id, email) => new CustomerInfo(id, email)` 는 효과를 모르는 단순한 2 인자 생성자. Lift3 / Lift4 도 같은 발상의 확장 — `(n, e, a, t) => new User(...)` 같은 N 인자 생성자에 `LiftN` 한 줄로 끌어올림 적용 (4 인자 회원가입 예제 (§5.4.3)). 생성자 코드는 검증 어법 없이 한 번만 작성. |
| **합성은 Lift 한 줄** | `Lift2(f, x, y)` 가 효과 (통과/실패) 두 분기 처리 + 두 측 에러 누적과 2 인자 일반 연산 적용을 한 줄로 합칩니다. 명령형의 `if (x.IsValid && y.IsValid) …` / 두 에러 합치는 boilerplate 가 사라짐. |

명령형 코드라면 두 검증 결과의 조합 + 에러 누적 + 생성자 호출이 함수 본문 안에 섞여서 작성됩니다. 함수형은 **효과는 타입, 연산은 N 인자 함수, 합성은 Lift** 의 세 책임이 깔끔히 나뉩니다. 독자가 작성하는 건 늘 효과를 모르는 N 인자 일반 함수뿐, N 개 효과의 결합 + 에러 누적 어법은 trait 의 `Apply` 가 자동으로 입혀 줍니다. 1장에서 정리한 결정적 통찰이 N 인자 자리로 확장되는 자리입니다.

![Lift2 — 2 인자 함수의 curried 사슬 끌어올림](./images/Ch05-Applicative/05-customer-validation.svg)

**그림 5-5. Lift2 — 2 인자 함수의 curried 사슬 끌어올림** — 4장의 1 인자 lift (`Map : (a → b) → (E<a> → E<b>)`) 가 N 인자로 확장된 형태입니다. 아래 행 Normal World 에 `createCustomer : CustomerId → Email → CustomerInfo` 의 curried 사슬 (세 박스 + 두 가로 화살표). 위 행 Elevated World 에 끌어올린 curried 사슬 `MyValidation<…, CustomerId> → MyValidation<…, Email> → MyValidation<…, CustomerInfo>` (세 박스 + 두 가로 화살표). 좌·우 파란 lift 실선 화살표가 양 끝 박스 (`CustomerId` 와 `CustomerInfo`) 의 lift 자리를 표시합니다. 가운데 점선 lift 화살표가 Lift2 함수 자체로, 2 인자 Normal 함수 `createCustomer` 를 받아 두 MyValidation 결합 함수로 승격시킵니다. **Lift2 의 가치는 인자 수가 늘어도 lift 패턴이 한 줄로 유지된다는 점에 있습니다**. 효과 결합 + 에러 누적은 trait 의 `Apply` 가 자동으로 처리합니다.

4장의 ParseInt + Map(f) 가 하나의 효과 위 1 인자 lift 였다면, 여기 ValidateId × ValidateEmail + Lift2(createCustomer) 는 두 효과 위 2 인자 lift 입니다. 효과 개수 + 일반 함수 인자 수가 같이 늘어나도 세 책임의 분리는 그대로 유지됩니다. `LiftN` 한 줄이 N 개의 분기 + 에러 누적을 자동으로 입혀 줍니다.

---

## 5.5 어떤 Applicative 든 받는 일반 함수 — 예제 (Curry + Lift)

5 장의 핵심 절은 함수형의 본질인 lift 의 다인자 확장을 코드로 보여 주는 자리입니다. 4장 *map* 이 1 인자 lift `(a → b) → (E<a> → E<b>)` 였다면, *Lift2 / Lift3 / Lift4* 는 같은 발상의 2 / 3 / 4 인자 확장입니다. 두 도구 `Curry` 와 `Apply` 가 다인자 함수를 한 인자씩 받는 사슬로 비틀어 N 회 Apply 로 끌어올립니다.
...
**그림 5-6. `Lift2`: Normal 의 curried 함수 사슬을 Elevated 로 통째 끌어올림** — Wlaschin 의 `lift2` 다이어그램을 따라간 상태 전환 시각. 좌측 (Lift2 전) 에서 Normal World 의 `a → b → c` 사슬이 활성, Elevated 의 `K<F, a> → K<F, b> → K<F, c>` 자리는 흐릿. 가운데 파란 `Lift2` 화살표 → 우측 (Lift2 후) 에서 Elevated 의 lifted curried function chain 이 활성, Normal 은 흐릿. **`Lift2` 는 curried 함수 사슬 전체의 World 이동** 으로, 인자가 한 개씩 들어올 때마다 사슬이 한 칸씩 줄어드는 부분 적용 가능성이 Elevated 안에서 그대로 보존됩니다.

> 참고 — 실전 LanguageExt 의 구현 방향은 다릅니다
>
> 위 4 단계 분해는 학습 명료성을 위해 `Curry.Of` 를 컨테이너 밖에서 명시적으로 호출합니다. 실전 LanguageExt v5 의 `Applicative.lift<F, A, B, C>` 는 같은 결과를 컨테이너 안 currying 으로 압축한 한 줄입니다.
>
> ```csharp
...
> - **사용자 어법** — 이 책은 `Lift2` 가 4 단계 분해를 그대로 보입니다. 라이브러리는 `lift(f, fa, fb)` 한 줄이 현장 사용 어법으로 압축됩니다.
>
> ```csharp
...
> ```
>
> 결과·의미는 이 책의 4 단계 분해와 동일합니다. Pure ∘ Curry → Apply → Apply 의 등식이 두 방향에서 모두 성립합니다. 이 책의 명시적 4 단계는 어떤 일이 일어나는지의 학습용 펼침이고, 라이브러리 한 줄은 현장 사용 어법으로의 압축입니다. 두 접근이 시그니처 단계에서 같은 함수임을 5장 코드의 `Lift.Lift2` 와 LanguageExt 의 `Applicative.lift` 를 나란히 호출해 검증할 수 있습니다.
...
**그림 5-7. `Lift3`: 4박스 사슬의 통째 끌어올림** — Lift2 와 같은 상태 전환 시각의 인자 N 개 일반화. 좌측 (Lift3 전). Normal 의 `a → b → c → d` 사슬 활성. 가운데 파란 `Lift3` → 우측 (Lift3 후) 에서 Elevated 의 `K<F, a> → K<F, b> → K<F, c> → K<F, d>` 활성. **인자 개수가 늘어도 패턴은 한 줄로 유지**. `Lift4` 는 5박스, `LiftN` 은 (N+1) 박스의 같은 시각.
...
`Lift2Curried` 의 시그니처가 Normal 함수 `(a, b) → c` 를 Elevated 함수 `K<F, a> → K<F, b> → K<F, c>` 로 끌어올린다는 의도를 그대로 담습니다. **elevation 의 본질은 함수 자체가 Elevated 세계에 사는 것** 입니다. *lift* 한 동사가 가리키는 것이 정확히 이 자리입니다.
...
`Lift1 = Map` 의 시각적 근거는 그림 5-4 의 `Map ≡ Apply ∘ Pure` 두 경로 등식이 그대로 `Lift1` 이 진짜 Applicative 의 능력을 쓰지 않는다는 사실의 시각화입니다. 그림의 우측 (Map 경로) 이 정확히 `Lift1(f)(fa)` 의 동작, 좌측 (Apply 경로) 이 `Lift2 / Lift3` 에서 본격적으로 살아나는 *Pure + Apply* 사슬의 가장 짧은 한 단계 (1 인자) 입니다.
...
Elevated 함수 + 끌어올린 Normal 값의 결합이, 그 값을 미리 묶은 적용 함수 + Elevated 함수의 결합과 같습니다. **`Apply` 의 좌우 비대칭이 사라지는 자리** 입니다. 함수 측과 값 측이 어디 있든 결과가 일치합니다.
...
> 참고 (LanguageExt v5 의 다섯 법칙 정렬) — LanguageExt `ApplicativeLaw<F>.validate(...)` 가 검증하는 다섯 법칙 중 마지막이 바로 이 *functorLaw* 입니다. 이 책의 다섯 절 (`Identity / Homomorphism / Interchange / Composition / Functor 정합`) 이 라이브러리의 다섯 법칙과 시그니처·의미 단계에서 완전히 정렬됩니다.
...
시그니처는 통과하지만 항등 법칙이 깨집니다. 부작용은 다음과 같습니다. Pure 한 함수를 끌어올려 Apply 하는 모든 일반 함수가 잘못된 결과를 냅니다.
...
값은 같지만 몇 번 들었는지가 다릅니다. 동형 사상 법칙 위반입니다. 부작용은 `Lift2` 의 결과 크기가 예상과 다릅니다.
...
시그니처가 같은 모양인데 결과 의미가 달라지는 자리입니다. Identity 는 `Pure(id)` 의 단순화를, Homomorphism 은 `Pure` 의 기준점을, Interchange / Composition 은 `Apply` 의 결합 형태 자유도를, Functor 정합은 상속한 `Map` 의 일관성을 각각 책임집니다. **다섯 법칙 모두 만족할 때만** `Apply` 가 **진짜 Applicative** 입니다. 시그니처를 통과한 모든 추론이 안전한 trait 입니다.
...
`Applicative<F> : Functor<F>` 의 상속이 시그니처 단계에서 의미하는 것은 `Map` 이 자동으로 따라온다는 점입니다. `Map ≡ Apply ∘ Pure` 의 등식이 그 한 줄 증명입니다. 새 자료 타입에 `Pure + Apply` 를 정의하면 `Map` 의 동작이 자동으로 일관됩니다.
...
Apply 가 Map 의 일반화입니다. 함수가 Elevated 한 칸 위로 올라간 모양입니다. Pure 가 Apply 의 좌측 입력 (`K<F, Func<A, B>>`) 을 만들 수 있게 해주는 보조 도구입니다. 세 시그니처가 한 가족입니다.
...
`Pure` 가 0 인자 lift 입니다. 끌어올릴 함수가 없는 자리이고, 값 자체만 끌어올립니다.
...
4장에서 `MyList<A>` 에 Functor 를 부착했습니다. 이제 같은 자료 타입에 Applicative 를 부착합니다. `Apply` 의 결합 의미는 *cartesian product* 입니다.
...
`Pair<W, A>` 가 부수 로그 W 와 값 A 의 쌍. `Pure` 가 `(W.Empty, value)`, `Apply` 가 `(w1 <> w2, f(a))` — 로그를 monoid 로 누적 합니다. 3 부 Reader / Writer 의 출발점.
...
5장이 다루는 네 자리 중 하나는 `E<a> → E<b>` 의 다인자 확장 + ⓟ 사다리 (`a → E<a>`). 두 Elevated 시민의 결합이 본격적으로 가능해지는 자리 — Wlaschin 의 용어로는 *unpacking a wrapped function*, *combining lifted values* 입니다.
...
부분적으로는 가능합니다. `Map(add, two)` 의 결과 `K<F, Func<int, int>>` 에 세 번째 함수 (`Apply`) 를 한 번 더 적용하면 됩니다. 그 한 번 더가 정확히 `Apply` 의 시그니처 (컨테이너 안 함수와 컨테이너 안 값의 결합) 입니다. 즉 `Lift2(f, fa, fb) = Map(f, fa).Apply(fb)` 가 같은 등식입니다. `Map` 만으로는 끌어올림이 반쪽입니다. `Apply` 가 그 반쪽을 닫는 도구입니다.
...
일반 `Func<A, B>` 라면 그 시그니처는 `Func<A, B> → K<F, A> → K<F, B>` 가 되어 정확히 Functor 의 Map 과 같습니다. 같은 trait 의 두 멤버가 되는 셈입니다. 함수가 컨테이너 안에 들어가야 그 함수 자체의 Elevated 효과 (Maybe 의 Nothing, Validation 의 Invalid) 를 표현할 수 있고, 다인자 함수의 부분 적용 결과 (`Map` 한 번 적용한 후의 모양) 가 자연스럽게 `Apply` 의 입력이 됩니다. **효과의 대칭성 + 다인자 결합** 이 함수가 컨테이너 안에 있어야 하는 이유입니다.
...
```
4장 — Functor:       (a → b) → (E<a> → E<b>)                map        (1 인자 lift)
...
> **실무 디딤돌** — Applicative 의 `Pure` / `Apply` 는 8 장 Validation 의 **오류 누적** 패턴과 5 부 `Eff` 의 효과 합성에서 **Monad 없이 N 인자 lift** 가 필요한 자리의 원형입니다.
>
> **테스트 디딤돌** — 다섯 법칙 (§5.6, Identity / Homomorphism / Interchange / Composition / Functor consistency) 은 9 부의 **합법칙 검증** (xUnit + Shouldly) 표준으로 옮겨갑니다. `ApplicativeLaws.cs` 가 그 출발점입니다.
