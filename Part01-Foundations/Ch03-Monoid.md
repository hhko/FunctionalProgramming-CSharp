...
**`static virtual A operator +`** — default 연산자. `+` 가 모든 Semigroup 구현체에 자동 제공되는 자리입니다. 본문은 한 줄, `lhs.Combine(rhs)` 의 위임입니다. 즉 **`+` 는 `Combine` 의 syntactic sugar** 입니다. 구현체가 `Combine` 한 메서드만 정의하면 `+` 가 자동으로 작동합니다. Haskell 의 `<>` 연산자가 C# 의 `+` 로 옮겨오는 자리입니다. 다만 trait 의 default `+` 는 *generic 제약 context 안에서만* 직접 호출됩니다 (`where M : Semigroup<M>` 안의 `a + b`). 구체 타입 `Sum + Sum` 직접 호출을 위해서는 `Functions/SemigroupExtensions.cs` 의 *extension static operator +* 가 필요합니다 (다음 §3.6.5).

**`static virtual SemigroupInstance<A> Instance`** — trait 을 record 형태로 노출. v5 의 결정적 어법으로, trait 을 **값처럼 전달** 할 수 있게 합니다. C# 의 generic 제약은 *제약 자체를 인자로 못 받는* 한계가 있는데, 이 한계를 우회하는 자리입니다. `record SemigroupInstance<A>(Func<A, A, A> Combine)` 형태이고, `Semigroup.combine` static helper 가 `Combine` 필드의 기본값입니다. `Monoid.instance<A>()` 같은 helper 로 가져와 `instance.Combine(x, y)` 어법으로 호출합니다.

`[Pure]` attribute — 결합 연산이 부작용 없는 순수 함수임을 약속. 모든 멤버에 명시해 *side-effect 가 없는 자리* 를 정적 분석 도구가 검증 가능하게 합니다 (`using System.Diagnostics.Contracts;`).
...
**`Monoid<A> : Semigroup<A>`** — trait 상속. Monoid 가 Semigroup 의 모든 능력 (`Combine` + default `+` + `Instance`) 을 물려받고 두 멤버를 더한다는 어법입니다. 2 장 §2.12 에서 봤던 trait 들 사이의 상속 관계가 처음 등장하는 자리이고, 5 장 Applicative (Functor 의 자식), 7 장 Monad (Applicative 의 자식) 같은 1 부의 다른 trait 사슬의 원형입니다.
...
`Semigroup<A>` 의 `static virtual A operator +` default 는 **generic 제약 context 안에서만** 직접 호출됩니다. 즉 `where M : Semigroup<M>` 안의 `a + b` 는 동작하지만, `Sum(1) + Sum(2)` 같은 *구체 타입 직접 호출* 은 막힙니다. 이 자리를 v5 의 정통 어법 *extension static operator* 가 풉니다.
...
C# 14 의 *extension static* 자리에 *operator +* 를 두고, *어떤 Semigroup 구현체에도* `+` 가 자동 적용됩니다. 자료 타입 자체에는 `operator +` 명시가 필요 없고 (Sum, Product, Concat 등 모두), `SemigroupExtensions` 한 파일이 모든 구현체에 직접 호출 어법을 제공합니다. v5 의 `LanguageExt.Core/Traits/Semigroup/Semigroup.Operators.cs` 와 정확히 정합한 자리입니다.
...
자료 타입에 `operator +` 명시가 **없는** 자리에 주의합니다. v5 정통 어법으로 §3.6.5 의 `SemigroupExtensions` 의 *extension static operator +* 가 모든 Semigroup 구현체에 자동으로 `+` 를 제공합니다. 자료 타입 자체는 두 멤버 (`Combine` + `Empty`) 만 정의하면 됩니다. `Sum(1) + Sum(2)` 같은 직접 호출이 extension operator 로 작동합니다.
...
여러 자료 / 두 어휘 / 한 패턴. Sum, Product, Concat 세 자료 타입이 모두 **같은 두 멤버 (`Empty` + `Combine`)** 를 정의하고, 같은 일반 함수 (`Monoid.combine`) 가 셋 모두에 자동 적용됩니다. trait 한 정의 + 부착 N 번이 N×M 비용을 N+M 으로 줄이는 자리입니다 (1 장 §1.7.2 의 N+5 비용 재현).
...
§3.8 의 다양한 자료 타입에 같은 누적 함수가 적용되는 모양을 한 자리에서 보겠습니다. 어떤 Monoid 든 받는 일반 함수입니다. v5 의 정통 어법은 `Monoid.combine<A>(IEnumerable<A>)` 입니다.
...
`Monoid.combine` 자유 함수의 자리는 6 장 Foldable 의 디딤돌이기도 합니다. Foldable 은 **컨테이너 (`E<a>`) 안의 구조를 Normal World 의 한 값으로 끌어내리는** 추상이고, 그 끌어내림의 출발점이 **어떤 값으로 시작해 무엇으로 합칠 것인가** 의 결정입니다. `Monoid.combine` 이 그 결정을 **Monoid 의 `Empty` + `Combine`** 으로 묶은 자리이고, 6 장에서 같은 발상에 **컨테이너 모양** (`E<a>`) 을 추가하면 Foldable 의 시그니처로 자연스럽게 진화합니다.

```text
Ch03 Monoid.combine  : Monoid<A> 의 [A] → A           (Normal World 안에서)
Ch06 fold            : Monoid<A> 의 E<A> → A          (Elevated World 의 E<a> 에서 Normal 의 A 로)
```

`Monoid.combine` 의 자료 타입이 `IEnumerable<A>` 에서 **임의의 컨테이너 `E<A>`** 으로 일반화되는 자리입니다. 3 장의 결합 어법 + 6 장의 컨테이너 어법이 만나 **Foldable 의 통합 시그니처** 가 등장합니다.
...
- **`Monoid.combine`** — Foldable 의 디딤돌. 어떤 Monoid 든 받는 일반 함수 한 정의가 모든 Monoid 인스턴스에 자동 적용됩니다. 1 장 §1.7.2 의 trait 부착 ROI 의 Order 0 원형이고, 6 장 Foldable 의 `fold` 가 **임의의 컨테이너 `E<A>`** 로 일반화하는 출발점입니다.
...
이 장에서 결합 어법을 손에 잡았습니다. self-bound + `static abstract` 두 도구를 **컨테이너 없는 가장 단순한 형태** 로 익혔고, 어떤 자료 타입에든 두 멤버 (`Empty` + `Combine`) 를 정의하면 일반 함수 (`Monoid.combine`) 가 자동 따라오는 ROI 도 봤습니다. 다만 3 장의 결합은 **Normal World 안의 자리** 였습니다. Elevated World 의 시민 (`Option<a>`, `MyList<a>`) 위로 결합 어법을 끌어올리는 자리가 4 장부터 본격적으로 등장합니다.
