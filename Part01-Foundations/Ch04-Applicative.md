...
> - **§4.2 *기능*** — `Applicative<F>` trait 의 시그니처와 `Pure` + `Apply` 두 멤버. Functor 상속 + 시그니처가 다인자 결합을 컴파일 타임에 강제.
...
3장에서 본 `Map` 의 첫 자리는 1인자 Normal 함수입니다.
...
`Apply` 가 두 컨테이너를 동시에 다룬다는 점이 `Map` 과의 결정적 차이입니다. `MyMaybe.Apply` 의 네 분기 (§4.3.3) 가 정확히 두 컨테이너 × 두 상태 (`Just` / `Nothing`) = 4가지 조합의 직접 표현입니다.
...
마지막 줄이 결정적입니다. `MyMaybe` 의 Apply 는 둘 다 Nothing 이라도 한 Nothing 만 돌려줍니다. 오류 정보가 한 자리만 있어 모을 곳이 없기 때문입니다. §4.4 의 `MyValidation` 이이 자리를 누적으로 바꿉니다.

### 4.3.4 호출 모양 — 세 가지 어법

§4.1.1 의 `add : (int, int) → int` 를 curried 형태로 비틀어 두면, 첫 인자를 `Map` 으로 적용한 결과가 정확히 두 번째 인자를 기다리는 wrapped 함수. 그 wrapped 함수에 `Apply` 한 번이면 결합이 끝납니다. **§4.1.1 의 약속이 비로소 풀리는 자리** 입니다.
...
>
> 두 코드 모두 시그니처와 결과 (`Just(8)`) 가 동일. **학습 단계에서는 펼친 형태로 무엇이 일어나는지 보고, 실무 코드에서는 압축형을 씁니다**. §4.5.2 에서 `Lift2` 의 4단계 분해 (`Curry → Pure → Apply → Apply`) 가 바로이 압축의 내부.
...
> ```
>
> **§4.1.1 의 2인자 `(a, b) → c` 는 어디로 갔나요?** — `addC` 가 그 `add` 의 curried 형태. `Map(addC, five)` 가 첫 인자 `5` 를 소비해 2인자가 1인자로 줄어든 wrapped 함수 (`mf`) 를 만듭니다. 이 자리에서 시그니처가 `(a, b) → c` 에서 `a → b` 로 바뀐 셈. `Apply(mf, three)` 가 마지막 1인자를 소비해 결합 완성. **§4.5.2 의 `Lift2` 는이 2단계 (`Map → Apply`) 를 `Pure → Apply → Apply` 의 3단계로 일반화** 합니다. 어떤 N인자 함수든 같은 패턴.
...
함수 측이 Nothing 이면 값을 보지도 않고 결과 Nothing. 어느 쪽이든 Nothing 이면 결과 Nothing 의 단락 동작입니다. 사용자에게 왜 실패했는지의 정보가 없다는 점이 7장 Validation 에서 누적으로 바뀝니다.
...
두 필드 (`CustomerId`, `EmailAddress`) 의 검증 결과로 `CustomerInfo` 를 만드는 자리는 효과 인코딩 + 일반 연산 합성의 정통 N 인자 확장 예제입니다. *Lift2 / Lift3 / Lift4* 의 결정적 가치는 N 회 Apply 라는 문법적 형태가 아니라 **세 책임의 깔끔한 분리** 에 있습니다 — 효과는 타입 / 연산은 함수 / 합성은 Lift. 3장 *Map* 이 하나의 효과 위에서 1인자 일반 함수를 끌어올렸다면, 여기서는 두 (또는 N 개) 효과 위에서 N 인자 일반 함수를 끌어올립니다.
...
**그림 4-5. Lift2 — 2인자 함수의 curried 사슬 끌어올림** — 3장의 1인자 lift (`Map : (a → b) → (E<a> → E<b>)`) 가 N 인자로 확장된 형태입니다. 아래 행 Normal World 에 `createCustomer : CustomerId → Email → CustomerInfo` 의 curried 사슬 (세 박스 + 두 가로 화살표). 위 행 Elevated World 에 끌어올린 curried 사슬 `MyValidation<…, CustomerId> → MyValidation<…, Email> → MyValidation<…, CustomerInfo>` (세 박스 + 두 가로 화살표). 좌·우 파란 lift 실선 화살표가 양 끝 박스 (`CustomerId` 와 `CustomerInfo`) 의 lift 자리를 표시합니다. 가운데 점선 lift 화살표가 Lift2 함수 자체로, 2인자 Normal 함수 `createCustomer` 를 받아 두 MyValidation 결합 함수로 승격시킵니다. **Lift2 의 가치는 인자 수가 늘어도 lift 패턴이 한 줄로 유지된다는 점에 있습니다**. 효과 결합 + 에러 누적은 trait 의 `Apply` 가 자동으로 처리합니다.

3장의 ParseInt + Map(f) (§3.4.5) 가 하나의 효과 위 1인자 lift 였다면, 여기 ValidateId × ValidateEmail + Lift2(createCustomer) 는 두 효과 위 2인자 lift 입니다. 효과 개수 + 일반 함수 인자 수가 같이 늘어나도 세 책임의 분리는 그대로 — `LiftN` 한 줄이 N 개의 분기 + 에러 누적을 자동으로 입혀 줍니다.

---

## §4.5 어떤 Applicative 든 받는 일반 함수 — 예제 (Curry + Lift)

§4.5 가 함수형의 본질 — 합성 가능한 Elevated World 로 lift (§1.6.1) 의 lift 의 다인자 확장을 코드로 보여 주는 자리입니다. 3장 *map* 이 1인자 lift `(a → b) → (E<a> → E<b>)` 였다면, *Lift2 / Lift3 / Lift4* 는 같은 발상의 2 / 3 / 4인자 확장입니다. 두 도구 `Curry` 와 `Apply` 가 다인자 함수를 한 인자씩 받는 사슬로 비틀어 N 회 Apply 로 끌어올립니다.
...
> ```
>
> 결과·의미는이 책의 4단계 분해와 동일 — Pure ∘ Curry → Apply → Apply 의 등식이 두 방향에서 모두 성립합니다. 이 책의 명시적 4단계는 어떤 일이 일어나는지의 학습용 펼침, 라이브러리 한 줄은 현장 사용 어법 으로의 압축. 두 접근이 시그니처 단계에서 같은 함수임을 4장 코드의 `Lift.Lift2` 와 LanguageExt 의 `Applicative.lift` 를 나란히 호출해 검증할 수 있습니다.
...
`Lift1 = Map` 의 시각적 근거는 §4.2.4 의 그림 4-4 — `Map ≡ Apply ∘ Pure` 의 두 경로 등식이 그대로 `Lift1` 이 진짜 Applicative 의 능력을 쓰지 않는다는 사실의 시각화입니다. 그림의 우측 (Map 경로) 이 정확히 `Lift1(f)(fa)` 의 동작, 좌측 (Apply 경로) 이 `Lift2 / Lift3` 에서 본격적으로 살아나는 *Pure + Apply* 사슬의 가장 짧은 한 단계 (1인자) 입니다.
...
`Applicative<F> : Functor<F>` 의 상속이 시그니처 단계에서 의미하는 것 — `Map` 이 자동으로 따라옵니다. `Map ≡ Apply ∘ Pure` 의 등식 (§4.2.4) 이 그 한 줄 증명. 새 자료 타입에 `Pure + Apply` 를 정의하면 `Map` 의 동작이 자동으로 일관됩니다.
...
>
> `MyValidation` 이 만약 `Bind` 를 정의한다면, 첫 Invalid 에서 단락 하여 누적이 사라집니다. 사용자에게 왜 네 자리에서 어떻게 실패했는지가 한 자리 에러로 줄어듭니다. 7장 Validation 실전의 핵심 결정. Validation 은 의도적으로 Bind 를 정의하지 않고 Applicative 의 누적만 유지합니다. 누적이 필요한 자리에서 실수로 `Bind` 를 쓰는 것 자체가 언어 수준 차단.
...
이 세 챌린지 중 하나 이상을 직접 작성·검증합니다. 정답 코드는 `code/Part1-Foundations/Ch04-Applicative/Challenges/` 안에 있습니다.
...
새 인스턴스가 `Applicative<F>` 를 만족하려면 `Pure`, `Apply`, 그리고 `Map` 세 멤버를 모두 구현해야 합니다 (interface 상속의 직접 결과). 단, `Map ≡ Apply ∘ Pure` 의 등식 (§4.2.4) 이 Map 의 일관성을 보장 — 두 정의가 의미적으로 같아야 법칙이 깨지지 않습니다. 즉 `Map` 을 직접 구현해도 되지만, 최적화 / 결합 의미 보존을 위해 `Apply` 와 같은 분기를 따르도록 손으로 작성합니다 (3장 §3.4 의 MyMaybe 의 Map 이 그 예).
...
시그니처는 똑같습니다 — `K<F, Func<A, B>> → K<F, A> → K<F, B>`. 갈리는 자리는 Apply 의 분기 구현입니다. `MyMaybe` 의 분기는 `(Just, Just) → Just(f(a))`, 그 외 → `Nothing`. `MyValidation` 의 분기는 `(Valid, Valid) → Valid(f(a))`, `(Invalid, Invalid) → Invalid(누적)`, 한 측만 Invalid → 그 에러 보존. 같은 시그니처가 자료 구조의 분기에 따라 다른 결합 의미를 가집니다. 6장 Monad 의 `Bind` 와는 시그니처 자체가 다릅니다. `Bind` 의 두 번째 인자가 `a → E<b>` (World-crossing) 라 왼쪽 결과가 오른쪽 호출을 결정 하므로 첫 실패에서 단락이 시그니처적으로 강제 됩니다. 7장 Validation 실전의 핵심.

> Q8. `Applicative<F>` 가 `IMonoid<E>` 같은 추가 제약을 받는 자리는 어디입니까?

`MyValidation<E, A>` 의 누적 결합 `[..fe.Errors, ..ae.Errors]` 가 사실 List monoid 의 Combine. 일반화하면 `MyValidation<MonoidE, A>` — `E` 가 임의 monoid 라면 두 실패의 결합이 그 monoid 의 Combine 으로. 라이브러리 LanguageExt 의 `Validation<MonoidE, A>` 가이 일반화의 직접 표현. 학습용 코드는 List 의 Concat 만 다루지만, §4.9.3.A 의 `ConstApplicativeF<M>` 챌린지가 그 일반화의 첫 단계.
...
이론적 한도는 없습니다. `Apply` 를 N 번 호출하면 N 인자 함수가 끌어올려집니다. 실무 한도는 가독성입니다. `Lift5` 이상은 인자 5 개 이상의 생성자 / 함수입니다. 일반적으로 도메인 객체 생성자의 자리입니다. 그 자리에서는 4장의 `Lift5 / Lift6` 보다 8장 Traversable 의 `sequence / traverse` 가 더 자연스럽습니다 — 인자가 같은 종류 N개 일 때 `K<F, IEnumerable<A>>` 로 묶어 한 번에 처리합니다. 4장의 `Lift2-Lift4` 가 실무에서 가장 자주 쓰이는 범위입니다.

> Q11. C# 의 LINQ `from-from-select` 가 Applicative 와 어떤 관계입니까?

LINQ `from a in fa from b in fb select f(a, b)` 는 Monad 의 SelectMany 의 syntactic sugar — `Bind` 의 어법이지 `Apply` 가 아닙니다. *Validation* 처럼 Apply 만 정의 한 자료 타입은 LINQ `from-from-select` 로 합성할 수 없습니다. 명시적 `Lift2 / Lift3 / Lift4` 호출만 가능합니다. 이 차이가 언어 수준에서 누적과 단락을 가르는 자리입니다. 누적이 필요하면 `Lift`, 단락이 필요하면 LINQ 입니다. 6장 Monad / 7장 Validation 실전에서 본격적으로 다룹니다.

---

## §4.12 다음 장으로 — 마무리 (5장 Foldable 다리)

4장에서 본 `Pure + Apply + LiftN` 이 *Normal → Elevated* 의 N 인자 lift 였다면, 5장 Foldable 은 반대 방향입니다. Elevated World 의 구조를 Normal World 의 한 값으로 끌어내리는 (lower) 함수가 `fold` 입니다. 3장 / 4장이 lift 가족 이었다면 5장이 *lower* 의 자리입니다.
...
> §4 → §5 의 다리 — Applicative 가 N 인자 Normal 함수를 *Elevated* 에서 쓸 수 있게 끌어올렸다면, Foldable 은 Elevated 의 구조를 Normal 의 한 값으로 끌어내립니다. 두 추상이 두 평행 세계 사이 두 방향 사다리의 위→아래 자리입니다.
