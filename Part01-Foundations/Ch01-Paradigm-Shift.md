...
> **이 장의 주제**. 세 패러다임 (명령형 / 객체 지향 / 함수형) 의 차이를 능력이 어디 사는가의 한 차원으로 압축한 뒤, 함수형의 어휘를 두 평행 세계 비유로 정착시킵니다. Normal World 와 Elevated World, 그리고 그 사이를 오가는 4가지 함수 유형 (이 격자가 2-9장의 모든 추상을 분류하는 지도가 됩니다).
...
또 다른 강점은 성능 통제입니다. 메모리에서 어떤 위치의 어떤 값이 언제 변경되는지가 코드 자체에 명시되어 있어, 낮은 수준의 최적화가 가능합니다. C / C++ / Rust 같은 시스템 언어가이 영역에서 결정적 가치를 갖습니다.
...
`AddItem` 이라는 절차가 `items` 와 `total` 을 함께 갱신하기로 약속되어 있지만, 그 약속은 절차의 본문에만 존재 합니다. 코드의 구조가 그 약속을 강제하지 못합니다. 두 변수 모두 어디서든 직접 접근 가능하므로, 다른 절차가 `items` 만 / `total` 만 손대고 떠나도 컴파일러는 알아채지 못합니다. 그 결과 `items` 의 합과 `total` 의 값이 어긋난 채 코드가 계속 돌아가고, 이 어긋남은 한참 떨어진 다른 절차 (예: `PrintReport`) 에서야 잘못된 결과로 표면화됩니다.
...
명령형 패러다임으로는 이 함정을 원리적으로 막기 어렵습니다. 락 / `lock` / `Interlocked` 같은 도구로 부분적으로 풀 수 있지만, 큰 코드에서는 어디 락이 빠졌는지를 모두 추적하기가 매우 어렵습니다. 이게 명령형 + 동시성이 큰 시스템에서 결정적인 비용을 낳는 이유입니다.

### 1.2.5 명령형이 여전히 강한 곳

명령형의 약점만 봤지만, 명령형이 틀리다는 게 아닙니다. 짧은 스크립트, 알고리즘 구현, 단일 스레드 작업에서는 여전히 결정적으로 강합니다. 이 책이 함수형을 가르친다고 해서 명령형을 버려라는 메시지가 아닙니다. 오히려 어느 자리에 무엇이 자연스러운가의 판단을 만드는 것이 이 책의 목표 중 하나입니다.
...
C# / Java 같은 객체 지향 언어가 이 발상의 주류 구현입니다. 객체 지향의 핵심을 위키피디아는 4 대 원칙 (four pillars) 으로 정리합니다 (캡슐화 / 추상화 / 상속 / 다형성). 네 원칙이 서로 다른 결의 도구로 복잡성이라는 같은 적을 다룹니다.
...
큰 시스템이 작은 객체들의 협력으로 추론된다는 점도 결정적 가치입니다. 책임 분리 원칙 (Single Responsibility Principle), 의존성 역전 (Dependency Inversion), 인터페이스 분리 같은 객체 지향 설계 원칙들이 그 협력을 건전하게 유지하는 도구들입니다.
...
명령형에서는 함수 안에 능력이 삽니다. 객체 지향에서는 객체에 능력이 삽니다. 함수형에서는 *trait* 에 능력이 삽니다 (trait 을 책장의 비유로 떠올려도 좋습니다. 같은 능력을 가진 타입들이 그 책장에 꽂힙니다). 이 trait 의 자리가 함수형의 결정적 도구이고, 2장부터 만나는 `K<F, A>` + Higher Kinds 가 그 trait 을 C# 안에서 표현하는 어법입니다. 객체 다형성 vs trait 다형성의 본격적 대비는 *함수형의 본질, 즉 합성 가능한 Elevated World 로 lift* (§1.6.1) 에서 다룹니다.
...
함수형의 원본이 그대로라는 점이 동시성 안전성과 추론 가능성의 결정적 출처입니다. 두 스레드가 같은 원본을 보고 있어도, 각자 새 객체를 만들어 반환하므로 서로의 작업이 서로에게 영향이 없습니다. 락 / 동기화 코드가 원리적으로 불필요 해집니다.
...
앞의 세 절에서 두 기호 (화살표 `→` 와 꺾쇠 `< >`) 를 만났습니다. 이 책이 수학·Haskell·F# 의 어휘를 그대로 가져오므로 시그니처 한 줄의 어법이 C# 의 메서드 선언과 다소 다릅니다. 본격적인 4분면 격자 (다음 절) 로 들어가기 전에 한자리에서 정리합니다.
...
1부의 함수형 어휘는 4분면 격자와 5개 trait 두 표가 전부입니다. 그 뒤 4분면 자리를 차례로 코드로 보고, 4자리가 두 그룹 (자연 합성 vs 합성 회복) 으로 갈린다는 사실을 마지막에 정리합니다.
...
**그림 1-2. 두 세계 사이를 오가는 4가지 함수 유형** — 4분면 그림이 `a → b` / `a → E<b>` / `E<a> → b` / `E<a> → E<b>` 네 자리를 보여 줍니다. 1부의 모든 추상은이 네 화살표 중 하나를 끌어올리거나 합성을 회복하는 도구입니다.
...
§1.5.2 / §1.6.1 의 trait 다형성 (= type class 다형성) 이 두 평행 세계 그림에서 작동하는 메커니즘이 trait 부착입니다. Elevated World 자체가 아닌 *trait* 에 능력이 살고, 컨테이너가 trait 을 부착하는 순간 시민들이 능력을 갖게 됩니다.
...
다음 네 절에서 4분면 자리를 차례로 코드로 봅니다. 특히 `E<a> → E<b>` 자리의 *Functor `Map`* 이 1부 전체의 작동 원리입니다.
...
이 합성 불가능의 자리가 7장 Monad 가 푸는 문제입니다. `bind : E<a> → (a → E<b>) → E<b>` 라는 도구가 *World-crossing* (`a → E<b>`) 함수를 *Elevated → Elevated* (`E<a> → E<b>`) 함수로 끌어올려 합성을 회복합니다. `switch` 반복이 `bind` 한 메서드 (또는 LINQ `from-from-select`) 안으로 흡수됩니다.
...
이 자리가 6장 Foldable 입니다. `fold` 가 끌어내림 (`E<a> → b`) 의 일반화로, 어떤 Elevated 컨테이너든 자기 안의 구조를 Normal 값으로 끌어내릴 수 있다는 추상입니다. §1.5.2 의 Sum 이 trait 에 사는 능력이라는 발상의 코드 표현이 정확히 이 자리, `Foldable` trait 의 정적 메서드입니다.
...
그리고 능력이 사는 곳은 Elevated World 가 아니라 trait (§1.7.2) 이고, 컨테이너가 trait 을 부착하는 순간이 다리들이 모든 시민에게 자동으로 적용됩니다. 데이터는 Elevated World 의 시민, 능력은 trait 의 정적 자리이며, 두 자리가 trait 부착 한 줄로 연결됩니다.

1부의 8개 장이 모두 이 네 자리 중 어느 다리를 어떻게 놓는가의 이야기입니다. §1.8 의 표가 1부 전체의 지도가 되는 이유입니다. 한 줄로 정리하면:
...
§1.4.3 에서 함수형의 세 결합 (불변 + 합성 + 추상) 을 봤습니다. 이 사슬 한 줄이 세 결합을 모두 동시에 만족합니다.
...
이 그림이 `E<a>` 표기 (§1.6.3) + 4분면 (§1.7.1) + trait 부착 (§1.7.2) + 세 결합 (§1.4.3) 을 한자리에 압축한 시각입니다. 사슬은 합성 (어법 일치로 자유 연결), 사슬 위 박스의 불변성 (`E<a>`, `E<b>`, ... 새 값을 만들어 옆으로 흐름), `E` 한 글자의 추상 (구체 컨테이너 무관) 의 세 가치를 동시에 보여 줍니다.
...
이 책의 1부가 하는 일은 그 LINQ 의 친숙한 표면 아래의 5 trait + 4분면 + 두 평행 세계 어휘를 명시화하는 것뿐입니다. LINQ 위 한 사슬을 Functor 의 Map 적용, Foldable 의 Fold 적용으로 읽을 수 있게 되면, 그 어휘가 *Option, Result, Task, Reader, State* 같은 수많은 다른 Elevated World 에도 그대로 적용됩니다. 어휘 학습의 ROI 가 결정적인 자리입니다.
...
불변·합성·추상. 첫째, 불변 + 순수 — 데이터 변경 없음, 같은 입력은 같은 결과 (참조 투명성). 둘째, 합성 (일급 + 고차 함수) — 작은 함수들의 조합으로 프로그램 구성. 셋째, 추상 (능력의 trait 화) — 같은 능력 (`Map` / `Bind` / `Fold`) 을 여러 타입 위에서 일반화. 세 결합이 서로를 강화합니다. 불변이 합성을 안전하게, 합성이 추상에 의미를, 추상이 합성을 확장 가능하게 만듭니다.
...
- 두 세계 사이의 함수는 정확히 4가지 유형 (`a → b`, `a → E<b>`, `E<a> → b`, `E<a> → E<b>`) 입니다. 1부의 추상은 이 격자 위의 어느 한 화살표를 끌어올리거나 합성을 회복하는 도구입니다.
