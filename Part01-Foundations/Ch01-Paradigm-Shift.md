...
두 확장 이동은 기본 자리의 도구가 모두 손에 잡힌 뒤 (9장, 11장) 그림과 함께 다시 만납니다. 지금은 지도에 두 칸이 예약되어 있다는 것만 가져가면 됩니다. *Applicative* (5장) 는 새 이동이 아니라 `E<a> → E<b>` 자리의 다인자 확장 (`(a, b) → c` 같은 함수의 끌어올림) 이라 기본 네 자리 안에 삽니다.
