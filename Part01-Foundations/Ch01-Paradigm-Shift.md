...
1부의 함수형 어휘는 4 가지 함수 유형과 5 개 trait 두 표가 전부입니다. 그 뒤 네 자리를 차례로 코드로 보고, 네 자리가 두 그룹 (자연 합성 vs 합성 되살리기) 으로 갈린다는 사실을 마지막에 정리합니다.
...
**그림 1-2. 두 세계 사이를 오가는 4 가지 함수 유형** — 두 세계 위 그림이 `a → b` / `a → E<b>` / `E<a> → b` / `E<a> → E<b>` 네 자리를 보여 줍니다. 1부의 모든 추상은 이 네 화살표 중 하나를 끌어올리거나 합성을 되살리는 도구입니다.
...
- 두 세계 사이의 함수는 정확히 4 가지 유형 (`a → b`, `a → E<b>`, `E<a> → b`, `E<a> → E<b>`) 입니다. 1부의 추상은 이 네 자리 위의 어느 한 화살표를 끌어올리거나 합성을 되살리는 도구입니다.
