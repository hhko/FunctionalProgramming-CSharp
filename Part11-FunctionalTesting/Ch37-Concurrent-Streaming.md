...
결과가 비결정적이기 때문입니다. 두 스레드가 같은 값을 읽고 각자 1 을 더해 같은 값을 쓰면 갱신 하나가 유실되어, 1000번 증가했는데 최종값이 998 일 수도 1000 일 수도 있습니다. 실행마다 다른 값은 `== 1000` 같은 단언의 대상이 될 수 없습니다. 그래서 비원자 카운터는 결정성을 단언하지 않고, `Atom` 의 결정성만 단언합니다. 첫 번째 챌린지 (37.8.1절) 가 이 유실을 직접 관찰합니다.
