# 37장. 동시 · 스트리밍 · 자원 효과 테스트 (비결정성을 길들여 결정적으로)

> **이 장의 목표** — 이 장을 읽고 나면 비결정성과 시간과 수명이 얽힌 동시 · 스트리밍 · 자원 효과를 결정적으로 검사할 수 있습니다. 1000개 스레드가 동시에 증가시킨 카운터가 항상 정확히 1000 임을 단언하고, 재시도 정책을 실제 지연 없이 시도 횟수로 검사하며, 자원이 예외에도 반드시 해제되고 중첩 자원이 LIFO 로 닫히는지를 종료 로그로 확인하고, lazy 스트림이 소비된 만큼만 흐르는지를 골든 시퀀스로 검증하는 네 가지 도구를 손에 쥡니다. 36장에서 효과를 값으로 인코딩해 결정적으로 검사했다면, 이 장은 그 발상이 가장 까다로운 영역, 곧 비결정 영역에서도 그대로 통한다는 것을 코드로 보입니다.
...
> - [ ] 순수성과 효과-as-value 설계가 왜 비결정 영역마저 결정적으로 만드는지 설명할 수 있습니다.

---

## 37.1 1장 비유로 출발 — 비결정 효과는 Elevated World 의 어느 자리인가

이 장의 핵심은 한 줄로 압축됩니다. 36장에서 효과를 값으로 인코딩하면 런타임만 바꿔 결정적으로 검사할 수 있다고 봤습니다. 이 장은 그 발상을 효과 코드의 가장 까다로운 영역, 곧 동시성 · 스트리밍 · 자원 수명에 적용합니다. 이 영역은 비결정성과 시간과 수명이 얽혀 있어 검사가 어렵지만, 함수형 설계 세 가지 (효과를 값으로, 자원을 bracket 으로, 시간을 인자로) 가 비결정 영역마저 결정적 단언으로 바꿉니다.

> 37장의 5부 구조 — 36장과 같은 narrative arc 로 구성됩니다.
>
> - **§37.1 ~ §37.2 목적** — 동시 · 스트리밍 · 자원 테스트의 불편 (레이스 · 시간 · 자원 누수 · 비결정).
> - **§37.3 기능** — 결정적 불변식의 발상. 비결정 순서와 결정적 최종값의 분리.
> - **§37.4 ~ §37.7 도구** — `Atom` 원자성, `Schedule` 재시도, `Resource` bracket, `Source` 스트림 골든.
> - **§37.8 ~ 마무리** — 챌린지 · 다시 읽기 · Q&A · 요약 · 38장 다리.

### 37.1.1 9 ~ 10부에서 만든 동시 · 스트리밍 효과 복습

이 장이 검사하는 대상은 9부 (동시성 · STM) 와 10부 (스트리밍 · 자원) 에서 만든 효과들입니다. LanguageExt v5 에서는 STM 의 원자적 참조 (`Atom`), 재시도 정책 (`Schedule`), 자원 수명 (`Resource` / `bracket`), 효과적 스트림 (`SourceT`) 이 그 실체입니다. 이 도구들은 모두 비결정성이나 시간을 다루므로, 실제 동작을 끼워 검사하면 결과가 흔들립니다.

이 장은 그 도구들의 핵심만 추려 의존성 없는 toy 로 다시 만듭니다. `Atom<Counter>` 는 CAS 루프로 락 없이 갱신하고, `Schedule.Retry` 는 시도 횟수만 세며, `Resource.Use` 는 `try` / `finally` 를 함수형으로 캡처하고, `Source` 는 `IEnumerable<int>` 위의 lazy 스트림입니다. 선행 Part 의 구체 동작이 궁금하더라도, 이 장의 단언은 모두 이 toy 코드만으로 자족합니다.

### 37.1.2 비결정 효과의 1장 매핑 — 효과는 값, 결과는 불변식

1장에서 두 평행 세계 (Normal World / Elevated World) 를 정했습니다. 이 장의 도구들은 모두 Elevated World 의 시민입니다. 비결정성 (스레드 순서) 이나 시간 (지연) 이나 수명 (자원 개폐) 이라는 효과를 값으로 가둔 컨테이너이기 때문입니다.

검사가 일어나는 자리는 Normal World 입니다. 효과를 값으로 가두면, 그 값에서 비결정에 휘둘리지 않는 한 가지 결정적 사실을 끌어내릴 수 있습니다. 동시 증가의 순서는 비결정이지만 최종값은 항상 1000 입니다. 재시도의 실제 시간은 비결정이지만 시도 횟수는 항상 3 입니다. 자원 개폐의 시각은 비결정이지만 닫히는 순서는 항상 LIFO 입니다. 이 결정적 사실 하나가 Normal World 의 단언 대상이 됩니다.

![비결정성과 시간과 수명을 결정적 불변식으로 끌어내림](./images/Ch37-Concurrent-Streaming/01-nondeterminism-tamed.svg)

**그림 37-1. 비결정 효과를 결정적 단언으로** — 위 행 Elevated World 의 네 도구 (`Atom` / `Schedule.Retry` / `Resource.Use` / `Source`) 는 각각 레이스 · 시간 · 수명 · 조기 정지라는 비결정 효과를 값으로 가둡니다. 아래 행 Normal World 에는 그 효과에 휘둘리지 않는 결정적 사실 (`== 1000`, `== 3`, `B, A 순서`, `[0,1,2]`) 만 남습니다. 36장이 콘솔 효과를 런타임 더블로 끌어내렸다면, 이 장은 비결정 효과를 불변식으로 끌어내립니다. 효과는 값이고, 단언은 그 값에서 끌어낸 한 가지 결정적 사실입니다.
...
각 도구의 결정적 부분이 단언 대상입니다. 순수성과 효과-as-value 설계가 이 분리를 가능하게 합니다. 비결정을 시간이나 스레드 순서에 맡기지 않고 값으로 가두면, 그 값에서 결정적 사실을 끌어내릴 수 있습니다.
...
├── Challenges/LostUpdate.cs · RetryWithLog.cs · NestedScope.cs ← §37.8 정답
...
첫 도구는 동시성의 원자성입니다. `Atom<T>` 는 갱신을 순수 함수 `T → T` 로 표현하고, 충돌 시 CAS (compare-and-swap) 루프로 자동 재시도하는 원자적 참조입니다. LanguageExt v5 `Atom` 의 축소판으로, `Interlocked.CompareExchange` 로 락 없이 안전합니다.
...
네 경우가 재시도의 경계를 모두 짚습니다. 중간에 성공 (3회), 즉시 성공 (1회), 끝까지 실패 (`maxAttempts` 포기), 마지막 시도에 성공 (`maxAttempts` 와 같음) 입니다. 어느 경우든 결과는 시간이 아니라 횟수라서 항상 같은 값입니다. 시간을 인자로 빼고 횟수만 단언하는 것이 재시도 테스트의 핵심입니다.

---

## 37.6 `Resource` bracket — 예외에도 해제, 중첩은 LIFO

셋째 도구는 자원 수명입니다. `Resource.Use` 는 획득 → 사용 → 해제를 한 함수로 묶는 bracket 입니다. 사용 중 예외가 나도 해제는 반드시 실행됩니다. C# 의 `try` / `finally` 를 함수형으로 캡처한 것으로, LanguageExt v5 의 use · release / bracket 의 축소판입니다.
...
중첩 자원은 `Scope` 가 다룹니다. 여러 자원을 추적하고 역순 (LIFO) 으로 해제합니다. 연 순서를 기록하는 `Log` 가 LIFO 검증의 골든 대상입니다.
...
넷째 도구는 effectful 스트림입니다. `Source` 는 `IEnumerable<int>` 기반의 작은 스트림으로, 원소를 흘려보내면서 부수 작용 (여기서는 흘러간 원소를 trace 에 기록) 을 남깁니다. LanguageExt v5 `SourceT` 의 축소판입니다.
...
스트림은 lazy 라 언제 무엇이 흘렀는지가 흐릿합니다. `From` 이 원소를 흘릴 때마다 trace 에 기록하므로, effect 를 trace 로 모으면 "생성 시퀀스 == 기대 골든" 으로 결정적으로 단언할 수 있습니다. 이것이 골든 테스트입니다. `Map` 과 `Filter` 는 게으름을 유지한 채 모양을 보존하거나 원소를 거릅니다.
...
네 예제 모두 결정적 값을 냅니다. 동시 증가는 항상 1000, 재시도는 항상 3회와 4회, 자원은 항상 예외에도 해제되고 LIFO 로 닫히며, 스트림은 항상 `[0, 20, 40]` 입니다. 데모는 이어서 13개 단언을 모두 `통과` 로 확인합니다 (검증 절 출력은 생략했습니다). xUnit 으로 옮긴다면 각 `bool` 헬퍼를 `[Fact]` 로 감싸고 `ShouldBeTrue()` 로 단언하면 그대로 표준 테스트가 됩니다.
...
> **본문 어느 자리의 이해도를 묻는가**
>
> 1. 결정적 불변식 (§37.3) — 순수 갱신의 최종값이 결정적인 까닭.
> 2. CAS 루프 (§37.4) 가 잃어버린 갱신을 어떻게 막는가.
>
> **해보기**
>
...
> **본문 어느 자리의 이해도를 묻는가**
>
> 1. 시간을 인자로 빼고 횟수만 세는 발상 (§37.5) 이 순서 로그로도 확장되는가.
> 2. `Flaky` 가 결정적 action 을 만드는 까닭 (§37.5).
>
> **해보기**
>
...
> **본문 어느 자리의 이해도를 묻는가**
>
> 1. LIFO 해제 (§37.6) 가 `Stack` 의 push / pop 으로 따라온다는 것.
> 2. bracket 이 예외에도 해제를 보장한다는 것 (§37.6).
>
> **해보기**
>
...
세 챌린지는 이 장의 핵심 (비결정에서 결정적 한 조각을 끌어내려 단언한다) 을 세 각도에서 묻습니다. 첫째는 순수 갱신이 없으면 결과가 왜 비결정으로 흩어지는지 직접 관찰하는 능력, 둘째는 시간을 뺀 횟수·순서가 결정적임을 로그로 확장하는 능력, 셋째는 LIFO 해제 순서를 예측해 자원 수와 무관하게 검사하는 능력입니다. 셋을 다 통과하면 "비결정 효과를 어떻게 결정적으로 단언하는가" 를 코드로 답할 수 있습니다.
...
| 37장 도구 | Elevated World 어휘 |
|---|---|
| `Atom<T>` | 효과 = 동시 갱신. 순수 함수로 갱신하는 원자적 시민 |
| `Schedule.Retry` | 효과 = 재시도. 시간을 인자로 빼 횟수만 남긴 시민 |
| `Resource.Use` | 효과 = 자원 수명. 획득 · 해제를 bracket 으로 묶은 시민 |
| `Source` | 효과 = lazy 스트림. 소비될 때만 흐르는 시민 |

36장이 콘솔 효과를 런타임 더블로 끌어내려 결정적으로 검사했다면, 37장은 비결정 효과 (스레드 순서 · 시간 · 수명 · 게으름) 를 불변식으로 끌어내려 검사합니다. 효과를 값으로 가두는 같은 발상이, 가장 까다로운 비결정 영역에서도 결정적 단언을 가능하게 합니다. 비유는 여기까지가 역할입니다. 무엇이 비결정이고 무엇이 결정인지는 각 도구의 시그니처가 정합니다.

---

## 37.10 Q&A — 자기 점검

> **Q1. 동시성 테스트인데 왜 결과가 항상 같습니까? 스레드 순서는 매번 다르지 않습니까?** (§37.3)
...
> **Q2. 비원자 카운터 (`n++`) 를 동시 증가하면 왜 단언할 수 없습니까?** (§37.4)

결과가 비결정적이기 때문입니다. 두 스레드가 같은 값을 읽고 각자 1 을 더해 같은 값을 쓰면 갱신 하나가 유실되어, 1000번 증가했는데 최종값이 998 일 수도 1000 일 수도 있습니다. 실행마다 다른 값은 `== 1000` 같은 단언의 대상이 될 수 없습니다. 그래서 비원자 카운터는 결정성을 단언하지 않고, `Atom` 의 결정성만 단언합니다. 첫 번째 챌린지 (§37.8.1) 가 이 유실을 직접 관찰합니다.

> **Q3. 재시도 테스트에서 왜 실제 지연을 넣지 않습니까?** (§37.5)
...
> **Q5. 중첩 자원이 왜 LIFO 로 닫힙니까?** (§37.6)

`Scope` 가 해제 동작을 `Stack` 에 push 하고 `ReleaseAll` 이 pop 하며 닫기 때문입니다. 마지막에 push 된 (= 마지막에 연) 자원이 가장 먼저 pop 됩니다. A → B 순서로 열면 B → A 순서로 닫힙니다. 이것이 중첩 자원의 올바른 정리입니다. 안쪽 자원이 바깥 자원보다 먼저 닫혀야 의존 관계가 깨지지 않습니다. 종료 로그 (`Log`) 가 그 순서를 골든으로 단언합니다.

> **Q6. lazy 스트림의 조기 정지를 어떻게 검사합니까?** (§37.7)
...
> **Q7. 이 장의 toy 와 LanguageExt v5 의 실제 도구는 어떻게 다릅니까?** (§37.1.1)
...
> **Q8. 이 도구들의 법칙 (예: Functor 법칙) 검사는 왜 안 합니까?** (§37.1)
...
- **불편에서 출발했습니다.** 실제 동작에 묶으면 동시성은 레이스로, 재시도는 시간으로, 자원은 수명으로, 스트림은 게으름으로 결과가 비결정이 됩니다 (§37.2).
- **결정적 불변식이 핵심입니다.** 비결정인 부분 (순서 · 시간) 은 단언하지 않고, 결정인 부분 (최종값 · 횟수 · 순서 · 시퀀스) 만 단언합니다 (§37.3).
- **`Atom` 의 최종값은 항상 증가 횟수입니다.** 순수 갱신과 CAS 루프가 1000개 동시 증가를 유실 없이 1000 으로 만듭니다 (§37.4).
- **`Schedule.Retry` 는 시간을 빼고 횟수만 셉니다.** 3번째 성공은 3회, 끝까지 실패는 `maxAttempts` 로 결정적입니다 (§37.5).
- **`Resource.Use` 는 예외에도 해제를 보장합니다.** `finally` 의 release 를 flag 로 확인하고, 중첩은 LIFO 로 닫힙니다 (§37.6).
- **`Source` 의 조기 정지를 trace 골든으로 단언합니다.** 소비된 만큼만 흐르는 게으름이 trace `[0, 1, 2]` 로 관찰됩니다 (§37.7).
- **함수형 설계가 비결정을 결정으로 바꿉니다.** 효과를 값으로, 자원을 bracket 으로, 시간을 인자로 두면 비결정 영역마저 결정적 단언이 가능합니다 (§37.3).
