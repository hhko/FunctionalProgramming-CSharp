...
> **이 장의 목표** — 11부의 첫 장으로, 부수 작용이 섞인 코드를 실제 부수 작용 없이 검사하는 길을 엽니다. 이 장을 읽고 나면 7부에서 만든 `Eff<RT>` 가 왜 테스트하기 좋은 모양인지 시그니처로 설명하고, 운영 런타임 대신 `MemoryConsole` 테스트 더블을 주입해 콘솔이 섞인 효과 코드를 결정적으로 검사할 수 있습니다. 순수 함수는 같은 입력에 같은 출력이라 쉽게 테스트되지만 콘솔·파일·시간이 섞이면 검사가 흔들립니다. 효과를 값으로 인코딩하고 런타임만 바꿔 끼우면 그 흔들림이 사라진다는 것을, 직접 작성한 `Eff.Run` 과 단언 코드로 손에 쥐게 됩니다.

> **이 장의 핵심 어휘**
>
> - `Eff<RT, A>` (런타임 `RT` 를 읽어 부수 작용을 실행하는 효과 값. 이 책에서는 `ReaderT<RT, A>` 로 재현하며, 함수 시그니처에는 `K<ReaderTF<RT>, A>` 로 나타납니다)
> - **효과는 값 (effect-as-value)** (부수 작용을 곧바로 일으키지 않고 "무엇을 할지" 만 담은 값으로 두는 설계)
> - **테스트 더블** (`MemoryConsole` — 실제 콘솔 대신 입력은 큐, 출력은 리스트로 잡는 가짜 구현)
> - **런타임 주입** (`Has<RT, IConsole>` DI 로 운영 런타임 대신 테스트 런타임을 끼우는 자리)
> - **결정적 테스트 (deterministic)** (같은 입력이면 항상 같은 출력. 부수 작용을 값으로 인코딩한 덕분)
...
36장의 핵심은 한 줄로 압축됩니다. 효과 코드를 테스트하기 어려운 까닭은 부수 작용이 코드에 박혀 있어서이고, 그 부수 작용을 값으로 끌어올려 Elevated World 의 시민으로 다루면 테스트가 결정적이 됩니다. 새 trait 을 배우는 장이 아니라, 7부에서 만든 `Eff<RT>` 가 왜 테스트하기 좋은 모양이었는지를 검증의 눈으로 다시 보는 장입니다.

순수 함수는 이미 기초에서 충분히 테스트했습니다. 같은 입력에 같은 출력이므로, 입력을 넣고 출력을 단언하면 끝입니다. 3장 (§3.7.1) 에서 익힌 최소 `ForAll` property 로 임의 입력에까지 법칙을 확인했고, 그 법칙 검증은 4장 ~ 11장 각 장에서 이미 끝났습니다. 11부의 출발점은 그 다음입니다. 콘솔·파일·시간 같은 부수 작용이 섞인 코드를 어떻게 결정적으로 검사하는가입니다.

> 36장의 5부 구조 — 기초의 각 장과 같은 흐름으로 구성됩니다.
>
> - **§36.1 ~ §36.2 목적** — 효과 코드 테스트의 불편 (실제 콘솔·시간에 묶이면 비결정·느림·격리 안 됨).
> - **§36.3 기능** — 효과는 값, 런타임은 의존성 (`Eff<RT>` 와 Live vs Test 런타임).
> - **§36.4 ~ §36.5 예제** — `MemoryConsole` 더블 주입, `Greeter.Ask` walkthrough 와 캡처 출력 단언.
> - **§36.6 기능 (payoff)** — 7부 `Has<RT, Trait>` DI 가 테스트 용이성으로 이어지는 자리.
> - **§36.7 ~ 마무리** — 챌린지 · 다시 읽기 · Q&A · 요약 · 37장 다리.

### 36.1.1 7부에서 만든 `Eff<RT>` 자리 복습

`Eff<RT, A>` 는 7부에서 효과를 값으로 다루려고 만든 타입입니다. 이름은 효과 (effect) 의 줄임이고, 모양은 런타임 `RT` 를 읽어 부수 작용을 실행하는 효과 값입니다. 이 책에서는 그 구조를 `ReaderT<RT, A>` 로 재현합니다. `IO` 는 타입 인자가 아니라 본체 `Func<RT, IO<A>>` 안에 고정되어 있습니다.
...
시그니처가 모든 것을 말합니다. `Eff<RT, A>` 는 속을 들여다보면 `Func<RT, IO<A>>`, 곧 런타임 `RT` 를 받아 부수 작용 `IO<A>` 를 돌려주는 함수입니다. 중요한 것은 이 값을 만드는 것만으로는 아무 부수 작용도 일어나지 않는다는 점입니다. 런타임 `RT` 를 넣어 `Run` 을 호출해야 비로소 콘솔에 쓰거나 읽습니다. 효과가 값으로 인코딩되어 실행이 한 자리로 미뤄진 것입니다.

표기 하나만 미리 정해 둡니다. 효과 값은 이 책에서 세 모습으로 나타나지만 모두 같은 것입니다. 개념 이름은 `Eff<RT, A>`, 속 구조는 `ReaderT<RT, A>` (곧 `Func<RT, IO<A>>`), 그리고 뒤이어 나올 함수 시그니처에는 `K<ReaderTF<RT>, A>` 로 적힙니다. 함수가 돌려주는 `K<ReaderTF<RT>, A>` 가 곧 효과 값이라고 읽으면 됩니다. 또 효과를 만드는 함수 모음인 `Eff` 클래스 (`Eff.Run` · `Eff.WriteLine`) 는 이 효과 값 타입과 이름만 닮은 별개의 정적 헬퍼입니다. 7부에서는 이 타입으로 효과를 합성하는 법을 봤고, 36장에서는 그 모양이 테스트에서 무엇을 열어 주는지 봅니다.

### 36.1.2 효과는 값, 런타임은 의존성 — 이 장의 1장 매핑

1장의 두 평행 세계로 옮기면 자리가 또렷해집니다. 효과 값 `Eff<RT, A>` 는 Elevated World 의 시민입니다. 부수 작용이라는 효과를 타입에 담은 끌어올림의 결과입니다. 그 효과를 실제로 일으키는 데 필요한 런타임 `RT` 는 Normal World 에서 바깥에서 주입하는 의존성입니다.
...
**그림 36-1. 런타임 주입: 같은 효과 값, 두 런타임** — 위 행 Elevated World 에 효과 값 `Greeter.Ask<RT>()` 한 개가 있습니다. 효과는 값이라 아직 아무 부수 작용도 일으키지 않습니다. `Run(eff, RT)` 이 런타임을 넣어 끌어내릴 때 비로소 실행됩니다. 아래 행 Normal World 에 두 런타임이 있습니다. 왼쪽 `LiveConsole` 을 담은 런타임은 실제 콘솔에 입출력해 운영에 쓰이지만 비결정적이고 느리며 격리되지 않습니다. 오른쪽 `MemoryConsole` 을 담은 런타임은 입력을 큐로, 출력을 리스트로 잡아 결정적이고 빠르며 격리됩니다. 효과 값은 하나인데 런타임만 갈라집니다.
...
부수 작용이 콘솔에서 끝나지 않는다는 점이 문제를 키웁니다. 같은 코드가 현재 시각 (`DateTime.Now`) 을 읽거나 파일을 쓰거나 네트워크를 호출하면, 테스트는 실행할 때마다 다른 결과를 봅니다. 시각은 매번 다르고, 파일은 이전 실행이 남긴 상태에 영향을 받고, 네트워크는 느리고 가끔 끊깁니다. 정리하면 명령형 IO 테스트의 불편은 네 가지입니다. 실제 콘솔·시간 같은 외부에 묶이고, 같은 입력에도 결과가 달라져 비결정적이고, 외부 호출이 느리고, 테스트끼리 상태를 공유해 격리되지 않습니다.

> **흔한 함정** — "그러면 테스트할 때만 `Console.SetOut` 으로 출력을 가로채고 가짜 입력 스트림을 끼우면 되지" 로 넘기면, 그 가로채기 코드가 전역 콘솔 상태를 건드려 테스트끼리 간섭하고, 시간·파일·네트워크가 섞이면 부수 작용마다 다른 가로채기를 또 만들어야 합니다. 필요한 것은 **부수 작용을 코드에서 떼어 값으로 인코딩하고, 실행을 한 자리로 모아 그 자리에 가짜를 끼우는** 도구입니다. 그 도구가 7부에서 만든 `Eff<RT>` 와 런타임 주입입니다.

명령형 IO 가 테스트하기 어려운 까닭은 한 곳에 모입니다. 부수 작용이 코드에 곧바로 박혀 있어 "무엇을 할지" 와 "어떻게 실행할지" 가 한 덩어리이기 때문입니다. 이 덩어리를 둘로 가르는 것이 효과를 값으로 두는 설계입니다.

---

## 36.3 효과가 값이고 런타임이 의존성이다

명령형 IO 의 덩어리를 둘로 가르는 첫걸음은 부수 작용을 코드에서 떼어 값으로 두는 것입니다. `Eff<RT>` 가 바로 그 값입니다.
...
└── Tests/EffectTests.cs     ← 결정적 효과 테스트 (§36.5)
```
...
테스트용 `MemoryConsole` 은 실제 콘솔 대신 메모리에 입출력을 담습니다 (자세한 모양은 §36.4). 두 구현이 같은 `IConsole` 을 만족하므로, 효과 코드는 둘 중 어느 것이 들어올지 모른 채 `IConsole` 의 두 메서드만 부릅니다. 이 무관심이 테스트 더블을 끼울 자리를 만듭니다.
...
콘솔에 한 줄 쓰는 효과를 값으로 적으면 다음과 같습니다. 런타임에서 콘솔 능력을 꺼내고, 그 능력으로 쓰는 부수 작용을 `IO` 로 끌어올립니다.
...
제약 `where RT : Has<RT, IConsole>` 이 핵심입니다. 이 효과는 콘솔 능력을 가진 런타임에서만 실행된다고 시그니처가 보장합니다. `Asks(rt => RT.Get(rt))` 가 런타임에서 콘솔을 꺼내고, `LiftIO` 가 실제 쓰기·읽기 부수 작용을 효과 값으로 끌어올립니다. 다시 말하지만 이 효과 값을 만드는 것만으로는 콘솔에 아무것도 찍히지 않습니다. 실행은 `Run` 한 자리에 모입니다.

```csharp
public static A Run<RT, A>(K<ReaderTF<RT>, A> eff, RT rt) => ((ReaderT<RT, A>)eff).Run(rt).Run();
```

`Run` 은 효과 값에 런타임 `rt` 를 넣어 `ReaderT.Run(rt)` 으로 안쪽 `IO` 를 꺼낸 뒤, 그 `IO.Run()` 으로 부수 작용을 실제로 일으킵니다. 어떤 런타임을 넣느냐가 이 한 줄에서 결과를 가릅니다.

![효과는 값으로 인코딩되어 실행이 Run 한 자리로 미뤄지는 흐름](./images/Ch36-Effect-Testing/02-effect-as-value.svg)

**그림 36-2. 효과는 값, 실행은 Run 한 자리** — 왼쪽 효과 값 `Eff<RT>` 는 "무엇을 할지" 만 담은 명세라 아직 콘솔에 쓰지 않습니다. `Run(eff, rt)` 이 런타임을 넣을 때 비로소 부수 작용이 일어나고, 런타임을 바꾸면 같은 효과의 결과가 바뀝니다. 아래 대비 행의 명령형 IO 는 `Console.WriteLine(msg)` 를 호출하는 순간 곧바로 부수 작용을 일으켜, 미룰 수도 가짜로 바꿀 수도 없습니다. 효과를 값으로 두면 "무엇을 할지" 와 "어떻게 실행할지" 가 분리되어, 테스트는 런타임만 가짜로 바꿔 끼우면 됩니다.

> **여기까지의 안전망** — `from … from … select` 로 적힌 효과 합성이나 `Has<RT, IConsole>` 제약이 처음엔 복잡해 보여도 괜찮습니다. 반환 타입 `K<ReaderTF<RT>, A>` 는 2부에서 본 higher-kind 우회 표기로, `RT` 를 읽어 부수 작용을 실행하는 효과 값을 가리킵니다. `Asks` 는 런타임을 읽는 자리, `LiftIO` 는 실제 부수 작용을 효과로 끌어올리는 자리 정도만 가져가면 충분합니다. 지금 가져갈 직감은 하나입니다. 효과는 런타임을 받아야 실행되는 값이고, 그 런타임을 바꿔 끼울 수 있다는 것입니다. 합성의 세부는 7부에서 다뤘고, 이 장에서 필요한 것은 갈아 끼울 수 있다는 것뿐입니다.
...
`Greeter.Ask<RT>()` 는 명령형 `Greet()` (§36.2) 과 하는 일이 같습니다. 그러나 결정적인 차이가 있습니다. 명령형 `Greet()` 은 `Console` 을 직접 부르므로 실제 콘솔에 묶이지만, `Greeter.Ask` 는 `Eff.WriteLine` / `Eff.ReadLine` 으로 콘솔 능력에만 의존하므로 어느 런타임이 들어올지 모른 채 효과 값을 만들 뿐입니다. 제약 `where RT : Has<RT, IConsole>` 만 만족하면 운영 런타임에서도 테스트 런타임에서도 같은 코드가 실행됩니다.
...
지역 함수 `Run()` 이 매번 새 `MemoryConsole(["민수"])` 로 실행해 출력을 한 문자열로 잇습니다. 두 번 부른 결과가 같으므로 (`Run() == Run()`) 효과 코드가 결정적임이 확인됩니다. 외부에 묶이지 않고 부수 작용이 값으로 인코딩되어 있으므로 단언할 수 있습니다. 명령형 `Greet()` 이었다면 두 번째 실행이 키보드 입력을 기다리며 멈췄을 자리입니다.
...
`from … from … select` 로 쓰기·읽기를 엮어 효과 값 하나를 만들고, 능력 제약을 시그니처에 명시하는 모양이 이 장의 `Greeter.Ask` (§36.4.2) 와 같습니다. 본문에서 직접 만든 toy 가 라이브러리에서는 이 모습이라고 읽으면 됩니다.
...
> **본문 어느 자리의 이해도를 묻는가**
>
> 1. `MemoryConsole.ReadLine` (§36.4.1) — 큐가 비면 빈 문자열을 돌려준다는 것.
> 2. `Greeter.Ask` (§36.4.2) 의 마지막 인사 줄이 읽은 이름을 그대로 끼워 넣는다는 것.
>
> **해보기**
>
...
> **본문 어느 자리의 이해도를 묻는가**
>
> 1. `IConsole` + `LiveConsole` / `MemoryConsole` (§36.3.1) 의 능력 + 두 구현 구조를 그대로 옮길 수 있는가.
> 2. `Has<RT, Trait>` (§36.3.2) 가 능력마다 런타임에서 꺼내 주는 자리라는 것.
>
> **해보기**
>
...
> **본문 어느 자리의 이해도를 묻는가**
>
> 1. `EffectTests` (§36.5) 의 더블 주입 + 단언 구조가 표준 테스트의 뼈대와 같다는 것.
> 2. 실무 노트 (§36.5.3) 의 이행 방식 (`[Fact]` + `ShouldBe`) 을 적용할 수 있는가.
>
> **해보기**
>
...
>
> 이행 안내: `Tests/EffectTests.cs` 의 세 헬퍼를 `[Fact]` 형태로 옮기는 방식은 11부 README (`Part11-FunctionalTesting/README.md`) 와 실무 노트 (§36.5.3) 에 정합합니다.

### 36.7.4 세 챌린지가 노리는 능력

세 챌린지는 36장의 핵심 (효과는 값이고 런타임이 의존성이라 더블을 끼워 결정적으로 검사한다) 을 세 각도에서 묻습니다. 첫째는 더블의 입력 시나리오를 마음대로 정해 결과를 예측하는 능력, 둘째는 콘솔 너머의 새 능력 (시간) 으로 같은 패턴을 확장하는 능력, 셋째는 학습용 `bool` 헬퍼를 실무 표준 테스트로 옮기는 능력입니다. 셋을 다 통과하면 "부수 작용이 섞인 코드를 어떻게 결정적으로 검사하는가" 를 코드로 답할 수 있습니다.
...
|---|---|
| `Eff<RT, A>` | 효과 = 부수 작용. 런타임을 받아야 실행되는 Elevated 시민 |
...
7부의 `Eff<RT>` 가 효과를 합성 가능한 Elevated 시민으로 끌어올린 도구였다면, 36장은 그 효과 값에 런타임만 바꿔 끼워 결정적으로 검사한다는 실전입니다. 효과는 값이라 실행이 미뤄지고, 미뤄진 실행에 가짜 런타임을 끼우는 자리가 검증입니다. 비유는 여기까지가 역할입니다. 어떤 더블을 끼울지는 검사하려는 부수 작용이 정합니다.

---

## 36.9 Q&A — 자기 점검

> **Q1. 순수 함수는 이미 테스트했는데, 효과 코드는 왜 따로 다룹니까?** (§36.1)

순수 함수는 같은 입력에 같은 출력이라 입력을 넣고 출력을 단언하면 끝나고, 그 법칙 검증은 기초의 각 장에서 `ForAll` property 로 이미 끝났습니다. 그러나 콘솔·파일·시간 같은 부수 작용이 섞이면 결과가 외부에 묶여 흔들립니다. 11부는 그 다음, 부수 작용이 섞인 효과 코드를 결정적으로 검사하는 전문 영역을 다룹니다. 36장은 그 첫 자리입니다.

> **Q2. 명령형 IO 테스트가 어려운 까닭은 무엇입니까?** (§36.2)

부수 작용이 코드에 곧바로 박혀 "무엇을 할지" 와 "어떻게 실행할지" 가 한 덩어리이기 때문입니다. `Console.WriteLine` 을 부르는 순간 실제 콘솔에 찍히고, `Console.ReadLine` 은 키보드를 기다립니다. 그 결과 테스트가 실제 콘솔·시간에 묶이고, 같은 입력에도 결과가 달라져 비결정적이고, 외부 호출이 느리고, 테스트끼리 상태를 공유해 격리되지 않습니다.

> **Q3. 효과를 값으로 둔다는 것이 무슨 뜻입니까?** (§36.3)

부수 작용을 곧바로 일으키지 않고 "무엇을 할지" 만 담은 값으로 둔다는 뜻입니다. `Eff<RT, A>` 는 속을 보면 `Func<RT, IO<A>>` 라, 만드는 것만으로는 콘솔에 아무것도 찍히지 않습니다. 런타임 `RT` 를 넣어 `Run` 을 부를 때 비로소 실행됩니다. 실행이 한 자리로 미뤄지므로, 그 자리에 어떤 런타임을 넣느냐로 동작을 바꿀 수 있습니다.

> **Q4. `MemoryConsole` 은 어떻게 실제 콘솔을 대신합니까?** (§36.4.1)
...
> **Q5. 효과 코드는 진짜 콘솔인지 가짜인지 어떻게 모릅니까?** (§36.4.2)
...
> **Q6. 결정적이라는 것을 코드로 어떻게 확인합니까?** (§36.5.3)

같은 입력으로 두 번 실행해 출력이 같은지 봅니다. `DeterministicHolds` 는 매번 새 `MemoryConsole(["민수"])` 로 실행해 출력을 한 문자열로 잇고, 두 번 부른 결과가 같은지 (`Run() == Run()`) 를 단언합니다. 부수 작용이 값으로 인코딩되어 외부에 묶이지 않으므로 같은 입력은 항상 같은 출력을 냅니다. 명령형이었다면 두 번째 실행이 키보드를 기다리며 멈췄을 자리입니다.

> **Q7. 7부의 DI 설계가 테스트와 무슨 관계입니까?** (§36.6)
...
> **Q8. 콘솔 말고 시간·파일도 같은 방식으로 테스트합니까?** (§36.6)

그렇습니다. 시각을 읽는 능력 `IClock`, 파일을 다루는 능력 `IFileSystem` 도 `IConsole` 과 같은 모양입니다. 운영 런타임에는 실제 시계·파일 시스템을, 테스트 런타임에는 고정 시각 더블과 메모리 파일 시스템을 담습니다. 부수 작용이 무엇이든 능력 인터페이스로 두고 런타임으로 주입하면 모두 결정적으로 검사됩니다. 새 능력을 더하는 것이 36.7.2 챌린지입니다.

---

## 36.10 요약

- **순수 함수의 법칙 검증은 이미 끝났습니다.** 11부는 그 다음, 부수 작용이 섞인 효과 코드를 결정적으로 검사하는 전문 영역입니다 (§36.1).
- **명령형 IO 는 테스트하기 어렵습니다.** 부수 작용이 코드에 박혀 실제 콘솔·시간에 묶이고, 비결정적이고, 느리고, 격리되지 않습니다 (§36.2).
- **효과를 값으로 인코딩합니다.** `Eff<RT>` 는 만드는 것만으로는 부수 작용이 없고, 런타임을 넣어 `Run` 할 때 실행됩니다 (§36.3).
- **`MemoryConsole` 이 실제 콘솔을 대신합니다.** 입력은 큐로 꺼내고 출력은 리스트로 잡아, 실제 콘솔 없이 입출력을 코드로 쥡니다 (§36.4).
- **더블을 주입해 결정적으로 검사합니다.** 출력 정확성·입력별 변화·결정성을 단언으로 확인합니다 (§36.5).
- **7부의 DI 가 테스트 용이성입니다.** `Has<RT, Trait>` 로 능력을 런타임에 주입한 설계가 곧 테스트 더블을 끼우는 자리입니다 (§36.6).
- **실무로 옮기기는 한 걸음입니다.** `bool` 헬퍼에 `[Fact]` 를 붙이고 단언을 `ShouldBe` 로 바꾸면 표준 테스트가 됩니다 (§36.5.3).
...
| 장 | 대상 | 핵심 | 검증의 어려움 |
|---|---|---|---|
| **이 장 (36장)** | **효과 코드** (`Eff<RT>`) | **런타임 더블 주입** | **부수 작용** |
| 다음 장 (37장) | 동시 · 스트리밍 · 자원 | 비결정성 길들이기 | 시간 · 수명 |
| 38장 | 임의 입력 성질 | 생성기 + 축소 | 무한한 입력 공간 |

36장에서 부수 작용을 값으로 인코딩하고 런타임을 바꿔 끼워 효과 코드를 결정적으로 검사했습니다. 37장은 그 검사를 가장 까다로운 영역으로 밀고 갑니다. 동시성·스트리밍·자원 수명은 부수 작용에 더해 시간과 비결정성이 얽혀 있어, STM 의 원자성을 결정적으로 확인하고, 스트림을 골든 테스트로 검사하며, Schedule 의 재시도 횟수와 Resource 의 개폐 순서 같은 수명을 검사하는 전용 도구가 필요합니다. 9 ~ 10부에서 만든 동시·스트리밍 효과가 테스트로 이어지는 자리입니다. [37장 — 동시 · 스트리밍 · 자원 효과 테스트](./Ch37-Concurrent-Streaming.md) 로 넘어갑니다.
...
>
> **테스트 디딤돌** — 이 장의 더블 주입 패턴 (`Eff.Run(eff, new AppRT(con))`) 은 37장에서 비결정적 효과 (동시·스트리밍·자원) 의 검사로 확장됩니다. 콘솔 더블이 결정적 출력을 잡았듯, 37장의 결정적 스케줄과 골든 테스트가 시간과 수명을 길들입니다.
