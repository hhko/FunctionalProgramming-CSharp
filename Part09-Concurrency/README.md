...
`Atom<A>` 은 단일 공유 값을 락 없이 안전하게 갱신합니다. 학습용 `MyAtom<A>` 의 `Swap(Func<A, A>)` 를 직접 구현하며 읽기 → 새 값 계산 → CAS → 실패 시 재시도 루프를 봅니다. 갱신이 순수 함수 이기 때문에 재시도가 안전하다는 점 (부수 작용을 넣으면 왜 깨지는지) 을 코드로 확인합니다.
