...
이 장의 도구가 무슨 일을 하는지 한 문장으로 잡습니다. 30장이 지킨 것은 값 하나였지만, 실무의 공유 상태는 흔히 맵이나 리스트 같은 컬렉션입니다. 여러 스레드가 같은 사전에 동시에 키를 넣는 자리입니다. 이때 컬렉션 전체를 불변으로 두고, 30장처럼 그 참조 하나만 CAS 로 통째 갈아 끼우면, 동시 삽입도 값 하나를 지키던 그 방식 그대로 안전해집니다. 이것이 `AtomHashMap` 입니다. 곧 `AtomHashMap` 은 새 발상이 아니라 30장 `Atom` 을 컬렉션에 입힌 것입니다.
