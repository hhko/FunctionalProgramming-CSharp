...
**노리는 능력** — 15장에서 손으로 짠 배관이 *내부 모나드 M 에 대해 한 번만* 작성되어 재사용됨을 본다. `OptionT<M, A>` 의 `Bind` 는 M 이 무엇이든 통한다.
...
**노리는 능력** — 변환기가 내부 모나드에 *다형적* 임을 체감한다. 이것이 15장의 고정된 ReaderOption 과의 결정적 차이다.

## 실행

```bash
dotnet run --project code/Part4-MonadTransformers/Ch16-Transformer-Idea/Ch16.csproj
```
