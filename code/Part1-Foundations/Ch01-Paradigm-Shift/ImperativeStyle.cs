...
public static class ImperativeStyle
{
    ...
    {
        Console.WriteLine("== §1.2 명령형 — 절차의 나열 ==");

        // §1.2.1 — sum 변수가 매 반복마다 *변경* 된다.
        var numbers = new[] { 1, 2, 3, 4, 5 };
        Console.WriteLine($"  SumImperative([1..5]) = {SumImperative(numbers)}      (= 15)");

        // §1.2.3 — *상태가 흩어진다* — Order 의 캡슐화 약속이 깨지는 시연.
        Console.WriteLine();
        Console.WriteLine("  [§1.2.3 캡슐화 깨짐 — AddItem 우회 시연]");
        var order = new Order();
        ...
        // §1.2.4 — 명령형 + 동시성 = race condition.
        Console.WriteLine();
        Console.WriteLine("  [§1.2.4 동시성 + 명령형 = race condition 시연]");
        RaceConditionDemo();
    }
