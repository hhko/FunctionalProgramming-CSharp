...
// 본문 §1.3 — 객체 지향 (능력의 캡슐화).
//
// 명령형의 *상태가 흩어진다* 를 *상태 변경을 객체 안으로 좁힌다* 로 푼다.
// 그러나 *변경 자체* 가 남아 있어 §1.3.5 의 *동시성 + 객체 그래프* 비용은 그대로다.
public static class ObjectOrientedStyle
{
    public static void Run()
    {
        Console.WriteLine("== §1.3 객체 지향 — 능력의 캡슐화 ==");

        // §1.3.1 — Counter 가 상태를 *내부* 에 가둔다.
        ...
        // §1.3.2 — 4 대 원칙 중 *다형성* — 같은 인터페이스가 다른 동작을 표현.
        Console.WriteLine();
        Console.WriteLine("  [§1.3.2 다형성 — IShape.Area() 한 호출, 도형마다 다른 공식]");
        IShape[] shapes = [new Circle(3), new Square(4), new Triangle(3, 4)];
        foreach (var s in shapes)
            Console.WriteLine($"    {s.GetType().Name,-9}.Area() = {s.Area():F2}");

        // §1.3.4 — 약점: *Sum* 같은 능력이 컬렉션마다 별도 정의됨.
        Console.WriteLine();
        Console.WriteLine("  [§1.3.4 약점 — 같은 능력이 컬렉션마다 별도 정의]");
        Console.WriteLine("    MyArrayList.Sum()  — 별도 구현");
        Console.WriteLine("    MyLinkedList.Sum() — 별도 구현 (같은 의미인데도)");
        Console.WriteLine("    → 함수형은 이 한계를 *Foldable* trait 으로 푼다 (5장).");
    }
