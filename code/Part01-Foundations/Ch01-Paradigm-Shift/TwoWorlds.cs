...
// 비유: 두 평행 세계 (Normal / Elevated World).
//
...
public static class TwoWorlds
{
    ...
    {
        Console.WriteLine("== 두 평행 세계 — Normal World vs Elevated World ==");

        // Normal World 시민 + 평범한 함수 합성.
        Console.WriteLine("  [Normal World — 평범한 값과 함수]");
        int n = 42;
        string s = "hello";
        ...
        // Elevated World 시민 (컨테이너로 끌어올린 값).
        Console.WriteLine();
        Console.WriteLine("  [Elevated World — 끌어올린 값]");
        ...
        // 함수도 두 세계에 산다.
        Console.WriteLine();
        Console.WriteLine("  [함수도 두 세계에 산다]");
        Func<int, int> plus1 = n => n + 1;
        Option<Func<int, int>> maybePlus1 = Option.Some<Func<int, int>>(n => n + 1);
        ...
        Console.WriteLine("    → 5장 Applicative 의 `apply : E<a→b> → E<a> → E<b>` 가 이 자리의 함수를 쓴다.");
    }
}
