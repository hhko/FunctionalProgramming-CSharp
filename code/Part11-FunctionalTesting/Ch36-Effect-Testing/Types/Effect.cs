namespace Ch36.Types;

// ── 효과 스택 (7부 Ch26 의 Eff<RT,A> = ReaderT<RT, IO, A> 축약 재현) ──
...
// ReaderT<RT, IOF, A> = Eff<RT, A>.
public sealed class ReaderT<RT, A>(Func<RT, IO<A>> run) : K<ReaderTF<RT>, A>
{
    public IO<A> Run(RT rt) => run(rt);
}

public sealed class ReaderTF<RT> : MonadIO<ReaderTF<RT>>, Readable<ReaderTF<RT>, RT>
{
    static IO<A> RunInner<A>(K<ReaderTF<RT>, A> m, RT rt) => ((ReaderT<RT, A>)m).Run(rt);
    ...
    public static K<ReaderTF<RT>, B> Map<A, B>(Func<A, B> f, K<ReaderTF<RT>, A> fa) =>
        new ReaderT<RT, B>(rt => new IO<B>(() => f(RunInner(fa, rt).Run())));
    public static K<ReaderTF<RT>, B> Bind<A, B>(K<ReaderTF<RT>, A> ma, Func<A, K<ReaderTF<RT>, B>> f) =>
        new ReaderT<RT, B>(rt => new IO<B>(() => RunInner(f(RunInner(ma, rt).Run()), rt).Run()));
