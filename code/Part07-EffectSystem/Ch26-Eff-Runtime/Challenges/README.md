...
**노리는 능력** — `Eff<RT,A>` 가 `ReaderT<RT, IO, A>` (6부 변환기) 임을, 그리고 `Has<RT, IConsole>` 제약이 *능력 기반 DI* 를 컴파일타임에 보장함을 본다. `TestConsole` 로 부수 효과 없이 결정적으로 테스트할 수 있다 (11부 미리보기).
...
**노리는 능력** — 런타임에 능력을 *조합* 해 쌓는 패턴 (`where RT : Has<RT,IConsole>, Has<RT,IClock>`) 을 익힌다. 12부 실무 앱의 토대다.
