...
// ── 효과 스택 (Eff<RT,A> = ReaderT<RT, IO, A>) + 능력 기반 DI ──────────
...
public interface IStore { void Save(string id, decimal amount); decimal Total(); int Count(); }
...
public sealed class MemoryStore : IStore
{
    ...
    public void Save(string id, decimal amount) => orders[id] = amount;
