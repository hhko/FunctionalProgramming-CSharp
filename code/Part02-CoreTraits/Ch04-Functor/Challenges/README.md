...
본 디렉토리에는 [`§4.9`](../../../../Part01-Foundations/Ch03-Functor.md#39-직접-해보기--ch03-챌린지) 의 챌린지 정답이 들어 있다. *필수 챌린지* 두 개 + *심화 챌린지* 두 개로 구성된다.
...
**노리는 능력** — *3-tuple 패턴을 재귀 자료에도 그대로 적용*. *§4.6 의 두 법칙이 내 구현에서 성립함을 코드로 검증*.
...
**노리는 능력** — *§4.8.1 시그니처 분류표를 새 메서드에 직접 적용*. *모양 보존 + 원소 개수 + 변환 함수 + 시그니처 유형* 네 기준으로 판정.
...
```bash
dotnet run --project code/Part01-Foundations/Ch03-Functor/Ch03.csproj
```
