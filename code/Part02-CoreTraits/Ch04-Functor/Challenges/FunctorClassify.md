...
[본문 §4.9.2](../../../../Part01-Foundations/Ch03-Functor.md#392-챌린지--functor-인지-아닌지-분류하기-필수) 의 다섯 후보에 대한 정답과 근거.
...
본문 §4.8.4 + §4.2.3 의 정의를 기반으로 네 가지를 차례로 묻는다.
...
네 기준 모두 *예* 면 Functor 의 `Map`. 하나라도 *아니오* 면 *어느 trait* 의 자리를 §4.8.2 표에서 찾는다.
...
**자리** — 표준 trait 으로 분류하기 어려운 *Sequence 특화 연산*. 본문 §4.8.2 표의 *(컨테이너 특화)* 행에 해당.
...
**근거** — `Distinct` 는 *원소 개수* 가 감소한다 (중복 제거). 본문 §4.8.1 표의 *Filter 행* 과 같은 사유 — *컨테이너 종류는 같지만 원소 개수가 줄어든다*.
...
**근거** — 시그니처가 `int → Option<User>` 로 *Normal 입력 → Elevated 출력*. 1장의 *② 유형* (`a → E<b>`) — *World-crossing function*. Functor 의 `Map` 은 *④ 유형* 만 만든다. ② 유형의 합성은 `bind` 가 푼다 (본문 §4.8 끝의 8장 §6.2.3 참조).
...
즉 `Replace` 는 *Functor 의 `Map` 위에 자라난 파생 함수* 다. 본문 §4.5.3 의 `MapTwice` / `MapThenMap` 과 같은 *trait 위의 일반 함수* 자리.
...
| (e) Replace | ✗ (그러나 Map 으로 정의됨) | Functor 의 파생 함수 (§4.5.3) |
...
- (a) Reverse 가 막혔다면 — §4.2.1 의 `Map` 시그니처 (`Func<A, B> f` 가 *인자* 라는 점) 와 §4.2.3 의 ③ container operation 시각을 다시 본다.
- (b) Distinct 가 막혔다면 — §4.8.1 표의 *Filter / Take* 행과 §4.8.4 *좁은 약속* 을 다시 본다.
- (c) Average 가 막혔다면 — §4.1.2 의 ③ 유형 정의를 본다.
- (d) Lookup 이 막혔다면 — §4.1.2 의 ② 유형 정의 + §4.8 끝의 8장 §6.2.3 미리보기를 본다.
- (e) Replace 가 막혔다면 — §4.5 *어떤 Functor 든 받는 일반 함수* 절을 본다. *`Map` 위에 함수를 자라게 하는 패턴*.
