using Ch07.Traits;

namespace Ch07.Functions;

// LINQ from-select 사용을 위한 *확장 메서드*.
//
// C# 컴파일러는 LINQ 표기를 *Select / SelectMany 메서드 호출* 로 변환한다.
// 그래서 Monad<M> 에 SelectMany 가 있어도 *확장 메서드 형태* 로 노출돼야 한다.
//
// 즉 LINQ 표기 → SelectMany 확장 메서드 호출 → M.Bind 호출.
public static class MonadExtensions
{
    public static K<M, B> Select<M, A, B>(this K<M, A> ma, Func<A, B> f)
        where M : Monad<M> =>
        M.MapDefault(f, ma);

    public static K<M, C> SelectMany<M, A, B, C>(
        this K<M, A> ma,
        Func<A, K<M, B>> bind,
        Func<A, B, C> project)
        where M : Monad<M> =>
        M.SelectMany(ma, bind, project);

    // 이름 그대로의 Bind — *명시적 호출* 도 가능.
    public static K<M, B> Bind<M, A, B>(this K<M, A> ma, Func<A, K<M, B>> f)
        where M : Monad<M> =>
        M.Bind(ma, f);
}
