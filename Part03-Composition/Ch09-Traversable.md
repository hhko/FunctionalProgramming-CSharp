...
**그림 9-2. 층 swap: `List<E<a>>` → `E<List<a>>`** — 두 세계를 층으로 쌓아 봅니다. 왼쪽 `List<E<a>>` 는 위에서부터 `List` (바깥 컨테이너) · `E` (안쪽 효과) · 값 순으로 쌓인 모양이라, 효과가 원소마다 흩어져 있습니다. `traverse` 는 위 두 층 `List` 와 `E` 의 순서를 맞바꿔 효과 `E` 를 맨 위로 끌어올립니다. 그래서 오른쪽 `E<List<a>>` 는 `E` 가 위, `List` 가 가운데인 모양이 되고, 효과가 바깥에 한 번 모입니다. 값 (Normal) 은 두 경우 모두 바닥에 그대로 남습니다.
