...
└── Challenges/NaturalChallenges.cs   ← §11.7 정답
```
...
> **표기 안내** — 본문은 읽기 편하도록 변환 trait 을 `Natural<F, G>` 로 줄여 적지만, 실제 trait 선언은 variance 를 단 `Natural<out F, in G>` 입니다 (§11.3 코드). `out` / `in` 의 정확한 정의는 2장의 `K<in F, A>` 마커에서 짚은 변성 직감에 기대고, 여기서는 `F` 가 출발 컨테이너, `G` 가 도착 컨테이너라는 한 줄만 가져가면 충분합니다.
...
가짜 변환 (§11.5.1) 은 이 사각형이 닫히지 않습니다. 진짜는 어느 입력에서도 두 경로가 같은 값에서 만납니다.
...
`Transform` 이 인터페이스 `K<G, A>` 를 돌려주므로 구체 타입 `MyMaybe<…>` 로 다운캐스트합니다. 이 캐스트가 §11.3 의 `K<F, A>` 마커 어휘와 직접 연결되는 자리입니다.
...
위 검증은 `[1, 2, 3]` 같은 특정 값으로 확인했습니다. 그러나 자연성 법칙은 그 한 리스트만의 약속이 아니라 모든 입력에 대한 약속입니다. 3장 §3.7.1 의 `ForAll` 을 그대로 가져와, 표본 함수 `f = x => x + 1` 은 고정하고 컨테이너 입력만 변주해 임의의 `MyList<int>` 100 건으로 자연성 법칙을 검사합니다.
...
특정 값 검증이 "이 리스트에서 성립" 이라면, 임의 입력 검증은 "임의의 100 건에서 성립" 입니다. 임의 입력 검증은 3장 §3.7.1 의 `ForAll` 로 이 장에서 수행하고, 실패를 최소 반례로 줄이는 (shrinking) 본격 도구와 실무 도구 (CsCheck / FsCheck) 로의 이행은 11부로 넘어갑니다 (§11.10 테스트 디딤돌).
...
이 변환은 비유로 끝나지 않습니다. 합성 컨테이너 `MyList<MyMaybe<_>>` 와 `MyMaybe<MyList<_>>` 를 각각 하나의 `F` / `G` 로 두면, `sequence` 는 §11.4 의 `ListToMaybe` 와 똑같은 모양의 `Natural` 인스턴스로 실제로 빌드되고 실행됩니다.
...
> **본문 어느 자리의 이해도를 묻는가**
>
> 1. `Transform : K<F, A> → K<G, A>` (§11.2) 가 값 `A` 를 유지하고 컨테이너만 바꾼다는 것.
>
> **해보기**
>
...
> **본문 어느 자리의 이해도를 묻는가**
>
> 1. 자연성 법칙 `Transform(map(f, fa)) == map(f, Transform(fa))` (§11.5) 의 두 경로.
>
> **해보기**
>
...
>
> §11.5.1 의 가짜 `BogusEvenHead` 를 `[1, 2, 3]` 에 돌려 두 경로가 `Just(2)` 와 `Nothing` 으로 갈리는 것을 확인하고, 진짜 변환과 달리 왜 자연성을 깨는지 한 줄로 적습니다.
>
> **본문 어느 자리의 이해도를 묻는가**
>
> 1. 가짜가 값을 들여다보고 분기해 자연성 (§11.5.1) 을 깬다는 것.
>
> **해보기**
>
...
> **Q1. `map` 과 NaturalTransformation 은 어떻게 다릅니까?** (§11.2)
...
> **Q2. `Transform` 의 타입 매개변수가 왜 `A` 하나뿐입니까?** (§11.3)
...
> **Q4. `sequence` 가 NaturalTransformation 이라는 말은 무슨 뜻입니까?** (§11.6)
...
> **Q5. NaturalTransformation 이 기초의 마지막에 오는 이유는 무엇입니까?** (§11.6)
...
> **Q7. 변환을 trait 으로 묶으면 무엇이 좋습니까?** (§11.1.1)

값 `a` 와 무관하게 작동함이 시그니처로 강제됩니다 (§11.1.1). 자유 함수로 두면 누군가 변환 안에서 값을 바꿔도 컴파일러가 막지 못하지만, `Natural<F, G>` 의 `Transform<A>` 는 타입 매개변수가 `A` 하나뿐이라 값을 손댈 길이 닫힙니다.

> **Q8. 실무에서 NaturalTransformation 은 어디에 쓰입니까?** (§11.10)
...
- **NaturalTransformation 은 컨테이너 자체를 다른 컨테이너로 옮깁니다.** `Transform : K<F, A> → K<G, A>` 가 값 타입 `A` 는 유지한 채 컨테이너만 `F` 에서 `G` 로 바꿉니다 (§11.2).
- **4장 Functor 의 `map` 과 직교합니다.** `map` 은 컨테이너를 고정하고 값을 바꾸는 가로축, NaturalTransformation 은 값을 고정하고 컨테이너를 바꾸는 세로축입니다 (§11.2).
- **`Natural<out F, in G>` 는 source / target 두 컨테이너를 인자로 받습니다.** 출발 컨테이너 `F` 와 도착 컨테이너 `G` 두 인자를 받고 `Transform` 의 타입 매개변수는 값 `A` 하나뿐입니다. variance 표기 `out F, in G` 가 카테고리 이론의 source / target 비대칭을 시각화하고, K 마커의 `K<in F, A>` contravariant 표기가 variance 반전을 만들어 컴파일을 통과시킵니다 (LanguageExt v5 와 정확히 정합) (§11.3).
- **자연성 법칙이 변환이 값과 무관함을 보장합니다.** `Transform(map(f, fa)) == map(f, Transform(fa))` 가 변환이 값을 들여다보지 않고 컨테이너 구조만 다룬다는 것을 약속합니다 (§11.5).
- **시그니처는 맞지만 자연성을 깨는 가짜 변환이 있습니다.** 값이 짝수일 때만 첫 원소를 꺼내는 `BogusEvenHead` 가 값을 들여다보아 자연성 사각형을 닫지 못합니다 (§11.5.1).
- **9장 Traversable 의 `sequence` 가 사실 NaturalTransformation 의 한 형태입니다.** 기초의 모든 어휘가 컨테이너 사이의 다리로 마무리됩니다 (§11.6).
...
>
> **테스트 디딤돌** — 자연성 법칙 (`Transform(map(f, fa)) == map(f, Transform(fa))`) 은 이 장에서 3장 §3.7.1 의 `ForAll` 로 임의의 컨테이너와 함수에 검증했습니다. 무작위 생성기를 Functor·Monad 로 키우고 실패를 최소 반례로 줄이는 (shrinking) 본격 도구, 그리고 실무 도구 (CsCheck / FsCheck) 로의 이행은 11부입니다.
