...
특정 값 검증이 "이 리스트에서 성립" 이라면, 임의 입력 검증은 "임의의 100 건에서 성립" 입니다. 임의 입력 검증은 3장에서 본 `ForAll` 로 이 장에서 수행합니다. 실패를 최소 반례로 줄이는 (shrinking) 본격 도구와 실무 도구 (CsCheck / FsCheck) 로 넘어가는 것은 11부이며, 뒤의 테스트 디딤돌에서 다시 짚습니다.
...
>
> **테스트 디딤돌** — 자연성 법칙 (`Transform(map(f, fa)) == map(f, Transform(fa))`) 은 이 장에서 3장 3.10.6절의 `ForAll` 로 임의의 컨테이너와 함수에 검증했습니다. 무작위 생성기를 Functor·Monad 로 키우고 실패를 최소 반례로 줄이는 (shrinking) 본격 도구, 그리고 실무 도구 (CsCheck / FsCheck) 로의 이행은 11부입니다.
