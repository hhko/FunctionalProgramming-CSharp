...
깊은 계단은 보기 불편한 데서 그치지 않습니다. 더 큰 문제는 자원을 닫는 일이 효과 코드와 한 덩어리로 엉킨다는 점입니다. 7부에서 효과를 값으로 인코딩한 까닭을 떠올려 봅니다. 부수 효과를 `IO` 라는 값으로 묶어, `Run` 하기 전까지 미루고, 다른 효과와 합성할 수 있게 만들기 위해서였습니다. 그런데 자원을 `using` 으로 닫으면, 그 닫힘은 값이 아니라 구문 (statement) 의 블록 구조에 묶입니다. 효과를 값으로 들고 다니다가, 자원을 닫을 자리에서만 갑자기 블록 구문으로 돌아와야 합니다. 값으로 합성하던 흐름이 거기서 끊깁니다.
