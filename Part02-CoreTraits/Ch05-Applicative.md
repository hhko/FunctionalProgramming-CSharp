...
누적이 애초에 가능한 까닭을 한 자리 더 짚어 둡니다. `Apply` 의 두 입력 `K<F, a → b>` 와 `K<F, a>` 는 **서로 독립**입니다. 두 번째 컨테이너의 값이나 성공·실패 여부가 첫 번째 컨테이너의 결과에 달려 있지 않습니다. 그래서 두 컨테이너를 어느 쪽 먼저 꺼내도 결과가 같고, 양쪽을 모두 꺼내 본 다음에야 비로소 두 실패를 한자리에 모을 수 있습니다. 7장 `Bind` 의 두 번째 인자는 `a → E<b>` (World-crossing) 라 두 번째 컨테이너가 첫 번째의 결과를 받아야 만들어집니다. 두 입력이 **의존** 관계라 순서가 강제되고, 첫 실패에서 두 번째를 꺼낼 길 자체가 없어 단락만 가능합니다. **독립이면 모두 모으고, 의존이면 첫 실패에서 멈춘다** 는 이 갈림이 8장 Validation 의 누적과 7장 Monad 의 단락을 시그니처 단계에서 가르는 뿌리입니다.
