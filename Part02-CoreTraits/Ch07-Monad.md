...
> - **`Bind`**: `E<a> → (a → E<b>) → E<b>`. `E<a>` 에서 값을 꺼내 `a → E<b>` 에 넘기고 결과 `E<b>` 를 그대로 되돌리는 합성 되살리기 멤버
...
> - **Kleisli 합성** (`>=>`): 두 World-crossing 함수 `a → E<b>` 와 `b → E<c>` 를 하나의 `a → E<c>` 로 잇는 정식 합성. `Pure` 가 항등원 (`>=>` 는 Haskell·수학의 기호이고, C# 코드에서는 같은 일을 하는 `Then` 으로 적습니다)
...
static MyMaybe<int> ParseThenReciprocal(string s)
{
    ...
    if (parsed is MyMaybe<int>.Just j)   // 꺼내고
        return Reciprocal(j.Value);      // 다음 함수에 넘김
...
![World-crossing 함수 f 의 출력 E<b> 와 g 의 입력 b 가 한 층 어긋난다](./images/Ch07-Monad/01-diagonal-mismatch.svg)

**그림 7-1. 대각선의 어긋남: World-crossing 함수는 한 층 올라간 채 머문다** — 아래 Normal World 의 `a` 가 `f : a → E<b>` 로 위 Elevated World 의 `E<b>` 에 대각선으로 올라갑니다. 그런데 다음 함수 `g : b → E<c>` 의 입력 `b` 는 아래층에 있어, 위층 `E<b>` 를 그 입구에 그대로 꽂을 수 없습니다 (빨간 점선의 `?`). 두 World-crossing 함수가 출력·입력이 한 층 어긋난 대각선 두 개가 되는 자리이고, `bind` 가 이 대각선을 수평으로 펴 (`E<b>` 에서 `b` 를 끌어내려) 잇습니다.
...
`bind` 는 `E<a>` 에서 `a` 를 꺼내 World-crossing 함수에 넘기고, 그 함수가 낸 `E<b>` 를 그대로 돌려줍니다. **꺼냄 → 적용 → 되돌림** 의 세 동작이 한 멤버 안에 들어 있습니다. 앞서 손으로 쓴 `if (… is Just j)` 분기가 바로 이 세 동작이었고, `bind` 는 그것을 trait 한 자리로 모읍니다.
...
그래서 `Bind` 과 `Pure` 만 있으면 `Map` 도 `Apply` 도 강제됩니다 (`Apply` 의 유도식은 아래 박스). 셋을 따로 고르는 것이 아니라, 가장 강한 `Bind` 하나가 나머지 둘을 결정합니다. Monad ⊃ Applicative ⊃ Functor 라는 상속은 단순한 계층 표기가 아니라, 능력이 실제로 파생된다는 뜻입니다. 학습용 코드에서는 명료성을 위해 `Map` 과 `Apply` 를 자료마다 직접 적지만, 그 본체는 늘 이 유도식과 같습니다.

> **더 깊이 — `Apply` 도 `Bind` 로 유도됩니다 (처음엔 건너뛰어도 됩니다)** — `Apply` 도 `Map` 과 같은 방식입니다. 함수를 `Bind` 로 꺼내고 (`f`), 값을 `Bind` 로 꺼낸 뒤 (`a`), `Pure(f(a))` 로 되올리면 됩니다. 곧 `Apply(mf, ma) = Bind(mf, f => Bind(ma, a => Pure(f(a))))` 입니다.

![bind 는 E<a> 의 a 를 꺼내 a → E<b> 에 넘기고 E<b> 로 되돌립니다](./images/Ch07-Monad/02-bind-world-crossing.svg)

**그림 7-2. `bind`: World-crossing 함수의 합성 되살리기** — 위 행 Elevated World 에 `E<a>` 와 `E<b>` 두 컨테이너. 아래 행 Normal World 에 꺼낸 값 `a`. `bind` 는 먼저 `E<a>` 에서 `a` 를 꺼내 (왼쪽 아래 화살표), World-crossing 함수 `f : a → E<b>` 에 넘겨 (오른쪽 위 화살표) `E<b>` 로 되돌립니다. 가운데 점선 `bind` 화살표가 이 세 동작을 한 멤버로 묶어 `E<a>` 를 `E<b>` 로 잇는 모습입니다.

![Map(f, ma) 좌항과 Bind(ma, a => Pure(f(a))) 우항이 같은 E<b> 에서 만난다](./images/Ch07-Monad/03-map-derive-commute.svg)

**그림 7-3. `Map` 유도: 좌항 = 우항** — 좌항 `Map(f, ma)` 는 Elevated World 안에서 `E<a>` 를 곧장 `E<b>` 로 옮깁니다 (Functor 직접). 우항 `Bind(ma, a => Pure(f(a)))` 는 `bind` 로 `a` 를 Normal 로 끌어내려 평범한 `f` 를 적용한 뒤 `Pure` 로 다시 끌어올립니다. 두 경로가 같은 `E<b>` 에서 만나므로 좌항과 우항이 같고, `Bind` 과 `Pure` 만 있으면 `Map` 이 이렇게 강제됩니다.
...
public static K<MyMaybeF, B> Bind<A, B>(K<MyMaybeF, A> ma, Func<A, K<MyMaybeF, B>> f) =>
    ma.As() switch
    {
        MyMaybe<A>.Just j  => f(j.Value),                 // 꺼내 다음 함수에 넘김
        MyMaybe<A>.Nothing => MyMaybe<B>.Nothing.Instance, // 꺼낼 게 없으면 f 호출 안 함
        _ => throw new InvalidOperationException()
    };
```

`Just` 면 안의 값을 꺼내 World-crossing 함수 `f` 에 넘깁니다. `Nothing` 이면 **`f` 를 아예 호출하지 않고** 곧장 `Nothing` 을 돌려줍니다. 이 한 줄이 단락 회로 (short-circuit) 입니다. 앞 단계가 실패하면 뒤 단계는 평가조차 되지 않습니다.
...
K<MyMaybeF, int> r =
    ParseInt("3").Bind(a =>
        ParseInt("xyz").Bind(b =>   // ← Nothing
            ParseInt("5").Bind(c =>  // ← 호출조차 안 됨
                MyMaybeF.Pure(a + b + c))));
// → Nothing
```
...
`Monad` 은 특정 컨테이너에 딸린 요령이 아니라, 어떤 Elevated World 든 World-crossing 함수를 잇는 일반 어휘입니다. `bind` 의 의미는 늘 "꺼내서 다음 함수에 넘기고 다시 모은다" 한 가지이고, 그 모으는 방식이 자료에 따라 단락도 비결정성도 됩니다.

`bind` 의 세 번째 얼굴은 상태 (state) 입니다. 컨테이너가 값이 아니라 함수 `S → (A, S)` (상태 `S` 를 받아 결과 `A` 와 바뀐 상태 `S` 를 함께 돌려주는 함수) 인 자료로, 숨은 상태를 사슬을 따라 다음 단계로 흘려보냅니다. 모양이 한 단계 더 복잡하므로 여기서는 이름과 모양만 짚고, 본격적인 구현은 State 모나드를 다루는 후속 Part 로 미룹니다. Wlaschin 의 〈Dr Frankenfunctor and the Monadster〉 연재가 이 상태 threading 을 "생명력을 흘려보내며 괴물을 조립" 하는 비유로 풀어낸 입문 자료입니다. 모양은 더 복잡해도 약속은 같습니다. 같은 `bind` 이 단락·비결정성·상태 세 얼굴을 모두 한 어휘로 잇습니다.
...
![Apply 의 둘째 인자는 독립된 값, Bind 의 둘째 인자는 a 를 받는 함수](./images/Ch07-Monad/06-apply-vs-bind.svg)

**그림 7-6. `Apply` 독립 결합 vs `Bind` 의존 결합** — `Apply` 의 둘째 인자는 이미 정해진 Elevated 값 `E<a>` 라, 두 입력이 서로를 모른 채 결합합니다 (독립·병렬). `Bind` 의 둘째 인자는 `a => E<b>` 함수라, 다음 Elevated 값이 앞에서 꺼낸 `a` 에 따라 정해집니다 (의존·순차). 이 한 가지 차이가 applicative 의 누적과 monadic 의 단락을 가릅니다.
...
![f >=> g 는 a → E<b> 와 b → E<c> 를 한 번에 잇습니다](./images/Ch07-Monad/07-kleisli-composition.svg)

**그림 7-7. Kleisli 합성: `f >=> g`** — 그림 7-1 에서 한 층 어긋나 곧장 잇지 못하던 두 World-crossing 함수를 `bind` 가 잇습니다. `a` 가 `f` 로 `E<b>` 에 오르고, `bind` 가 `E<b>` 에서 `b` 를 한 층 끌어내려 (그림 7-1 의 빨간 `?` 자리) `g` 의 입구에 맞추면, `g` 가 다시 `E<c>` 로 올립니다. 올림 → 내림 → 올림 의 이 경로 전체가 한 함수 `f >=> g : a → E<c>` 입니다. 두 World-crossing 함수 `f` 와 `g` 가 `∘` 처럼 합성됩니다.
...
중첩 `Bind` 와 글자 그대로 같은 사슬에 도착합니다. 같은 값을 두 번 본 관찰이 아니라, LINQ 가 `Bind` 의 다른 표기 (syntactic sugar) 임을 대입으로 확인한 것입니다. `LINQ` 의 친숙한 문법이 사실 Monad 의 합성이었습니다.
...
바깥 `Bind` 가 `FindUser` 에서 `user` 를 꺼내 `FindAccount` 에 넘기고, 안쪽 `Bind` 가 `account` 를 꺼내 `Withdraw` 에 넘깁니다. 각 단계가 앞 단계의 성공 값을 받아야 진행됩니다. 같은 파이프라인을 LINQ 로 적으면 의존 관계가 더 또렷합니다.
...
![Map 으로 E<a> 가 E<E<b>> 로 겹치고 flatten 으로 E<b> 로 펴진다; Bind 는 한 걸음](./images/Ch07-Monad/08-flatten.svg)

**그림 7-8. `flatten` 으로 중첩 펴기 (`Bind` = `Map` + `flatten`)** — `Map(f)` 는 `E<a>` 에 World-crossing 함수 `f` 를 적용해 `E<E<b>>` 로 한 겹 더 겹치고, `flatten` 이 바깥 한 겹을 벗겨 `E<b>` 로 폅니다. `Bind(f)` 는 이 두 걸음을 한 걸음으로 합쳐 곧장 `E<b>` 를 냅니다. 두 경로가 같은 `E<b>` 에 도착하므로 `Bind` 는 `Map` 한 뒤 `flatten` 한 것과 같습니다.
...
타입만 맞춘 가짜 Monad 가 정말 컴파일되는지 직접 봅니다. 꺼낸 값을 다음 함수에 넘기지 않고 버리는 가짜 `Bind` 입니다.
...
`Bind : E<a> → (a → E<b>) → E<b>` 입니다. 세 자리는 각각 이미 Elevated 에 있는 값 (`E<a>`, 앞 단계의 결과), World-crossing 함수 (`a → E<b>`, 다음 단계), 이어진 결과 (`E<b>`) 입니다. `bind` 는 `E<a>` 에서 `a` 를 꺼내 함수에 넘기고 그 함수가 낸 `E<b>` 를 그대로 돌려줍니다. 꺼냄 → 적용 → 되돌림 세 동작이 한 멤버 안에 있고, 세 `E` 는 모두 같은 한 컨테이너입니다. 예를 들어 `Bind(Just(3), n => Just(n + 1))` 은 `3` 을 꺼내 `Just(4)` 를 냅니다.
