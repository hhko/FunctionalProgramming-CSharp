...
>
> - **§4.1 목적** — §1.6.1 의 1인자 끌어올림 자리. 컨테이너마다 변환 코드를 다시 쓰는 반복을 `map` 한 번 정의로 해소합니다.
...
> - **§4.10 ~ §4.13 마무리** — 다시 읽기 · Q&A · 요약 · 5장 Applicative 다리입니다.
...
이 `map` 이 **함수형의 본질** (합성 가능한 Elevated World 로 lift, §1.6.1) 의 끌어올림이 가장 단순한 형태로 등장하는 자리입니다. `Map : (a → b) → (E<a> → E<b>)` 는 끌어올림의 1인자 형태입니다. Normal World 의 함수 한 개를 Elevated World 에서 그대로 쓸 수 있게 끌어올리는 사다리입니다. 5장 Applicative 의 `Lift2 / Lift3 / Lift4` 가 다인자 확장이고, 4장은 그 사다리의 원형을 다룹니다.
...
└── Challenges/TreeFunctor.cs …    ← §4.9 정답 (재귀 자료 등)
```
...
위 두 [Fact] 는 고정한 입력 하나만 확인합니다. 3장 §3.7.1 의 `ForAll` 로 이 두 법칙도 임의 입력 100 건에 검증합니다. 표본 함수 `f` / `g` 는 고정하고 컨테이너 입력만 무작위로 변주합니다.
...
Normal World 의 함수에는 두 가지 핵심 구조가 있습니다. Map 이 bridge 로 제대로 작동하려면 **둘 다** 옮겨야 합니다. 하나만 옮기면 어느 한쪽 구조가 보존되지 않습니다.
...
함수형 trait 의 장점은 각 trait 가 좁은 능력만 약속하는 데 있습니다. 좁은 약속 → 강한 보장 → 추론 단순화.
...
- `Functor<F>` trait 는 `E<a> → E<b>` 유형 함수의 일반화입니다 (1장 4 가지 함수 유형의 `E<a> → E<b>`). *함수형의 본질 — 합성 가능한 Elevated World 로 lift* (§1.6.1) 의 끌어올림이 가장 단순한 형태로 등장하는 자리입니다.
...
- 두 법칙 (항등·합성) 이 모양 보존 약속을 구현 사이의 공통성으로 확정합니다. 셋째 축 합성 가능성 (§1.6.1) 이 코드에 박히는 자리입니다.
- 5장의 Applicative 는 2인자 이상 함수 (`Lift2`, `Lift3`) 로 확장합니다. 7장의 Monad 는 World-crossing 함수 (`a → E<b>`) 를 `E<a> → E<b>` 유형으로 끌어올리는 `bind` 를 추가합니다.

네 줄이 4 장이 기초 안에서 차지하는 위치의 요약입니다. 끌어올림의 1 인자 원형이 4 장, 다인자 확장이 5 장, 반대 방향 (lower) 이 6 장, World-crossing 합성 살리기가 7 장입니다. 한 동사 끌어올림이 기초 모든 추상의 공통 지도입니다. 비유는 여기까지가 역할입니다. 정확한 모양 보존 규칙은 두 법칙 (§4.6) 이 정합니다.
...
> **Q2. C# 의 LINQ `Select` 가 사실 Functor 의 `Map` 입니까?** (§4.2)
...
> **Q3. 세 가지 호출 어법 (trait 정적 / 모듈 / 확장) 중 언제 어느 것을 쓰나?** (§4.2)
...
> **Q4. `K<F, A>` 다운캐스트 (`fa.As()`) 가 어색합니다. 자동으로 안 되나?** (§4.3)
...
> **Q5. 시그니처는 통과하지만 법칙은 어긴 "가짜 Functor" 가 정말 존재할 수 있나?** (§4.6)
...
> **Q6. Functor 의 두 법칙이 왜 함께 필요한가?** (§4.6)
...
> **Q7. `Map` 이 부수 효과를 가지면 왜 합성 법칙이 깨지나?** (§4.6)
...
> **Q8. `Map` 안에서 예외가 발생하면 어떻게 되나?** (§4.8)
...
> **Q9. Functor 가 왜 첫 정통 trait 입니까?** (§4.1)
...
> **Q10. `Map` 이 어떤 컨테이너에 적용 가능한가 — 실용적 판단 기준은?** (§4.8)
...
- **`map` 은 1인자 함수의 끌어올림입니다.** `(a → b) → (E<a> → E<b>)` 가 Normal World 함수 하나를 Elevated World 에서 그대로 쓰도록 끌어올립니다. 함수형의 본질 (합성 가능한 Elevated World 로 lift) 의 가장 단순한 형태입니다 (§4.1).
- **모양 보존이 시그니처에 박혀 있습니다.** `Map<A, B>(Func<A, B> f, K<F, A> fa)` 에서 `F` 가 입력과 출력에 같이 등장해, 컨테이너 모양은 그대로 두고 안의 값만 바꾼다는 약속이 컴파일 단계에서 강제됩니다 (§4.2).
- **3-tuple 패턴으로 부착합니다.** 자료 (`MyList<A>`) / 태그 (`MyListF`) / trait (`Functor<F>`) 세 조각과 `.As()` 다운캐스트로 새 컨테이너를 Functor 로 등록합니다 (§4.3, §4.4).
- **세 호출 어법은 같은 곳으로 디스패치됩니다.** trait 정적 (`MyListF.Map`) / 모듈 (`Functor.map`) / 확장 (`fa.Map`) 셋 다 같은 `Functor<F>.Map` 입니다 (§4.5).
- **두 법칙이 시그니처가 약속 못 하는 성질을 채웁니다.** 항등 (`Map(x => x) == 그대로`) 과 합성 (`Map(g, Map(f, fa)) == Map(g ∘ f, fa)`) 은 컴파일러가 검증하지 못하는 사용 계약입니다 (§4.6).
- **`Map` 은 함수 합성을 보존합니다.** Normal 세계의 `∘` 가 Elevated 안에서도 그대로 성립합니다 (§4.7).
- **시그니처는 맞지만 법칙을 깨는 가짜 Functor 가 있습니다.** `BogusListF` (항등 위반) 와 `CountingListF` (합성 위반) 가 법칙이 시그니처와 별도인 계약임을 보입니다 (§4.8).
...
>
> **테스트 디딤돌** — §4.6 의 두 법칙 (항등 / 합성) 은 이 장에서 3장 §3.7.1 의 `ForAll` 로 임의 입력 100 건에 검증했습니다. 무작위 생성기를 Functor·Monad 로 키우고 실패를 최소 반례로 줄이는 (shrinking) 본격 도구, 그리고 실무 도구 (CsCheck / FsCheck) 로의 이행은 11 부 (Functional Testing) 입니다.