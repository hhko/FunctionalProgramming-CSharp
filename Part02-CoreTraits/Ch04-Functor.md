...
>
> **테스트 디딤돌** — 4.6절의 두 법칙 (항등 / 합성) 은 이 장에서 3장 3.7.1절의 `ForAll` 로 임의 입력 100 건에 검증했습니다. 무작위 생성기를 Functor·Monad 로 키우고 실패를 최소 반례로 줄이는 (shrinking) 본격 도구, 그리고 실무 도구 (CsCheck / FsCheck) 로의 이행은 11 부 (Functional Testing) 입니다.