...
이 `map` 이 **함수형의 본질** (합성 가능한 Elevated World 로 lift, §1.6.1) 의 끌어올림이 가장 단순한 형태로 등장하는 자리입니다. `Map : (a → b) → (E<a> → E<b>)` 는 끌어올림의 1인자 형태로, Normal World 의 함수 한 개를 Elevated World 에서 그대로 쓸 수 있게 끌어올리는 사다리입니다. 5장 Applicative 의 `Lift2 / Lift3 / Lift4` 가 다인자 확장이고, 4장은 그 사다리의 원형을 다룹니다.
...
추상 trait 만으로는 동작하지 않습니다. 2장 §2.8 의 3-tuple 패턴으로 첫 자료 타입에 부착합니다.
