...
이 책은 12 개 Part 로 구성됩니다. **Part 1 ~ 4 와 Part 11 의 본문과 실행 코드 (`code/`) 가 구현** 되어 있고, 이 책은 모든 Part 가 **`K<F, A>` 직접 구현과 Elevated World 비유** 두 축 위에서 자란다는 점이 차별점입니다.
...
│
├── Part04-Collections/                 : 4부 (완성) — 불변 컬렉션, 추상의 실전 적용
...
│
├── Part11-FunctionalTesting/           : 11부 (완성) — 효과·전문성 테스트
...
본문 (`Part…/*.md`) 과 코드 (`code/Part…/Ch…/*`) 가 시그니처 단계에서 정합합니다. 본문을 읽으면서 해당 코드를 IDE 에서 직접 실행 / 수정해 볼 수 있습니다 (위 트리에서는 코드 계층을 생략했습니다. Part 5 ~ 10 과 12 는 예정).
...
각 챕터 코드는 `code/PartNN-…/ChNN-…/` (Part 1 ~ 4 와 11 은 완성, Part 5 ~ 10 과 12 는 예정) 에 독립적으로 들어 있어 한 챕터만 따로 실행해 볼 수 있습니다. 검증은 `Tests/` 의 콘솔 `bool` 헬퍼로 이뤄지며 (각 trait 장이 3 장 §3.7.1 의 `ForAll` 로 법칙을 임의 입력에 검증), 11부 (Ch36 ~ Ch38) 에서 효과 코드의 결정적 테스트와 xUnit + Shouldly / property-based 심화로 옮기는 법을 다룹니다. 각 챕터의 `Challenges/` 에는 직접 해보기 정답 코드가 들어 있습니다.
