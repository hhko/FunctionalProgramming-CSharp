...
OO 개발자의 눈으로 옮기면 이렇습니다. 명령형이라면 `s = step1(s); s = step2(s);` 처럼 같은 변수 `s` 에 단계마다 새 값을 덮어쓰며 이어 갔을 것입니다. `Bind` 의 ①과 ②가 하는 일이 바로 그 "앞 단계 결과를 다음 단계 입력으로" 입니다. 다른 점은 가변 변수를 덮어쓰는 대신, 매 단계가 새 상태 값을 다음 함수에 넘긴다는 것뿐입니다. Part 1 에서 명령형의 `sum = sum + n` 과 함수형의 "매번 새 값을 다음으로" 를 견준 그 대비가, 여기서는 상태 위에서 일어납니다. 7장에서 `Bind` 를 프로그래밍 가능한 세미콜론이라 불렀던 것을 떠올리면 이 그림이 또렷합니다. 명령형의 세미콜론 두 개가 앞 줄의 상태 `s` 를 다음 줄로 그냥 넘겼다면, State 의 `Bind` 는 그 이음매에 상태 스레딩이라는 규칙을 박아 첫 단계의 새 상태를 다음 단계로 실어 나릅니다.
...
- `gets` 와 `Pure` 는 상태나 상수를 Elevated 로 올리는 `a → E<a>` 끌어올림 자리입니다.
